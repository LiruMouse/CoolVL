...
void LLCharacter::updateMotions(e_update_t update_type)
{
	...
	{
		LLFastTimer t(LLFastTimer::FTM_UPDATE_HIDDEN_ANIMATION);
		mMotionController.updateMotionsMinimal();
	}
	else
	{
		LLFastTimer t(LLFastTimer::FTM_UPDATE_ANIMATION);
		// unpause if the number of outstanding pause requests has dropped to
		// the initial one
...
BOOL LLCharacter::setVisualParamWeight(const char* param_name, F32 weight, BOOL upload_bake)
{
	...
	llwarns << "LLCharacter::setVisualParamWeight() Invalid visual parameter: "
			<< param_name << llendl;
	return FALSE;
}
...
void LLCharacter::addVisualParam(LLVisualParam *param)
{
	...
	if (!idxres.second)
	{
		llwarns << "Visual parameter " << param->getName()
				<< " already exists with same ID as "
