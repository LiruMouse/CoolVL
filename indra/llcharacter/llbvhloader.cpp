...
// LLBVHLoader()
//-----------------------------------------------------------------------------
LLBVHLoader::LLBVHLoader(const char* buffer, ELoadStatus& loadStatus,
						 S32& errorLine)
{
	reset();
	errorLine = 0;
	...
	else
	{
		if (mStatus != E_ST_OK)
		{
			//llwarns << "ERROR: [line: " << getLineNumber() << "] " << mStatus
			//		<< llendl;
...
//			llinfos << "NOTE: Priority: " << mPriority << llendl;
			...
			LLStringUtil::compareInsensitive(token, "constraint") == 0)
		{
			...
			if (sscanf(mLine,									/* Flawfinder: ignore */
...
void LLBVHLoader::applyTranslations()
{
	...
	for (ji = mJoints.begin(); ji != mJoints.end(); ++ji)
	{
		...
		{
			//llinfos << "NOTE: Changing " << joint->mName.c_str() << " to "
			//		<< trans.mOutName.c_str() << llendl;
		...
		{
//			llinfos << "NOTE: Removing 1st position offset from all keys for "
//					<< joint->mOutName.c_str() << llendl;
		...
		{
//			llinfos << "NOTE: Removing 1st rotation from all keys for "
//					<< joint->mOutName.c_str() << llendl;
...
BOOL LLBVHLoader::serialize(LLDataPacker& dp)
{
	...
	for (ji = mJoints.begin(); ji != mJoints.end(); ++ji)
	{
		...
		S32 outcount = 0;
		S32 frame = 1;
		for (ki = joint->mKeys.begin(); ki != joint->mKeys.end(); ++ki)
		{
			if (frame == 1 && joint->mRelativeRotationKey)
			{
				...
				fixup_rot.shortestArc(LLVector3::z_axis * first_frame_rot * frameRot,
									  LLVector3::z_axis);
			}
		...
		if (ji == mJoints.begin() && !joint->mIgnorePositions)
		{
			...
			for (ki = joint->mKeys.begin(); ki != joint->mKeys.end(); ++ki)
			{
				...
				U16 x, y, z;
				outPos.quantize16(-LL_MAX_PELVIS_OFFSET, LL_MAX_PELVIS_OFFSET,
								  -LL_MAX_PELVIS_OFFSET, LL_MAX_PELVIS_OFFSET);
				x = F32_to_U16(outPos.mV[VX], -LL_MAX_PELVIS_OFFSET,
							   LL_MAX_PELVIS_OFFSET);
				y = F32_to_U16(outPos.mV[VY], -LL_MAX_PELVIS_OFFSET,
							   LL_MAX_PELVIS_OFFSET);
				z = F32_to_U16(outPos.mV[VZ], -LL_MAX_PELVIS_OFFSET,
							   LL_MAX_PELVIS_OFFSET);
				dp.packU16(x, "pos_x");
				dp.packU16(y, "pos_y");
				dp.packU16(z, "pos_z");
	...
	for (ConstraintVector::iterator constraint_it = mConstraints.begin();
		 constraint_it != mConstraints.end(); ++constraint_it)
		{
			...
			dp.packBinaryDataFixed((U8*)constraint_it->mSourceJointName, 16,
								   "source_volume");
			dp.packVector3(constraint_it->mSourceOffset, "source_offset");
			dp.packBinaryDataFixed((U8*)constraint_it->mTargetJointName, 16,
								   "target_volume");
