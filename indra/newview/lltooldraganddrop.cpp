...
void LLCategoryDropDescendentsObserver::done()
{
	...
	for ( ; it != end; ++it)
	{
		gInventory.collectDescendents((*it), cats, items,
									  ...
									  const LLUUID& src_id)
{
	...
	if (LLToolDragAndDrop::getInstance()->mSource == SOURCE_WORLD ||
		LLToolDragAndDrop::getInstance()->mSource == SOURCE_NOTECARD)
	{
		llwarns << "Call to LLToolDragAndDrop::dropInventory() from world or notecard."
				<< llendl;
		return;
	}
...
struct LLGiveInventoryInfo
{
	...
	LLUUID mIMSessionID;
	LLGiveInventoryInfo(const LLUUID& to_agent,
						const LLUUID& obj_id,
												...
												const LLUUID& im_session_id)
{
	...
	pack_instant_message(gMessageSystem, gAgent.getID(), FALSE,
						 gAgent.getSessionID(), to_agent, name,
	...
	if (im_session_id != LLUUID::null && gIMMgr)
	{
		LLSD args;
											  ...
											  const LLUUID& im_session_id)
{
	...
	else
	{
		...
		else 
		{
			...
			LLNotifications::instance().add("CannotCopyCountItems", args,
											payload,
...
//mk
	...
	if (drop)
	{
		...
		{
			LLAppearanceMgr::instance().rezAttachment(item, 0);
		}
	}
...
/*
EAcceptance LLToolDragAndDrop::dad3dTextureSelf(LLViewerObject* obj,
												S32 face, MASK mask, BOOL drop)
{
	LL_DEBUGS("DragAndDrop") << "LLToolDragAndDrop::dad3dTextureAvatar()"
							 << LL_ENDL;
	if (drop)
	{
		if (!(mask & MASK_SHIFT))
											 ...
											 S32 face, MASK mask, BOOL drop)
{
	...
	if (mSource == SOURCE_AGENT || mSource == SOURCE_LIBRARY)
	{
		...
		if (drop)
		{
			...
			LLAppearanceMgr::instance().wearItemOnAvatar(item->getUUID(),
														 false);
		}
		return ACCEPT_YES_MULTI;
	}
...
//mk
	...
	if (mSource == SOURCE_AGENT)
	{
		...
		if (drop)
		{
			...
			LLAppearanceMgr::instance().wearInventoryCategory(category, false,
															  append);
		}
		return ACCEPT_YES_MULTI;
	}
	else if (mSource == SOURCE_LIBRARY)
	{
		if (drop)
		{
			LLAppearanceMgr::instance().wearInventoryCategory(category, true,
															  false);
		}
		return ACCEPT_YES_MULTI;
	}
															...
															BOOL drop)
{
	...
	for (i = 0; i < count; ++i)
	{
		rv = gInventory.isCategoryComplete(cats.get(i)->getUUID()) ? ACCEPT_YES_MULTI
																   : ACCEPT_NO;
