	...
	else if (source_sections < dest_sections)
	{
			 ...
			 section -= num_steps)
		{
			...
			for (S32 step = 1; step < num_steps; ++step)
			{
				...
				dest[section + step].mAxisRotation = slerp(t,
														   last_source_section->mAxisRotation,
														   source_section->mAxisRotation);

				// Evaluate output interpolated values
				F32 t_sq = t * t;
				dest[section + step].mPosition = t_sq * (t * A + B) + t * C + D;
				dest[section + step].mRotation = slerp(t,
													   last_source_section->mRotation,
													   source_section->mRotation);
				dest[section + step].mVelocity = lerp(last_source_section->mVelocity,
													  source_section->mVelocity,
													  t);
				dest[section + step].mDirection = lerp(last_source_section->mDirection,
													   source_section->mDirection,
													   t);
				dest[section + step].mdPosition = lerp(last_source_section->mdPosition,
													   source_section->mdPosition,
													   t);
...
void LLVolumeImplFlexible::setAttributesOfAllSections(LLVector3* inScale)
{
	...
	mSection[0].mScale.setVec(scale.mV[VX] * bottom_scale.mV[0],
	...
	{
		mSection[i].mAxisRotation.setQuat(lerp(begin_rot, end_rot, t), 0, 0,
										  1);
...
BOOL LLVolumeImplFlexible::doIdleUpdate(LLAgent& agent, LLWorld& world,
										const F64& time)
{
	...
	F32 app_angle = llround((F32)atan2(mVO->getScale().mV[2] * 2.f,
									   mVO->mDrawable->mDistanceWRTCamera) * RAD_TO_DEG,
							0.01f);

	// Rendering sections increases with visible angle on the screen
	mRenderRes = (S32)(FLEXIBLE_OBJECT_MAX_SECTIONS * 4 * app_angle *
					   DEG_TO_RAD / LLViewerCamera::getInstance()->getView());
	...
	else if	(visible && !mVO->mDrawable->isState(LLDrawable::IN_REBUILD_Q1) &&
			 mVO->getPixelArea() > 256.f)
	{
		...
		U32 update_period = (U32)(LLViewerCamera::getInstance()->getScreenPixelArea() *
								  0.01f / (pixel_area * (sUpdateFactor + 1.f))) + 1;

		if ((LLDrawable::getCurrentFrame() + id) % update_period == 0)
		{
			gPipeline.markRebuild(mVO->mDrawable, LLDrawable::REBUILD_POSITION,
								  FALSE);
		}
	}
...
void LLVolumeImplFlexible::doFlexibleUpdate()
{
	...
	LLVector3 AnchorPosition = BasePosition -
							   (anchorScale.mV[VZ] / 2 * anchorDirectionRotated);
	...
	F32 wind_factor = mAttributes->getWindSensitivity() * 0.1f *
					  section_length * secondsThisFrame;
	...
	for (i = 1; i <= num_sections; ++i)
	{
		...
		{
			mSection[i].mPosition += gAgent.getRegion()->mWind.getVelocity(mSection[i].mPosition) *
									 wind_factor;
		}
	...
	for (i = 1; i < num_sections; ++i)
	{
		...
		LLVector3 a = (mSection[i - 1].mPosition - mSection[i].mPosition +
					   mSection[i + 1].mPosition - mSection[i].mPosition) *
					   0.5f * inv_section_length * inv_section_length;
		LLVector3 b = mSection[i + 1].mPosition - mSection[i].mPosition -
	...
	// i = NumSections
	mSection[i].mdPosition = (mSection[i].mPosition - mSection[i - 1].mPosition) *
							 inv_section_length;
	...
	for (i = 0; i <= num_render_sections; ++i)
	{
		...
		if (!mUpdated ||
			(new_point->mPos-pos).magVec() / mVO->mDrawable->mDistanceWRTCamera > 0.001f)
		{
...
void LLVolumeImplFlexible::updateRelativeXform()
{
	...
	delta_pos = vo->mDrawable->isSpatialRoot() ? LLVector3(0, 0, 0)
											   : vo->mDrawable->getPosition();
