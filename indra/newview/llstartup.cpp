...
public:
	...
	void result(const LLSD& content)
	{
		...
		gInventory.setLibraryOwnerID(content["Skeleton"][0]["owner_id"]);
		gInventory.setLibraryRootFolderID(content["Skeleton"][0]["folder_id"]);
		LL_DEBUGS("Inventory") << " All the inventory_library_skeleton content: \n "
							   << ll_pretty_print_sd(content) << LL_ENDL;
		LL_DEBUGS("Inventory") << " Library Owner: "
							   << gInventory.getLibraryOwnerID().asString()
							   << LL_ENDL;
	}
};
	...
	if (STATE_LOGIN_CLEANUP == LLStartUp::getStartupState())
	{
		...
		gSavedSettings.setString("PerAccountSettingsFile",
								 gDirUtilp->getExpandedFilename(LL_PATH_PER_SL_ACCOUNT, 
																LLAppViewer::instance()->getSettingsFilename("Account",
																											 "PerAccount")));
...
//#define LL_MINIMIAL_REQUESTED_OPTIONS
	...
	{
		LL_DEBUGS("AppInit") << "STATE_XMLRPC_LEGACY_LOGIN" << LL_ENDL;
		progress += 0.02f;
		display_startup();
...
//mk
		...
		LLViewerLogin::getInstance()->setIsInSecondlife();
	...
	if (STATE_LOGIN_PROCESS_RESPONSE == LLStartUp::getStartupState())
	{
		...
		if (successful_login && gSavedSettings.getBOOL("OpenGridProtocol"))
		{
			...
			{
				gInventory.setRootFolderID(LLUserAuth::getInstance()->mResult["inventory-root"]);
			}
		...
		if (successful_login)
		{
			...
			if (gAgentID.notNull() && gAgentSessionID.notNull() &&
				gMessageSystem->mOurCircuitCode && first_sim.isOk())
								   ...
								   const std::string& gender_name)
{
	...
	{
		LLAppearanceMgr::instance().wearOutfitByName(outfit_folder_name);
	}
	LLAppearanceMgr::instance().wearOutfitByName(gestures);
	LLAppearanceMgr::instance().wearOutfitByName(COMMON_GESTURES_FOLDER);
