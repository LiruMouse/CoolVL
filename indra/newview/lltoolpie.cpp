...
//MK
		if (gRRenabled &&
			!gAgent.mRRInterface.canTouch(object, mPick.mIntersection))
		{
			return TRUE;
		}
//mk
		...
		case CLICK_ACTION_SIT:
			if (isAgentAvatarValid() && !gAgentAvatarp->mIsSitting) // agent not already sitting
			{
				handle_sit_or_stand();
		...
		case CLICK_ACTION_PAY:
				...
				(parent && parent->flagTakesMoney()))
			{
				...
				mLeftClickSelection = LLToolSelect::handleObjectSelection(mPick,
																		  FALSE,
																		  TRUE);
				if (LLSelectMgr::getInstance()->selectGetAllValid())
				{
					// call this right away, since we have all the info we need
		...
		case CLICK_ACTION_BUY:
			...
			if (LLSelectMgr::getInstance()->selectGetAllValid())
			{
				// call this right away, since we have all the info we need to
				// continue the action
				selectionPropertiesReceived();
			}
			return TRUE;
	...
	// Eat the event.
	if (!always_show && !gSavedSettings.getBOOL("LeftClickShowMenu"))
	{
	...
	if (mPick.mPickType == LLPickInfo::PICK_LAND)
	{
		...
		selection = LLViewerParcelMgr::getInstance()->selectParcelAt(mPick.mPosGlobal);
		gMenuHolder->setParcelSelection(selection);
		gPieLand->show(x, y, mPieMouseButtonDown);

		// VEFFECT: ShowPie
		LLHUDEffectSpiral* effectp;
		effectp = (LLHUDEffectSpiral*)LLHUDManager::getInstance()->createViewerEffect(LLHUDObject::LL_HUD_EFFECT_SPHERE, TRUE);
	...
	else if (object)
	{
		...
		if (object->isAvatar() ||
			(object->isAttachment() && !object->isHUDAttachment() &&
			 !object->permYouOwner()))
		{
		...
		else
		{
			std::string name;
			LLSelectNode* node;
			node = LLSelectMgr::getInstance()->getSelection()->getFirstRootNode();
			if (node)
			{
				name = node->mName;
			}
			...
			{
				gMenuHolder->childSetText("Object Mute",
										  LLTrans::getString("unmute"));
			}
			else
			{
				gMenuHolder->childSetText("Object Mute",
										  LLTrans::getString("mute"));
			}
...
{
	return !always_show && mask == MASK_NONE &&
...
U8 final_click_action(LLViewerObject* obj)
{
	...
	if (obj->getClickAction() || (parent && parent->getClickAction()))
	{
...
ECursorType cursor_from_object(LLViewerObject* object)
{
	...
	{
	case CLICK_ACTION_SIT:
		if (isAgentAvatarValid() && !gAgentAvatarp->mIsSitting) // not already sitting?
		{
...
//MK
				if (gRRenabled &&
					!gAgent.mRRInterface.canEdit(LLSelectMgr::getInstance()->getSelection()->getPrimaryObject()))
				{
					return;
				}

				if (gRRenabled &&
					!gAgent.mRRInterface.canTouchFar(selected_object,
