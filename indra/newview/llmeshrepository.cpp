...
public:
						   ...
						   const LLSD& content)
	{
		...
		dump_llsd_to_file(cc, make_dump_name("whole_model_fee_response_",
											 dump_num));
		...
		if (isGoodStatus(status) && cc["state"].asString() == "upload")
		{
			...
			if (observer)
			{
				...
				observer->onModelPhysicsFeeReceived(cc["data"],
...
#endif
		mThread->mPendingUploads--;
		dump_llsd_to_file(cc, make_dump_name("whole_model_upload_response_",
											 dump_num));
...
void LLMeshRepoThread::run()
{
	...
	while (!LLApp::isQuitting())
	{
		...
		if (!LLApp::isQuitting())
		{
			...
			// NOTE: throttling intentionally favors LOD requests over header
			// requests

			while (!mLODReqQ.empty() && count < MAX_MESH_REQUESTS_PER_SECOND &&
				   sActiveLODRequests < sMaxConcurrentRequests)
			{
				if (mMutex)
				{
					mMutex->lock();
			...
			while (!mHeaderReqQ.empty() &&
				   count < MAX_MESH_REQUESTS_PER_SECOND &&
				   sActiveHeaderRequests < sMaxConcurrentRequests)
			{
				if (mMutex)
				{
					mMutex->lock();
			...
			{	// mSkinRequests is protected by mSignal
				...
				for (std::set<LLUUID>::iterator iter = mSkinRequests.begin();
					 iter != mSkinRequests.end(); ++iter)
				{
					LLUUID mesh_id = *iter;
			...
			{	// mDecompositionRequests is protected by mSignal
				std::set<LLUUID> incomplete;
				for (std::set<LLUUID>::iterator iter = mDecompositionRequests.begin();
					 iter != mDecompositionRequests.end(); ++iter)
				{
					LLUUID mesh_id = *iter;
			...
			{	// mPhysicsShapeRequests is protected by mSignal
				std::set<LLUUID> incomplete;
				for (std::set<LLUUID>::iterator iter = mPhysicsShapeRequests.begin();
					 iter != mPhysicsShapeRequests.end(); ++iter)
				{
					LLUUID mesh_id = *iter;
	...
	if (mSignal->isLocked())
	{	// make sure to let go of the mutex associated with the given signal
	...
	if (header_size > 0)
	{
		...
		if (version <= MAX_MESH_VERSION && offset >= 0 && size > 0)
		{
			...
			if (file.getSize() >= offset + size)
			{
				...
				// make sure buffer isn't all 0's by checking the first 1KB
				// (reserved block but not written)
				bool zero = true;
			...
			if (!http_url.empty())
			{
				ret = mCurlRequest->getByteRange(constructUrl(mesh_id),
												 headers, offset, size,
												 new LLMeshSkinInfoResponder(mesh_id,
																			 offset,
																			 size));
				if (ret)
				{
...
{	// protected by mMutex
	...
	if (header_size > 0)
	{
		...
		if (version <= MAX_MESH_VERSION && offset >= 0 && size > 0)
		{
			...
			if (!http_url.empty())
			{
				ret = mCurlRequest->getByteRange(http_url, headers,
												 offset, size,
												 new LLMeshDecompositionResponder(mesh_id,
																				  offset,
																				  size));
				if (ret)
				{
	...
	if (header_size > 0)
	{
		...
		if (version <= MAX_MESH_VERSION && offset >= 0 && size > 0)
		{
			...
			if (!http_url.empty())
			{
				ret = mCurlRequest->getByteRange(http_url, headers,
												 offset, size,
												 new LLMeshPhysicsShapeResponder(mesh_id,
																				 offset,
																				 size));
				if (ret)
				{
									   ...
									   U32& count)
{
	...
	// either cache entry doesn't exist or is corrupt, request header from
	// simulator
	bool retval = true;
	std::vector<std::string> headers;
	headers.push_back("Accept: application/octet-stream");
	...
	if (!http_url.empty())
	{
		// grab first 4KB if we're going to bother with a fetch. Cache will
		// prevent future fetches if a full mesh fits within the first 4KB
		// NOTE: this will break of headers ever exceed 4KB
		retval = mCurlRequest->getByteRange(http_url, headers, 0, 4096,
											new LLMeshHeaderResponder(mesh_params));
		if (retval)
		{
			LLMeshRepository::sHTTPRequestCount++;
		}
...
{	// protected by mMutex
	...
	if (header_size > 0)
	{
		...
		if (version <= MAX_MESH_VERSION && offset >= 0 && size > 0)
		{
			...
			if (!http_url.empty())
			{
				retval = mCurlRequest->getByteRange(constructUrl(mesh_id),
													headers, offset, size,
													new LLMeshLODResponder(mesh_params,
																		   lod,
																		   offset,
																		   size));
...
{
	LLPointer<LLVolume> volume = new LLVolume(mesh_params,
		 ...
		 iter != mInstance.end(); ++iter)
	{
		...
		if (mesh_index.find(data.mBaseModel) == mesh_index.end())
		{
			...
			LLModel::Decomposition& decomp =
				data.mModel[LLModel::LOD_PHYSICS].notNull() ? data.mModel[LLModel::LOD_PHYSICS]->mPhysics
															: data.mBaseModel->mPhysics;
			decomp.mBaseHull = mHullMap[data.mBaseModel];

			LLSD mesh_header = LLModel::writeModel(ostr,  
...
void LLMeshUploadThread::doWholeModelUpload()
{
	...
	else
	{
		...
		{
			LLCurl::ResponderPtr responder = new LLWholeModelUploadResponder(this,
																			 full_model_data,
																			 mUploadObserverHandle);

			while (!mCurlRequest->post(mWholeModelUploadURL, headers, body,
									   responder, mMeshUploadTimeOut))
			{
...
void LLMeshUploadThread::requestWholeModelFee()
{
	...
	dump_llsd_to_file(model_data,make_dump_name("whole_model_fee_request_",
												dump_num));
	...
	{
		LLCurl::ResponderPtr responder = new LLWholeModelFeeResponder(this,
																	  model_data,
																	  mFeeObserverHandle);
		while (!mCurlRequest->post(mWholeModelFeeCapability, headers,
								   model_data, responder, mMeshUploadTimeOut))
		{
			// sleep for 10ms to prevent eating a whole core
			apr_sleep(10000);
		}
...
void LLMeshPhysicsShapeResponder::completedRaw(U32 status,
											   const std::string& reason,
											   const LLChannelDescriptors& channels,
											   const LLIOPipe::buffer_ptr_t& buffer)
{
...
void LLMeshHeaderResponder::completedRaw(U32 status,
										 const std::string& reason,
										 const LLChannelDescriptors& channels,
										 const LLIOPipe::buffer_ptr_t& buffer)
{
		...
		if (version <= MAX_MESH_VERSION)
		{
			...
			{	// figure out how many bytes we'll need to reserve in the file
				...
				lod_bytes = llmax(lod_bytes,
								  header[lod_name]["offset"].asInteger() + header[lod_name]["size"].asInteger());
			}
		
			// Just in case skin info or decomposition is at the end of the file
			// (which it shouldn't be)
			lod_bytes = llmax(lod_bytes,
							  header["skin"]["offset"].asInteger() + header["skin"]["size"].asInteger());
			lod_bytes = llmax(lod_bytes,
							  header["physics_convex"]["offset"].asInteger() + header["physics_convex"]["size"].asInteger());
...
S32 LLMeshRepository::loadMesh(LLVOVolume* vobj, const LLVolumeParams& mesh_params, S32 detail, S32 last_lod)
{
	...
	LL_DEBUGS("Mesh") << "Requested LOD for mesh object "
...

	{
		...
		{	// request pending for this mesh, append volume id to list
			LL_DEBUGS("Mesh") << "Adding object to pending requests for the associated mesh"
							  << LL_ENDL;
	...
	// do a quick search to see if we can't display something while we wait for
	// this mesh to load
	...
	if (volume)
	{
		...
		if (group)
		{
			// first, see if last_lod is available (don't transition down to
			// avoid funny popping a la SH-641)
			if (last_lod >= 0)
			{
...
{	// called from main thread
		...
		while (!mInventoryQ.empty())
		{
			...
			if (data.mResponse.has("new_texture_folder_id"))
			{
				...
				if (folder_id.notNull())
				{
					...
					cat = new LLViewerInventoryCategory(folder_id, parent_id,
...

	{
		...
		{
			LLNotifications::instance().add("MeshUploadError",
											mUploadErrorQ.front());
			mUploadErrorQ.pop();
		}
		...
		if (push_count > 0)
		{
			...
			for (U32 i = 0; i < 4; ++i)
			{
				for (mesh_load_map::iterator iter = mLoadingMeshes[i].begin();
					 iter != mLoadingMeshes[i].end(); ++iter)
				{
					F32 max_score = 0.f;
					for (std::set<LLUUID>::iterator obj_iter = iter->second.begin();
						...
						if (object)
						{
							...
							if (drawable)
							{
								F32 cur_score = drawable->getRadius() /
												llmax(drawable->mDistanceWRTCamera, 1.f);
			...
			for (std::vector<LLMeshRepoThread::LODRequest>::iterator iter = mPendingRequests.begin();
				 iter != mPendingRequests.end(); ++iter)
			{
...
void LLMeshRepository::notifySkinInfoReceived(LLMeshSkinInfo& info)
{
	...
	{
		for (std::set<LLUUID>::iterator obj_id = iter->second.begin();
			 obj_id != iter->second.end(); ++obj_id)
		{
...
{	// called from main thread
	...
	if (volume && obj_iter != mLoadingMeshes[detail].end())
	{
		...
		{	// update system volume
			LLVolume* sys_volume = LLPrimitive::getVolumeManager()->refVolume(mesh_params,
																			  detail);
			if (sys_volume)
			{
		...
		// available
		for (std::set<LLUUID>::iterator vobj_iter = obj_iter->second.begin();
			 vobj_iter != obj_iter->second.end(); ++vobj_iter)
		{
...
bool LLMeshRepository::hasPhysicsShape(const LLUUID& mesh_id)
{
	...
	if (mesh.has("physics_mesh") && mesh["physics_mesh"].has("size") &&
		mesh["physics_mesh"]["size"].asInteger() > 0)
	{
		return true;
	}
...
//static
F32 LLMeshRepository::getStreamingCost(LLSD& header, F32 radius, S32* bytes,
									   S32* bytes_visible, S32 lod,
									   F32 *unscaled_value)
{
	...
	// discount 128 bytes to cover the cost of LLSD tags and compression domain
	// overhead
...
void LLPhysicsDecomp::doDecomposition()
{
	...
	for (decomp_params::iterator iter = mCurRequest->mParams.begin();
		 iter != mCurRequest->mParams.end(); ++iter)
	{
		...
		{
			ret = LLConvexDecomposition::getInstance()->setParam(param->mName,
																 (F32) value.asReal());
		}
	...
	else
	{
		...
		for (S32 i = 0; i < num_hulls; ++i)
		{
			...
			LLCDHull hull;
			// if LLConvexDecomposition is a stub, num_hulls should have been
			// set to 0 above, and we should not reach this code
			LLConvexDecomposition::getInstance()->getHullFromStage(stage, i,
																   &hull);
			...
			// if LLConvexDecomposition is a stub, num_hulls should have been
			// set to 0 above, and we should not reach this code
			LLConvexDecomposition::getInstance()->getMeshFromStage(stage, i,
																   &mesh);
...
LLSD LLModelInstance::asLLSD()
{
	...
	U32 i = 0;
	for (std::map<std::string, LLImportMaterial>::iterator iter = mMaterial.begin();
		 iter != mMaterial.end(); ++iter)
	{
...
void LLMeshRepository::buildPhysicsMesh(LLModel::Decomposition& decomp)
{
	...
	for (U32 i = 0; i < decomp.mHull.size(); ++i)
	{
		...
		if (LLConvexDecomposition::getInstance() != NULL)
		{
			res = LLConvexDecomposition::getInstance()->getMeshFromHull(&hull,
																		&mesh);
		}
		if (res == LLCD_OK)
		{
