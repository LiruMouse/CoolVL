...
void LLToolSelectRect::handleRectangleSelection(S32 x, S32 y, MASK mask)
{
	...
	if (limit_select_distance)
	{
		...
		F32 new_far = relative_av_pos * camera->getAtAxis() + max_select_distance;
		F32 new_near = relative_av_pos * camera->getAtAxis() - max_select_distance;
...
//mk
	camera->setPerspective(FOR_SELECTION, center_x-width / 2,
...
//mk
				S32 result = LLViewerCamera::getInstance()->sphereInFrustum(drawable->getPositionAgent(),
																			drawable->getRadius());
				switch (result)
				{
				  case 0:
...
//mk
			if (limit_select_distance && dist_vec_squared(drawable->getWorldPosition(),
														  av_pos) > select_dist_squared)
			{
				continue;
			}

			S32 result = camera->sphereInFrustum(drawable->getPositionAgent(),
												 drawable->getRadius());
			if (result)
			{
				switch (result)
				...
				case 1:
					// check vertices
					if (camera->areVertsVisible(vobjp,
...
void LLCompass::draw()
{
	...
	F32 rotation = atan2(gAgent.getFrameAgent().getAtAxis().mV[VX],
						 gAgent.getFrameAgent().getAtAxis().mV[VY]);
...
void LLWind::renderVectors()
{
	...
	for (j = 0; j < mSize; j++)
	{
		for (i = 0; i < mSize; i++)
		{
			...
			gGL.pushMatrix();
			gGL.translatef((F32)i * region_width_meters / mSize,
						   (F32)j * region_width_meters / mSize, 0.0);
			gGL.color3f(0,1,0);
										 ...
										 LLViewerRegion* regionp)
{
	...
	z1 = regionp->getLand().resolveHeightRegion(LLVector3(clamped_x1,
														  clamped_y1,
														  0.f));
	z2 = regionp->getLand().resolveHeightRegion(LLVector3(clamped_x2,
														  clamped_y2,
														  0.f));
	...
	for (y = 0; y < STRIDE; y++)
	{
		for (x = 0; x < STRIDE; x++)
		{
			...
			if (segment_mask & SOUTH_MASK)
			{
				...
				// avoid Z fighting
				renderOneSegment(x1 + 0.1f, y1 + 0.1f, x2 + 0.1f, y2 + 0.1f,
