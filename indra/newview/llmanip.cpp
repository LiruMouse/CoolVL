								 ...
								 F32 grid_scale, S32 min_pixel_spacing)
{
	...
	F32 subdivisions = llmax(projected_translation_axis_length * grid_scale /
							 (current_range / LLViewerCamera::getInstance()->getPixelMeterRatio() *
							 min_pixel_spacing), 0.f);
	subdivisions = llclamp((F32)pow(2.f, llfloor(log(subdivisions) / log(2.f))),
						   1.f / 32.f, 32.f);
...
BOOL LLManip::handleHover(S32 x, S32 y, MASK mask)
{
	...
	if (hasMouseCapture())
	{
		...
		lldebugst(LLERR_USER_INPUT) << "hover handled by LLManip (active)"
									<< llendl;
	}
	else
	{
		lldebugst(LLERR_USER_INPUT) << "hover handled by LLManip (inactive)"
									<< llendl;
	}
										 ...
										 LLVector3 normal) const
{
	...
	{
		BOOL result = FALSE;
		F32 mouse_x = ((F32)x / gViewerWindow->getWindowWidth() - 0.5f) *
					  LLViewerCamera::getInstance()->getAspect() /
					  gAgent.mHUDCurZoom;
		F32 mouse_y = ((F32)y / gViewerWindow->getWindowHeight() - 0.5f) /
					  gAgent.mHUDCurZoom;
...
{
	if (mObjectSelection->getFirstObject() &&
		mObjectSelection->getObjectCount() == 1 &&
...
void LLManip::renderGuidelines(BOOL draw_x, BOOL draw_y, BOOL draw_z)
{
	...
	glPushMatrix();
	{
		glTranslatef(center_agent.mV[VX], center_agent.mV[VY],
					 center_agent.mV[VZ]);
...
void LLManip::renderXYZ(const LLVector3 &vec)
{
	...
	LLVector3 camera_pos = LLViewerCamera::getInstance()->getOrigin() +
...

	{
		...
		hud_render_text(utf8str_to_wstring(feedback_string), camera_pos,
					    *LLResMgr::getInstance()->getRes(LLFONT_SANSSERIF),
						LLFontGL::NORMAL, -102.f + 1.f,
						(F32)vertical_offset - 1.f, LLColor4::black, FALSE);

		feedback_string = llformat("Y: %.3f", vec.mV[VY]);
		hud_render_text(utf8str_to_wstring(feedback_string), camera_pos,
						*LLResMgr::getInstance()->getRes(LLFONT_SANSSERIF),
						LLFontGL::NORMAL, -27.f + 1.f,
						(F32)vertical_offset - 1.f, LLColor4::black, FALSE);

		feedback_string = llformat("Z: %.3f", vec.mV[VZ]);
		hud_render_text(utf8str_to_wstring(feedback_string), camera_pos,
						*LLResMgr::getInstance()->getRes(LLFONT_SANSSERIF),
						LLFontGL::NORMAL, 48.f + 1.f,
						(F32)vertical_offset - 1.f, LLColor4::black, FALSE);
		...
		hud_render_text(utf8str_to_wstring(feedback_string), camera_pos,
						*LLResMgr::getInstance()->getRes(LLFONT_SANSSERIF),
						LLFontGL::NORMAL, -102.f, (F32)vertical_offset,
						LLColor4(1.f, 0.5f, 0.5f, 1.f), FALSE);

		glColor3f(0.5f, 1.f, 0.5f);
		feedback_string = llformat("Y: %.3f", vec.mV[VY]);
		hud_render_text(utf8str_to_wstring(feedback_string), camera_pos,
						*LLResMgr::getInstance()->getRes(LLFONT_SANSSERIF),
						LLFontGL::NORMAL, -27.f, (F32)vertical_offset,
						LLColor4(0.5f, 1.f, 0.5f, 1.f), FALSE);

		glColor3f(0.5f, 0.5f, 1.f);
		feedback_string = llformat("Z: %.3f", vec.mV[VZ]);
		hud_render_text(utf8str_to_wstring(feedback_string), camera_pos,
						*LLResMgr::getInstance()->getRes(LLFONT_SANSSERIF),
						LLFontGL::NORMAL, 48.f, (F32)vertical_offset,
						LLColor4(0.5f, 0.5f, 1.f, 1.f), FALSE);
	}
}
							 ...
							 const LLColor4& color)
{
	...
	hud_render_utf8text(text, render_pos, *big_fontp, LLFontGL::NORMAL,
						-0.5f * big_fontp->getWidthF32(text), 3.f,
						shadow_color,
						mObjectSelection->getSelectType() == SELECT_TYPE_HUD);
	gViewerWindow->setupViewport();
	hud_render_utf8text(text, render_pos, *big_fontp, LLFontGL::NORMAL,
						-0.5f * big_fontp->getWidthF32(text), 3.f, color,
						mObjectSelection->getSelectType() == SELECT_TYPE_HUD);
							  ...
							  const LLColor4& color)
{
	...
	if (val_to_print < 0.f)
	{
		...
		{
			val_string = llformat("-%d%s", lltrunc(llabs(val_to_print)),
								  suffix.c_str());
		}
		else
		{
	...
	if (fractional_portion != 0)
	{
		fraction_string = llformat("%c%02d%s",
								   LLResMgr::getInstance()->getDecimalPoint(),
								   fractional_portion, suffix.c_str());

		gViewerWindow->setupViewport(1, -1);
		hud_render_utf8text(val_string, render_pos, *big_fontp,
							LLFontGL::NORMAL,
							-1.f * big_fontp->getWidthF32(val_string),
							3.f, shadow_color, hud_selection);
	...
	else
	{
		...
		hud_render_utf8text(val_string, render_pos, *big_fontp,
							LLFontGL::NORMAL,
							-0.5f * big_fontp->getWidthF32(val_string),
							3.f, shadow_color, hud_selection);
		gViewerWindow->setupViewport();
		hud_render_utf8text(val_string, render_pos, *big_fontp,
							LLFontGL::NORMAL,
