...
void LLToolCamera::pickCallback(const LLPickInfo& pick_info)
{
			...
			(LLToolMgr::getInstance()->getCurrentTool()->getName() == "Camera")) 
	{
		...
		if (!(pick_info.mKeyMask & MASK_ALT) && gAgent.cameraThirdPerson() &&
			gViewerWindow->getLeftMouseDown() && !LLPipeline::sFreezeTime &&
...
BOOL LLToolCamera::handleHover(S32 x, S32 y, MASK mask)
{
	...
	if (mOutsideSlopX || mOutsideSlopY)
	{
		if (!mValidClickPoint)
		{
			lldebugst(LLERR_USER_INPUT) << "hover handled by LLToolFocus [invalid point]"
										<< llendl;
			...
			mask == (MASK_ALT | MASK_ORBIT))
		{
			...
			lldebugst(LLERR_USER_INPUT) << "hover handled by LLToolFocus [active]"
										<< llendl;
		}
	...
	else if (gCameraBtnPan || mask == MASK_PAN || mask == (MASK_PAN | MASK_ALT))
	{
