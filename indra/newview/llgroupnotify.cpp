	...
	mInventoryOffer(NULL)
{
	...
	std::string time_buf;
	timeToFormattedString(timestamp,
	...
	{
	public:
		NoticeText(const std::string& name,
				   const LLRect& rect,
	...
	addChild(new NoticeText("group",
							LLRect(x, y, RIGHT - HPAD, y - LINE_HEIGHT),
							from.str(), LLFontGL::getFontSansSerif()));
	...
	if (mHasInventory)
	{
		addChild(new NoticeText("subjecttitle",
								...
								"Attached: ", LLFontGL::getFontSansSerif()));
	...
	btn_rect.setOriginAndSize(x, BOTTOM_PAD, btn_width, BTN_HEIGHT);

	btn = new LLButton("OK", btn_rect, LLStringUtil::null, onClickOk, this);
	addChild(btn, -1);
	setDefaultBtn(btn);

	x += btn_width + HPAD;

	btn_rect.setOriginAndSize(x, BOTTOM_PAD, wide_btn_width, BTN_HEIGHT);

	// TODO: Translate
	btn = new LLButton("Group Notices", btn_rect, LLStringUtil::null,
					   onClickGroupInfo, this);
	btn->setToolTip(std::string("View past notices or opt-out of receiving these messages here."));
	addChild(btn, -1);
	...
	if (mHasInventory)
	{
		...
		mSaveInventoryBtn = new LLButton(btn_lbl, btn_rect, LLStringUtil::null,
...
bool LLGroupNotifyBox::onNewNotification(const LLSD& notify)
{
	...
	if (notification && gNotifyBoxView)
	{
		...
		{
			llwarns << "Group notice for unkown group: "
