...
void LLDriverParam::stopAnimating(BOOL upload_bake)
{
	...
	for (entry_list_t::iterator iter = mDriven.begin(); iter != mDriven.end();
		 iter++)
	{
		LLDrivenEntry* driven = &(*iter);
									 ...
									 BOOL only_cross_params)
{
	...
	for (iter = getInfo()->mDrivenInfoList.begin();
		 iter != getInfo()->mDrivenInfoList.end(); ++iter)
	{
		...
		for (entry_list_t::iterator driven_iter = mDriven.begin();
			 driven_iter != mDriven.end() && !found; ++driven_iter)
		{
...
void LLDriverParam::updateCrossDrivenParams(LLWearableType::EType driven_type)
{
	...
	{
		LLDrivenEntry* driven = &(*iter);
		if (driven && driven->mParam && driven->mParam->getCrossWearable() &&
			driven->mParam->getWearableType() == driven_type)
		{
			needs_update = true;
		}
	}
...
void LLDriverParam::setDrivenWeight(LLDrivenEntry *driven, F32 driven_weight,
									bool upload_bake)
{
	if (isAgentAvatarValid() && mWearablep && mWearablep->isOnTop() &&
		driven->mParam->getCrossWearable())
	{
		// call setWeight through LLVOAvatarSelf so other wearables can be
		// updated with the correct values
		gAgentAvatarp->setVisualParamWeight((LLVisualParam*)driven->mParam,
