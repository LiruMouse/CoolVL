...
bool LLCanCache::operator()(LLInventoryCategory* cat, LLInventoryItem* item)
{
	...
	else if (cat)
	{
		...
		if (c->getVersion() != LLViewerInventoryCategory::VERSION_UNKNOWN)
		{
			...
			mModel->getDirectDescendentsOf(c->getUUID(), cats, items);
...
U32 LLInventoryModel::updateItem(const LLViewerInventoryItem* item)
{
	...
	else
	{
		...
		if (item->getParentUUID().isNull())
		{
			...
			{
				llwarns << "Couldn't find parent-child item tree for "
...
void LLInventoryModel::updateCategory(const LLViewerInventoryCategory* cat)
{
	...
	else
	{
		...
		new_cat = new LLViewerInventoryCategory(cat->getParentUUID());
...
void LLInventoryModel::purgeDescendentsOf(const LLUUID& id)
{
	...
	if (cat.notNull())
	{
		...
		cat_array_t categories;
		item_array_t items;
		collectDescendents(id, categories, items, INCLUDE_TRASH);
...
{
	if (mModifyMask == LLInventoryObserver::NONE &&
		mChangedItemIDs.size() == 0)
	{
		return;
	}
	notifyObservers();
...
{ 
	if (mIsNotifyObservers)
	{
		...
		llwarns << "Adding changed mask within notify observers !  Change will likely be lost."
				<< llendl;
	}
...
void  LLInventoryModel::fetchInventoryResponder::result(const LLSD& content)
{
	...
	for (S32 i = 0; i < count; ++i)
	{
		...
		LL_DEBUGS("Inventory") << "fetchInventoryResponder::result() item id: "
...
void LLInventoryModel::fetchInventoryResponder::error(U32 status,
...
void LLInventoryModel::addItem(LLViewerInventoryItem* item)
{
	if (item)
	{
		...
		// viewer prototypes prior to when  the AT_LINK enum changed from 23
		// to 24.
		...
		if (item->getIsBrokenLink())
		{
			llinfos << "Adding broken link [ name: " << item->getName()
					<< " itemID: " << item->getUUID() << " assetID: "
					<< item->getAssetUUID() << ")  parent: "
...
//	llinfos << "LLInventoryModel::empty()" << llendl;
	std::for_each(mParentChildCategoryTree.begin(),
				  mParentChildCategoryTree.end(),
				  DeletePairedPointer());
	mParentChildCategoryTree.clear();
	std::for_each(mParentChildItemTree.begin(),
...
void LLInventoryModel::accountForUpdate(const LLCategoryUpdate& update)
{
	...
	if (cat)
	{
		...
		if (!accounted)
		{
			// Error condition, this means that the category did not register
			// that it got new descendents (perhaps because it is still being
			// loaded) which means its descendent count will be wrong.
			llwarns << "No accounting for: '" << cat->getName()
					<< "' version: " << version << llendl;
		}
	}
	else
	{
...
void LLInventoryModel::incrementCategorySetVersion(const std::set<LLUUID>& categories)
{
	if (!categories.empty())
	{ 
...
LLInventoryModel::EHasChildren LLInventoryModel::categoryHasChildren(const LLUUID& cat_id) const
{
	...
	if (cat->getVersion() == LLViewerInventoryCategory::VERSION_UNKNOWN ||
		cat->getDescendentCount() == LLViewerInventoryCategory::DESCENDENT_COUNT_UNKNOWN)
	{
...
void LLInventoryModel::buildParentChildMap()
{
	...
	if (!mItemMap.empty())
	{
		...
		for (item_map_t::iterator iit = mItemMap.begin();
			 iit != mItemMap.end(); ++iit)
		{
			item = (*iit).second;
			items.put(item);
		}
	...
	if (lost)
	{
		...
		for (std::vector<LLUUID>::iterator it = lost_item_ids.begin();
	...
	{
		cat_array_t* catsp = get_ptr_in_map(mParentChildCategoryTree,
											agent_inv_root_id);
		if (catsp)
		{
			// 'My Inventory',
									...
									bool &is_cache_obsolete)
{
	...
	while (!feof(file) && fgets(buffer, MAX_STRING, file)) 
	{
		...
		if (0 == strcmp("inv_cache_version", keyword))
		{
			...
			if (succ == 1 && version == sCurrentInvCacheVersion)
			{
				// Cache is up to date
		...
		else if (strcmp("inv_category", keyword) == 0)
		{
			if (is_cache_obsolete)
				break;

			LLPointer<LLViewerInventoryCategory> inv_cat;
			inv_cat = new LLViewerInventoryCategory(LLUUID::null);
			...
			{
				llwarns << "Ignoring invalid inventory category: "
						<< inv_cat->getName() << llendl;
				// automatic when inv_cat is reassigned or destroyed
				//delete inv_cat;
			}
		}
		else if (strcmp("inv_item", keyword) == 0)
		{
			if (is_cache_obsolete)
				break;
			...
			if (inv_item->importFileLocal(file))
			{
				...
				if (inv_item->getUUID().isNull())
				{
					...
					// automatic when inv_item is reassigned or destroyed
					//delete inv_item;
				}
				else
				{
			...
			{
				llwarns << "Ignoring invalid inventory item: "
						<< inv_item->getName() << llendl;
				// automatic when inv_item is reassigned or destroyed
				//delete inv_item;
			}
...
{
	LL_DEBUGS("Inventory") << "LLInventoryModel::processUpdateCreateInventoryItem()"
						   << LL_ENDL;
...
{
	LL_DEBUGS("Inventory") << "LLInventoryModel::processRemoveInventoryItem()"
						   << LL_ENDL;
...
void LLInventoryModel::processUpdateInventoryFolder(LLMessageSystem* msg,
													void**)
{
	LL_DEBUGS("Inventory") << "LLInventoryModel::processUpdateInventoryFolder()"
						   << LL_ENDL;
	...
	for (S32 i = 0; i < count; ++i)
	{
		LLPointer<LLViewerInventoryCategory> tfolder = new LLViewerInventoryCategory(gAgentID);
		lastfolder = tfolder;
...
//MK
			...
			!gSavedSettings.getBOOL("RestrainedLoveForbidGiveToRLV"))
		{
			...
			if (folder_name.find(RR_RLV_REDIR_FOLDER_PREFIX) == 0)
			{
				...
				LLUUID rlv_shared = gAgent.mRRInterface.getRlvShare()->getUUID();
				LLUUID parent = tfolder->getParentUUID();
				if (parent != rlv_shared)
				{
					// Grrr... Lindens and their stupid "Received Items" folder
					std::string parent_name = parent.asString();
					LLViewerInventoryCategory* folderp = gInventory.getCategory(parent);
					if (folderp)
					{
						parent_name = folderp->getName();
					}
					llwarns << "Server-side failure to parent to #RLV/ for category: "
							<< folder_name << " - Current parent folder: "
							<< parent_name
							<< (parent == gInventory.getRootFolderID() ? "(inventory root)"
																	   : "(NOT inventory root)")
							<< llendl;
				}
			}
...
void LLInventoryModel::processRemoveInventoryFolder(LLMessageSystem* msg,
													void**)
{
	LL_DEBUGS("Inventory") << "LLInventoryModel::processRemoveInventoryFolder()"
						   << LL_ENDL;
...
void LLInventoryModel::processRemoveInventoryObjects(LLMessageSystem* msg,
													 void**)
{
	LL_DEBUGS("Inventory") << "LLInventoryModel::processRemoveInventoryObjects()"
						   << LL_ENDL;
													 ...
													 void**)
{
	...
	{
		llinfos << "LLInventoryModel::processSaveAssetIntoInventory item not found: "
				<< item_id << llendl;
	}
	if (gViewerWindow)
	{
				...
				if (folder_name.find(RR_RLV_REDIR_FOLDER_PREFIX) == 0)
				{
					...
					if (parent != rlv_shared)
					{
						...
						llwarns << "Server-side failure to parent to #RLV/ for category: "
								<< folder_name << " - Current parent folder: "
								<< parent_name
								<< (parent == gInventory.getRootFolderID() ? " (inventory root)" : " (NOT inventory root)")
								<< llendl;
					}
				}
			}
//mk
	...
	for (i = 0; i < count; ++i)
	{
		...
		LL_DEBUGS("Inventory") << "unpacked item '" << titem->getName()
							   << "' in " << titem->getParentUUID() << LL_ENDL;
		U32 callback_id;
	...
	if (tid.notNull() && tid == LLInventoryView::sWearNewClothingTransactionID)
	{
		...
		for (i = 0; i < count; ++i)
		{
			...
			LLAppearanceMgr::instance().wearInventoryItemOnAvatar(wearable_item, true);
		}
	}
...
{
	LL_DEBUGS("Inventory") << "LLInventoryModel::processMoveInventoryItem()"
						   << LL_ENDL;
	LLUUID agent_id;
	msg->getUUIDFast(_PREHASH_AgentData, _PREHASH_AgentID, agent_id);
	if (agent_id != gAgentID)
	...
	for (S32 i = 0; i < count; ++i)
	{
		...
		{
			llinfos << "LLInventoryModel::processMoveInventoryItem item not found: "
					<< item_id << llendl;
		}
	}
	if (anything_changed)
...
void LLInventoryFetchObserver::changed(U32 mask)
{
	...
	//llinfos << "LLInventoryFetchObserver::changed() mComplete size "
	//		<< mComplete.size() << llendl;
	//llinfos << "LLInventoryFetchObserver::changed() mIncomplete size "
	//		<< mIncomplete.size() << llendl;
}
...
void fetch_items_from_llsd(const LLSD& items_llsd)
{
	...
	for (S32 i = 0; i < body.size(); i++)
	{
		...
		if (!url.empty())
		{
			...
			LLHTTPClient::post(url, body[i],
							   new LLInventoryModel::fetchInventoryResponder(body[i]));
			break;
		}
...
{
	LL_DEBUGS("Inventory") << "LLInventoryFetchComboObserver::fetch()"
						   << LL_ENDL;
