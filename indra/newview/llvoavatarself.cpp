...
struct LocalTextureData
{
	...
	bool mIsBakedReady;
	S32 mDiscard;
	// UUID of the wearable that this texture belongs to, not of the image itself
	LLUUID mWearableID;
	...
	if (!gNoRender)
	{
			...
			else
			{
					 ...
					 iter != mAttachmentPoints.end(); )
				{
					...
					if (attachment->getGroup() == i)
					{
						...
						item->addListener(gMenuHolder->getListenerByName("Object.AttachToAvatar"),
			 ...
			 iter != mAttachmentPoints.end(); )
		{
			...
			if (attachment->getGroup() == 8)
			{
				...
				gDetachScreenPieMenu->append(item);
			}
		}
		...
		for (S32 pass = 0; pass < 2; ++pass)
		{
				 ...
				 iter != mAttachmentPoints.end(); )
			{
				...
				item = new LLMenuItemCallGL(attachment->getName(), NULL,
											&object_selected_and_point_valid,
		...
		for (S32 group = 0; group < 8; ++group)
		{
			...
			if (!gAttachBodyPartPieMenus[group] ||
				!gDetachBodyPartPieMenus[group])
			{
				continue;
			}
			...
			// gather up all attachment points assigned to this group, and
			// throw into map sorted by pie slice number
			for (attachment_map_t::iterator iter = mAttachmentPoints.begin();
				 iter != mAttachmentPoints.end(); )
			{
				attachment_map_t::iterator curiter = iter++;
				 ...
				 attach_it != attachment_pie_menu_map.end(); ++attach_it)
			{
				...
				if (attachment)
				{
					...
					item = new LLMenuItemCallGL(attachment->getName(),
												&handle_detach_from_avatar,
												object_attached, attachment);
					gDetachBodyPartPieMenus[group]->append(item);
					++cur_pie_slice;
				}
			}
		}
...
BOOL LLVOAvatarSelf::loadLayersets()
{
		 ...
		 iter != sAvatarXmlInfo->mLayerInfoList.end(); ++iter)
	{
		...
		// scan baked textures and associate the layerset with the appropriate
		// one
...
BOOL LLVOAvatarSelf::idleUpdate(LLAgent &agent, LLWorld &world, const F64 &time)
{
	...
	LLVOAvatar::idleUpdate(agent, world, time);
	{
		...
		LLAppearanceMgr::instance().checkOutfit();
	}
	return TRUE;
}
...
// virtual
BOOL LLVOAvatarSelf::setVisualParamWeight(LLVisualParam* which_param,
										  F32 weight, BOOL upload_bake)
{
	...
	param = (LLViewerVisualParam*)LLCharacter::getVisualParam(which_param->getID());
										  ...
										  BOOL upload_bake)
{
	...
	LLViewerVisualParam* param;
	param = (LLViewerVisualParam*)LLCharacter::getVisualParam(param_name);
	return setParamWeight(param, weight, upload_bake);
}
...
{
	LLViewerVisualParam* param;
	param = (LLViewerVisualParam*)LLCharacter::getVisualParam(index);
	return setParamWeight(param, weight, upload_bake);
}
...
void LLVOAvatarSelf::idleUpdateAppearanceAnimation()
{
	...
	{
		LLWearable* wearable;
		wearable = gAgentWearables.getTopWearable((LLWearableType::EType)type);
		if (wearable)
		{
...
{
	for (AnimSourceIterator motion_it = mAnimationSources.find(source_id),
...
void LLVOAvatarSelf::updateRegion(LLViewerRegion* regionp)
{
	...
	if (!regionp || regionp->getHandle() != mLastRegionHandle)
	{
		if (mLastRegionHandle != 0)
		{
			...
			// Diagnostics
			llinfos << "Region crossing took " << (F32)(delta * 1000.0f) << " ms " << llendl;
		}
		if (regionp)
		{
		 ...
		 iter != end; ++iter)
	{
		...
		else
		{
			switch (camera_mode)
			{
				...
				{
					if (LLVOAvatar::sVisibleInFirstPerson &&
...
// virtual
BOOL LLVOAvatarSelf::isWearingWearableType(LLWearableType::EType type) const
{
									 ...
									 BOOL upload_result)
{
		 ...
		 baked_iter != end; ++baked_iter)
	{
		...
		if (baked_dict)
		{
			for (LLVOAvatarDefines::wearables_vec_t::const_iterator
					type_iter = baked_dict->mWearables.begin(),
					end2 = baked_dict->mWearables.end();
				 type_iter != end2; ++type_iter)
			{
				...
				if (comp_type == type)
				{
					if (mBakedTextureDatas[index].mTexLayerSet)
					{
						...
						invalidateComposite(mBakedTextureDatas[index].mTexLayerSet,
											upload_result);
					}
					break;
				}
...
#if 0	// Attachment requests tracking not yet used in the Cool VL Viewer
//-----------------------------------------------------------------------------
...
LLViewerObject* LLVOAvatarSelf::getWornAttachment(const LLUUID& inv_item_id)
{
	...
	{
		LLViewerJointAttachment* attachment = iter->second;
		LLViewerObject* attached_object = attachment->getAttachedObject(base_inv_item_id);
 		if (attached_object)
		{
...
//MK
	if (gRRenabled)
	{
		// if this inventory item is shared and does not contain any attachment
		// info in its name, rename it for later (truncate the name first if
		// needed)
...
BOOL LLVOAvatarSelf::detachObject(LLViewerObject* viewer_object)
{
	...
	if (LLVOAvatar::detachObject(viewer_object))
	{
			 ...
			 iter != end; ++iter)
		{
			...
			// the simulator should automatically handle permissions revocation
...
LLViewerFetchedTexture* LLVOAvatarSelf::getLocalTextureGL(LLVOAvatarDefines::ETextureIndex type,
														  U32 index) const
{
...
const LLUUID& LLVOAvatarSelf::getLocalTextureID(ETextureIndex type, U32 index) const
{
	...
	const LLLocalTextureObject* local_tex_obj = getLocalTextureObject(type, index);
	if (local_tex_obj && local_tex_obj->getImage())
	{
		 ...
		 baked_iter != end; ++baked_iter)
	{
		...
		if (layerset == mBakedTextureDatas[baked_index].mTexLayerSet)
		{
				 ...
				 local_tex_iter != end2; ++local_tex_iter)
			{
				...
				for (U32 wearable_index = 0; wearable_index < wearable_count;
					 ++wearable_index)
				{
...
// virtual
BOOL LLVOAvatarSelf::isTextureVisible(LLVOAvatarDefines::ETextureIndex type,
									  U32 index) const
{
...
bool LLVOAvatarSelf::hasPendingBakedUploads() const
{
	for (U32 i = 0; i < mBakedTextureDatas.size(); ++i)
	{
		...
		if (layerset && layerset->getComposite() &&
			layerset->getComposite()->uploadPending())
		{
			return true;
		}
	}
...
// virtual
S32 LLVOAvatarSelf::getLocalDiscardLevel(ETextureIndex type,
										 U32 wearable_index) const
{
	...
	if (local_tex_obj)
	{
		if (type >= 0 && local_tex_obj->getID() != IMG_DEFAULT_AVATAR &&
			local_tex_obj->getImage() &&
	...
	// We don't care about this (no image associated with the layer) treat as
	// fully loaded.
	return 0;
}
	...
	for (S32 type = 0; type < TEX_NUM_INDICES; ++type)
	{
		...
		for (U32 num = 0; num < max_tex; ++num)
		{
			...
			if (local_tex_obj)
			{
				...
				if (image_gl)
				{
					S32 bytes = (S32)image_gl->getWidth() *
								image_gl->getHeight() *
								image_gl->getComponents();
									 ...
									 U32 index)
{
	...
	if (!local_tex_obj)
	{
		...
		{
			llerrs << "Tried to set local texture with invalid type: ("
				   << (U32) type << ", " << index << ")" << llendl;
			return;
		}
		...
		if (!local_tex_obj)
		{
			llerrs << "Unable to create LocalTextureObject for wearable type & index: ("
				   << (U32) wearable_type << ", " << index << ")" << llendl;
			return;
		}
		...
		if (layer_set)
		{
			layer_set->cloneTemplates(local_tex_obj, type,
									  gAgentWearables.getWearable(wearable_type,
																  index));
		}
	}
...
// virtual
void LLVOAvatarSelf::setBakedReady(LLVOAvatarDefines::ETextureIndex type,
		...
		if (isTextureDefined(baked_equiv, 0))
		{
			...
			else
			{
				llinfos << "LocTex " << name << ": Baked "
						<< getTEImage(baked_equiv)->getID() << llendl;
			}
		}
...
// virtual
void LLVOAvatarSelf::setImage(const U8 te, LLViewerTexture* imagep,
							  const U32 index)
{
...
BOOL LLVOAvatarSelf::getIsCloud()
{
	...
	{
		LL_DEBUGS("Avatar") << "No body parts" << LL_ENDL;
		return TRUE;
	}
...
BOOL LLVOAvatarSelf::canGrabBakedTexture(EBakedTextureIndex baked_index) const
{
	...
	for (texture_vec_t::const_iterator
			iter = baked_dict->mLocalTextures.begin(),
			end = baked_dict->mLocalTextures.end();
		 iter != end; ++iter)
	{
		...
		for (U32 wearable_index = 0; wearable_index < count; ++wearable_index)
		{
			...
			if (wearable)
			{
				...
				if (texture_id != IMG_DEFAULT_AVATAR)
				{
					...
					bool can_grab = false;
					LL_DEBUGS("Avatar") << "item count for asset " << texture_id
										<< ": " << items.count() << LL_ENDL;
					if (items.count())
					{
						...
						for (S32 i = 0; i < items.count(); ++i)
						{
							...
							if (item_permissions.allowOperationBy(PERM_MODIFY,
																  gAgent.getID(),
																  gAgent.getGroupID()) &&
...
LLLocalTextureObject* LLVOAvatarSelf::getLocalTextureObject(LLVOAvatarDefines::ETextureIndex i,
															U32 wearable_index) const
{
	LLWearableType::EType type = LLVOAvatarDictionary::getInstance()->getTEWearableType(i);
...
//-----------------------------------------------------------------------------
void LLVOAvatarSelf::setNewBakedTexture(LLVOAvatarDefines::EBakedTextureIndex i,
										const LLUUID &uuid)
{
...
void LLVOAvatarSelf::setNewBakedTexture(ETextureIndex te, const LLUUID& uuid)
{
	...
	setTEImage(te, LLViewerTextureManager::getFetchedTextureFromHost(uuid,
																	 target_host));
	updateMeshTextures();
	dirtyMesh();
	...
	const LLVOAvatarDictionary::TextureEntry* texture_dict;
	texture_dict = LLVOAvatarDictionary::getInstance()->getTexture(te);
	if (texture_dict->mIsBakedTexture)
	{
		llinfos << "New baked texture: " << texture_dict->mName << " UUID: "
				<< uuid <<llendl;
	}
	else
	{
...
void LLVOAvatarSelf::setCachedBakedTexture(ETextureIndex te, const LLUUID& uuid)
{
	...
	for (U32 i = 0; i < mBakedTextureDatas.size(); ++i)
	{
		if (mBakedTextureDatas[i].mTextureIndex == te &&
...
BOOL LLVOAvatarSelf::isUsingBakedTextures() const
{
	...
	return gFloaterCustomize ? FALSE : TRUE;
}
...
void LLVOAvatarSelf::requestLayerSetUpdate(ETextureIndex index)
{
	...
	const LLVOAvatarDictionary::TextureEntry* texture_dict;
	texture_dict = LLVOAvatarDictionary::getInstance()->getTexture(index);
	if (!texture_dict->mIsLocalTexture ||
		!texture_dict->mIsUsedByBakedTexture)
	{
		return;
	}
	const EBakedTextureIndex baked_index = texture_dict->mBakedTextureIndex;
...
void LLVOAvatarSelf::deleteScratchTextures()
{
	...
	for (LLGLuint* namep = sScratchTexNames.getFirstData(); namep;
