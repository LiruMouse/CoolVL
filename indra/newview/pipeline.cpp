		...
		if (sRenderSoundBeacons && gAudiop)
		{
			if (sRenderInvisibleSoundBeacons)
			{
				...
				for (iter = gAudiop->mAllSources.begin(); iter != gAudiop->mAllSources.end(); ++iter)
				{
					...
					{
						static LLCachedControl<S32> beacon_line_width(gSavedSettings,
																	  "DebugBeaconLineWidth");
						//pos += LLVector3(0.f, 0.f, 0.2f);
						gObjectList.addDebugBeacon(pos, "",
												   LLColor4(1.f, 1.f, 0.f, 0.5f),
												   LLColor4(1.f, 1.f, 1.f, 0.5f),
												   beacon_line_width);
					}
...
void LLPipeline::setupHWLights(LLDrawPool* pool)
{
	...
	if (mLightingDetail >= 1)
	{
			 ...
			 iter != mNearbyLights.end(); ++iter)
		{
			...
			static LLCachedControl<bool> render_spot_lights_in_non_deferred(gSavedSettings, "RenderSpotLightsInNondeferred");
			if (light->isLightSpotlight() &&		// directional (spot-)light
				(LLPipeline::sRenderDeferred || render_spot_lights_in_non_deferred)) // these are only rendered as GL spotlights if we're in deferred rendering mode *or* the setting forces them on
			{
	...
	if (isAgentAvatarValid() && gAgentAvatarp->mSpecialRenderMode == 3)
	{
...
void LLPipeline::enableLightsPreview()
{
	...
	static LLCachedControl<LLColor4> preview_ambient_color(gSavedSettings, "PreviewAmbientColor");
	LLColor4 ambient = preview_ambient_color;
	glLightModelfv(GL_LIGHT_MODEL_AMBIENT, ambient.mV);

	static LLCachedControl<LLColor4> diffuse0(gSavedSettings, "PreviewDiffuse0");
	static LLCachedControl<LLColor4> specular0(gSavedSettings, "PreviewSpecular0");
	static LLCachedControl<LLColor4> diffuse1(gSavedSettings, "PreviewDiffuse1");
	static LLCachedControl<LLColor4> specular1(gSavedSettings, "PreviewSpecular1");
	static LLCachedControl<LLColor4> diffuse2(gSavedSettings, "PreviewDiffuse2");
	static LLCachedControl<LLColor4> specular2(gSavedSettings, "PreviewSpecular2");

	static LLCachedControl<LLVector3> preview_direction0(gSavedSettings, "PreviewDirection0");
	static LLCachedControl<LLVector3> preview_direction1(gSavedSettings, "PreviewDirection1");
	static LLCachedControl<LLVector3> preview_direction2(gSavedSettings, "PreviewDirection2");
	LLVector3 dir0 = preview_direction0;
	LLVector3 dir1 = preview_direction1;
	LLVector3 dir2 = preview_direction2;
...
void LLPipeline::renderBloom(BOOL for_snapshot, F32 zoom_factor, int subfield)
{
	...
	if (LLPipeline::sRenderDeferred)
	{
		...
		if (dof_enabled)
		{
			...
			static LLCachedControl<F32> fnumber(gSavedSettings, "CameraFNumber");
		...
		S32 channel = shader->enableTexture(LLViewerShaderMgr::DEFERRED_DIFFUSE,
		...
		//channel = shader->enableTexture(LLViewerShaderMgr::DEFERRED_DEPTH,
		//								  LLTexUnit::TT_RECT_TEXTURE);
		//if (channel > -1)
		//{
	...
	else
	{
		...
		//tex unit 1
		gGL.getTexUnit(1)->setTextureColorBlend(LLTexUnit::TBO_ADD,
												LLTexUnit::TBS_TEX_COLOR,
...
void LLPipeline::bindDeferredShader(LLGLSLShader& shader,
									U32 light_index,
									LLRenderTarget* gi_source,
									LLRenderTarget* last_gi_post,
									U32 noise_map)
{
	...
	shader.bind();
	S32 channel = 0;
	channel = shader.enableTexture(LLViewerShaderMgr::DEFERRED_DIFFUSE,
								   LLTexUnit::TT_RECT_TEXTURE);
	if (channel > -1)
	{
	...
	channel = shader.enableTexture(LLViewerShaderMgr::DEFERRED_SPECULAR,
								   LLTexUnit::TT_RECT_TEXTURE);
	if (channel > -1)
	{
	...
	channel = shader.enableTexture(LLViewerShaderMgr::DEFERRED_NORMAL,
								   LLTexUnit::TT_RECT_TEXTURE);
	if (channel > -1)
	{
	...
	if (gi_source)
	{
		...
		if (channel > -1)
		{
			...
			glTexParameteri(LLTexUnit::getInternalType(mGIMap.getUsage()),
	...
	if (channel > -1)
	{
		...
		glTexParameteri(LLTexUnit::getInternalType(mDeferredDepth.getUsage()),
	...
	if (channel > -1)
	{
		gGL.getTexUnit(channel)->bindManual(LLTexUnit::TT_TEXTURE,
											mLuminanceMap.getTexture(),
											true);
	...
	channel = shader.enableTexture(LLViewerShaderMgr::DEFERRED_GI_LIGHT,
								   LLTexUnit::TT_RECT_TEXTURE);
	if (channel > -1)
	{
		gi_source->bindTexture(0, channel);
	...
	channel = shader.enableTexture(LLViewerShaderMgr::DEFERRED_EDGE,
								   LLTexUnit::TT_RECT_TEXTURE);
	if (channel > -1)
	{
	...
	channel = shader.enableTexture(LLViewerShaderMgr::DEFERRED_SUN_LIGHT,
								   LLTexUnit::TT_RECT_TEXTURE);
	if (channel > -1)
	{
	...
	channel = shader.enableTexture(LLViewerShaderMgr::DEFERRED_LOCAL_LIGHT,
								   LLTexUnit::TT_RECT_TEXTURE);
	if (channel > -1)
	{
	...
	channel = shader.enableTexture(LLViewerShaderMgr::ENVIRONMENT_MAP,
