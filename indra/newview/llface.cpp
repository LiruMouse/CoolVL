							   ...
							   bool force_rebuild)
{
	...
	if (rebuild_color)	// false if tep == NULL
	{
		...
		if (getPoolType() != LLDrawPool::POOL_ALPHA &&
			(LLPipeline::sRenderDeferred ||
			 (LLPipeline::sRenderBump && tep->getShiny())))
		{
			...
			for (S32 i = 0; i < num_vertices; ++i)
			{
				...
				*tex_coords++ = tc;
				...
				if (bump_code && mVertexBuffer->hasDataType(LLVertexBuffer::TYPE_TEXCOORD1))
				{
					...
					tc += LLVector2(bump_s_primary_light_ray.dot3(tangent).getF32(),
									bump_t_primary_light_ray.dot3(binormal).getF32());
	...
	{
		LLVector4a::memcpyNonAliased16((F32*)weights, (F32*)vf.mWeights,
