...
{
	// Reset the text accumulator. We shouldn't have strings that are
	...
	if (responseDepth == 0)
	{
		...
		LL_DEBUGS("VivoxProtocolParser") << tag << " (" << responseDepth
										 << ")"  << LL_ENDL;
	}
	else
	{
		...
		else
		{
			...
			if (!stricmp("InputXml", tag))
			{
				...
				LL_DEBUGS("VivoxProtocolParser") << "starting ignore, ignoreDepth is "
												 << ignoreDepth << LL_ENDL;
			}
...
void LLVivoxProtocolParser::EndTag(const char* tag)
{
	...
	if (!ignoringTags)
	{
		...
		{
			LLVoiceClient::getInstance()->processBuddyListEntry(uriString,
																displayNameString);
		}
		else if (!stricmp("BlockRule", tag))
		{
			LLVoiceClient::getInstance()->addBlockRule(blockMask,
													   presenceOnly);
		}
		...
		{
			LLVoiceClient::getInstance()->addAutoAcceptRule(autoAcceptMask,
															autoAddAsBuddy);
		}
...
{
	// This method is called for anything that isn't a tag, which can be text
	// you want that lies between tags, and a lot of stuff you don't want like
...
void LLVivoxProtocolParser::processResponse(std::string tag)
{
	...
	// SLIM SDK: the SDK now returns a statusCode of "200" (OK) for success.
	...
	if (isEvent)
	{
		...
		{
			LLVoiceClient::getInstance()->accountLoginStateChangeEvent(accountHandle,
																	   statusCode,
																	   statusString,
																	   state);
		}
		else if (!stricmp(eventTypeCstr, "SessionAddedEvent"))
		{
			...
			*/
			LLVoiceClient::getInstance()->sessionAddedEvent(uriString,
															alias,
															sessionHandle,
															sessionGroupHandle,
															isChannel,
		...
		else if (!stricmp(eventTypeCstr, "MediaStreamUpdatedEvent"))
		{
			...
			*/
			LLVoiceClient::getInstance()->mediaStreamUpdatedEvent(sessionHandle,
																  sessionGroupHandle,
																  statusCode,
																  statusString,
																  state,
		...
		else if (!stricmp(eventTypeCstr, "TextStreamUpdatedEvent"))
		{
			...
			*/
			LLVoiceClient::getInstance()->textStreamUpdatedEvent(sessionHandle,
																 sessionGroupHandle,
																 enabled,
																 state,
																 incoming);
		...
		else if (!stricmp(eventTypeCstr, "ParticipantAddedEvent"))
		{
			...
			*/
			LLVoiceClient::getInstance()->participantAddedEvent(sessionHandle,
																sessionGroupHandle,
																uriString,
																alias,
																nameString,
		...
		else if (!stricmp(eventTypeCstr, "ParticipantRemovedEvent"))
		{
			...
			*/
			LLVoiceClient::getInstance()->participantRemovedEvent(sessionHandle,
																  sessionGroupHandle,
																  uriString,
																  alias,
																  nameString);
		...
		else if (!stricmp(eventTypeCstr, "ParticipantUpdatedEvent"))
		{
			...
			*/
			...
			LLVoiceClient::getInstance()->participantUpdatedEvent(sessionHandle,
																  sessionGroupHandle,
																  uriString,
																  alias,
																  isModeratorMuted,
		...
		{
			LLVoiceClient::getInstance()->buddyPresenceEvent(uriString,
															 alias,
															 statusString,
															 applicationString);
		}
		...
		{
			LLVoiceClient::getInstance()->messageEvent(sessionHandle,
													   uriString,
													   alias,
													   messageHeader,
													   messageBody,
		...
		{
			LLVoiceClient::getInstance()->sessionNotificationEvent(sessionHandle,
																   uriString,
																   notificationType);
		}
		else if (!stricmp(eventTypeCstr, "SubscriptionEvent"))  
		{
			LLVoiceClient::getInstance()->subscriptionEvent(uriString,
															subscriptionHandle,
															alias,
															displayNameString,
															applicationString,
		...
		else if (!stricmp(eventTypeCstr, "SessionUpdatedEvent"))  
		{
			...
			*/
			// We don't need to process this, but we also shouldn't warn on it,
			// since that confuses people.
		}
	...
	else
	{
		...
		{
			LLVoiceClient::getInstance()->connectorCreateResponse(statusCode,
																  statusString,
																  connectorHandle,
																  versionID);
		}
		...
		{
			LLVoiceClient::getInstance()->loginResponse(statusCode,
														statusString,
														accountHandle,
														numberOfAliases);
		}
		...
		{
			LLVoiceClient::getInstance()->sessionCreateResponse(requestId,
																statusCode,
																statusString,
																sessionHandle);
		}
		...
		{
			LLVoiceClient::getInstance()->sessionGroupAddSessionResponse(requestId,
																		 statusCode,
																		 statusString,
																		 sessionHandle);
		}
		...
		{
			LLVoiceClient::getInstance()->sessionConnectResponse(requestId,
																 statusCode,
																 statusString);
		}
		else if (!stricmp(actionCstr, "Account.Logout.1"))
		{
			LLVoiceClient::getInstance()->logoutResponse(statusCode,
														 statusString);
		}
		else if (!stricmp(actionCstr, "Connector.InitiateShutdown.1"))
		{
			LLVoiceClient::getInstance()->connectorShutdownResponse(statusCode,
																	statusString);
		}
		else if (!stricmp(actionCstr, "Account.ListBlockRules.1"))
		{
			LLVoiceClient::getInstance()->accountListBlockRulesResponse(statusCode,
																		statusString);
		}
		else if (!stricmp(actionCstr, "Account.ListAutoAcceptRules.1"))
		{
			LLVoiceClient::getInstance()->accountListAutoAcceptRulesResponse(statusCode,
																			 statusString);
		}
		else if (!stricmp(actionCstr, "Session.Set3DPosition.1"))
		{
			// We don't need to process these, but they're so spammy we don't
			// want to log them.
			squelchDebugOutput = true;
		}
/*
		...
		{
			LLVoiceClient::getInstance()->channelGetListResponse(statusCode,
																 statusString);
		}
...
void LLVoiceClientCapResponder::result(const LLSD& content)
{
	...
	LL_DEBUGS("Voice") << "ParcelVoiceInfoRequest response:"
					   << ll_pretty_print_sd(content) << LL_ENDL;
...
static bool isGatewayRunning()
{
	...
	if (sGatewayPID != 0)
	{
		// A kill with signal number 0 has no effect, just does error checking.
		// It should return an error if the process no longer exists.
	...
	mLipSyncEnabled(false)
{
	...
	// Since we're now launching the gateway with fork/exec instead of
	// system(), we need to deal with zombie processes.
	// Ignoring SIGCHLD should prevent zombies from being created.
	// Alternately, we could use wait(), but I'd rather not do that.
...
{
	if (LLVoiceClient::instanceExists() && LLVoiceClient::getInstance()->mConnected)
	{
		...
		// later.
		LLVoiceClient::getInstance()->closeSocket();
	}
#if 0 // This will do unpleasant things on windows.
	else
	{
		killGateway();
	}
...
void LLVoiceClient::requestVoiceAccountProvision(S32 retries)
{
	...
	{
		std::string url;
		url = region->getCapability("ProvisionVoiceAccountRequest");
		if (!url.empty())
		{
			LLHTTPClient::post(url, LLSD(),
...
void LLVoiceClient::stateMachine()
{
	...
	{
		// Tuning mode is special -- it needs to launch SLVoice even if voice
		// is disabled.
	}
	else
	{
...
//			LL_DEBUGS("Voice") << "Region name = \"" << regionName
//							   << "\", parcel local ID = " << parcelLocalID
//							   << ", cap URI = \"" << capURI << "\"" << LL_ENDL;

			// The region name starts out empty and gets filled in later.  
			// Also, the cap gets filled in a short time after the region
			// cross, but a little too late for our purposes.
			// If either is empty, wait for the next time around.
			if (!regionName.empty())
			{
				if (!capURI.empty())
				{
					if (parcelLocalID != mCurrentParcelLocalID ||
						regionName != mCurrentRegionName)
					{
				...
				{
					LL_DEBUGS("Voice") << "region doesn't have ParcelVoiceInfoRequest capability."
									   << "This is normal for a short time after teleporting, but bad if it persists for very long."
									   << LL_ENDL;
				}
			}
		}
...
//						std::string args = " -p tcp -h -c";
						...
						// So retarded.  Windows requires that the second
						// parameter to CreateProcessA be a writable
						// (non-const) string...
						char* args2 = new char[args.size() + 1];
						strcpy(args2, args.c_str());

						if (!CreateProcessA(exe_path.c_str(), args2, NULL,
											NULL, FALSE, 0, NULL,
											exe_dir.c_str(), &sinfo, &pinfo))
						{
//							DWORD dwErr = GetLastError();
						}
						else
						{
							// Get your pid here if you want to use it later on
							//foo = pinfo.dwProcessId;
						...
						// This should be the same for mac and linux
						{
							...
							// Split the argument string into separate strings
							// for each argument
...
#endif	// LL_WINDOWS
						mDaemonHost = LLHost(gSavedSettings.getString("VoiceHost").c_str(),
											 gSavedSettings.getU32("VoicePort"));
					}
					else
					{
				...
				else
				{
					...
					// host.  This is useful for testing.
					// To do this, launch the gateway on a nearby host like this:
					//  vivox-gw.exe -p tcp -i 0.0.0.0:44125
					// and put that host's IP address here.
					mDaemonHost = LLHost(gSavedSettings.getString("VoiceHost"),
										 gSavedSettings.getU32("VoicePort"));
				}
				...
				// Dirty the states we'll need to sync with the daemon when it
				// comes up.
				mPTTDirty = true;
				mMicVolumeDirty = true;
				mSpeakerVolumeDirty = true;
		...
		case stateConnecting://MARK: stateConnecting
		...
		//MARK: stateIdle
		case stateIdle:
			// This is the idle state where we're connected to the daemon but
			// haven't set up a connector yet.
			if (mTuningMode)
			{
			...
			else if (!mAccountName.empty())
			{
				...
				if (region)
				{
					...
					{
						LL_DEBUGS("Voice") << "region doesn't have ProvisionVoiceAccountRequest capability!"
										   << LL_ENDL;
					}
				}
			}
		...
		case stateMicTuningStart:
			if (mUpdateTimer.hasExpired())
			{
				...
				{
					// These can't be changed while in tuning mode. Set them
					// before starting.
					std::ostringstream stream;
		...
		case stateConnectorStarting:	// waiting for connector handle
			// connectorCreateResponse() will transition from here to
		...
		case stateLoggedIn:				// account handle received
		{
			...
			// request the current set of block rules (we'll need them when
			...
			// Set up the friends list observer if it hasn't been set up
			// already.
...
#else
			// Not using session groups -- skip the stateCreatingSessionGroup
			// state.
		...
		case stateSessionJoined:		// session handle received
			// It appears that I need to wait for BOTH the
			// SessionGroup.AddSession response and the
			// SessionStateChangeEvent with state 4 before continuing from
			// this state. They can happen in either order, and if I don't
		...
		case stateRunning:				// steady state
			...
			else
			{
				// Figure out whether the PTT state needs to change
				{
					...
					{
						// If not configured to use PTT, it should always be
						// true (otherwise the user will be unable to speak).
						newPTT = true;
					}
				...
				// Send an update if the ptt state has changed (which shouldn't
				// be able to happen that often -- the user can only click so
	...
	if (mAudioSession)
	{
		...
		switch (getState())
		{
			case stateNoChannel:
				...
				// Skip the join failed transition state so we don't send out
				// error notifications.
				setState(stateJoinSessionFailedWaiting);
				break;
...
//		LL_DEBUGS("Voice") << "Sending listener position " << earPosition << LL_ENDL;
			...
			if (p->mVolumeDirty)
			{
				...
				if (!p->mIsSelf)
				{
					...
					if (mute)
					{
						...
						// If we want the user to be muted, set their volume to
						// 0 as well.
						// This isn't perfect, but it will at least reduce
						// their volume to a minimum.
						volume = 0;
					}
	...
	// Friends list updates can be huge, especially on the first voice login of
	// an account with lots of friends.
	// Batching them all together can choke SLVoice, so send them in separate
	// writes.
	sendFriendsListUpdates();
}
...
void LLVoiceClient::checkFriend(const LLUUID& id)
{
	...
	if (gCacheName->getFullName(id, name))
	{
		...
		// When we get here, mNeedsSend is true and mInSLFriends is false.
		...
		// In all the above cases, the buddy is in the SL friends list (which
		// is how we got here).
	...
	{
		// This name hasn't been looked up yet. Don't do anything with this
		// buddy list entry until it has.
		if (buddy)
		{
		...
		// Initiate a lookup.
		// The "lookup completed" callback will ensure that the friends list
		// is rechecked after it completes.
		lookupName(id);
	}
}
...
void LLVoiceClient::clearAllLists()
{
	...
	// This will send the necessary commands to delete ALL buddies, autoaccept
	// rules, and block rules SLVoice tells us about.
	...
	for (buddy_it = mBuddyListMap.begin(); buddy_it != mBuddyListMap.end(); )
	{
		...
		if (buddy->mInVivoxBuddies)
		{
			...
			LL_DEBUGS("Voice") << "delete " << buddy->mURI << " ("
							   << buddy->mLegacyName << ")" << LL_ENDL;
		...
		{
			// Delete the associated block list entry (so the block list
		...
		{
			// Delete the associated auto-accept list entry (so the auto-accept
			// list doesn't fill up with junk)
...
{
	if (mBuddyListMapPopulated && mBlockRulesListReceived &&
		mAutoAcceptRulesListReceived && mFriendsListDirty)
	{
		mFriendsListDirty = false;

#if 0
		// FOR TESTING ONLY -- clear all buddy list, block list, and
		// auto-accept list entries.
		clearAllLists();
		return;
#endif
			 ...
			 buddy_it != mBuddyListMap.end(); )
		{
			...
			if (buddy->mNameResolved)
			{
				...
				if (buddy->mInSLFriends &&
					(!buddy->mInVivoxBuddies || buddy->mNeedsNameUpdate))
				{
					if (mNumberOfAliases > 0)
					{
						...
						LL_DEBUGS("Voice") << "add/update " << buddy->mURI
										   << " (" << buddy->mLegacyName << ")"
										   << LL_ENDL;
				...
				{
					// This entry no longer exists in your SL friends list.
					// Remove all traces of it from the Vivox buddy list.
 					if (buddy->mInVivoxBuddies)
					{
						// delete this entry from the vivox buddy list
						buddy->mInVivoxBuddies = false;
				...
				if (buddy->mInSLFriends)
				{
					if (buddy->mCanSeeMeOnline)
					{
						...
						// If this buddy doesn't already have either a block or
						// autoaccept list entry, we'll update their status
						// when we receive a SubscriptionEvent.
						...
						if (buddy->mHasBlockListEntry)
						{
							...
							// If we just deleted a block list entry, add an
							// auto-accept entry.
							if (!buddy->mHasAutoAcceptListEntry)
							{
					...
					{
						// Delete this entry from the local buddy list. This
...
void LLVoiceClient::loginResponse(int statusCode,
								  std::string& statusString,
								  std::string& accountHandle, int numberOfAliases)
{ 
	LL_DEBUGS("Voice") << "Account.Login response (" << statusCode << "): "
...
void LLVoiceClient::sessionCreateResponse(std::string& requestId,
										  int statusCode,
										  std::string& statusString,
										  std::string& sessionHandle)
{
...
void LLVoiceClient::sessionGroupAddSessionResponse(std::string& requestId,
												   int statusCode,
												   std::string& statusString,
												   std::string& sessionHandle)
{
	...
	{
		LL_DEBUGS("Voice") << "SessionGroup.AddSession response received (success), session handle is "
...
void LLVoiceClient::sessionConnectResponse(std::string& requestId,
										   int statusCode,
										   std::string& statusString)
{
...
void LLVoiceClient::sessionAddedEvent(std::string& uriString, 
									  std::string& alias, 
									  std::string& sessionHandle, 
                                       std::string& sessionGroupHandle, 
									  bool isChannel, 
	...
	if (session)
	{
		...
		if (!session->mIsChannel)
		{
			...
			else
			{
				...
				// Can't look up the name in this case -- we have to extract it
				// from the URI.
				std::string namePortion = nameFromsipURI(session->mSIPURI);
				if (namePortion.empty())
				{
					// Didn't seem to be a SIP URI, just use the whole provided
					// name.
			...
			{
				// If we got here, we don't have a proper name. Initiate a
				// lookup.
...
void LLVoiceClient::sessionGroupAddedEvent(std::string& sessionGroupHandle)
{
	...
	{
		LL_DEBUGS("Voice") << "Already had a session group handle "
						   << mMainSessionGroupHandle << LL_ENDL;
	}
#endif
}
...
void LLVoiceClient::joinedAudioSession(sessionState* session)
{
	...
	if (getState() == stateJoiningSession)
	{
		...
		// SLIM SDK: we don't always receive a participant state change for
...
void LLVoiceClient::reapSession(sessionState* session)
{
	if (session)
	{
		...
		else
		{
			...
			// We don't have a reason to keep tracking this session, so just
			// delete it.
...
bool LLVoiceClient::sessionNeedsRelog(sessionState* session)
{
	...
	if (session != NULL)
	{
		// Only make this check for spatial channels (so it won't happen for
		// group or p2p calls)
		if (session->mIsSpatial)
		{
			...
			if (atsign != std::string::npos)
			{
				...
				{
					// The hostname in this URI is different from what we
					// expect. This probably means we need to relog.

					// We could make a ProvisionVoiceAccountRequest and compare
					// the result with the current values of
					// mVoiceSIPURIHostName and mVoiceAccountServerURI to be
					// really sure, but this is a pretty good indicator.
...
void LLVoiceClient::accountLoginStateChangeEvent(std::string& accountHandle, 
												 int statusCode, 
												 std::string& statusString, 
												 int state)
{
...
void LLVoiceClient::mediaStreamUpdatedEvent(std::string& sessionHandle, 
											std::string& sessionGroupHandle, 
											int statusCode, 
											std::string& statusString, 
											int state, 
											bool incoming)
{
	...
	LL_DEBUGS("Voice") << "session " << sessionHandle << ", status code "
					   << statusCode << ", string \"" << statusString << "\""
					   << LL_ENDL;
		...
		switch (state)
		{
			...
			case streamStateRinging:
				if (incoming)
				{
					...
					// TODO: Question: Should we correlate with the mute list
					//                 here ?
					session->mIMSessionID = LLIMMgr::computeSessionID(IM_SESSION_P2P_INVITE,
																	  session->mCallerID);
										  ...
										  int participantType)
{
	...
	if (session)
	{
		...
		if (participant)
		{
			...
			{
				// If we don't have a valid avatar UUID, we need to fill in the
				// display name to make the active speakers floater work.
											...
											std::string& nameString)
{
	...
	if (session)
	{
		...
		{
			LL_DEBUGS("Voice") << "unknown participant " << uriString
							   << LL_ENDL;
		}
	}
	else
									   ...
									   std::string& applicationString)
{
	...
	if (buddy)
	{
		LL_DEBUGS("Voice") << "Presence event for " << buddy->mLegacyName
						   << " status \"" << statusString
						   << "\", application \"" << applicationString << "\""
						   << LL_ENDL;
		LL_DEBUGS("Voice") << "before: mOnlineSL = "
						   << (buddy->mOnlineSL ? "true" : "false")
						   << ", mOnlineSLim = "
						   << (buddy->mOnlineSLim ? "true" : "false")
						   << LL_ENDL;

		if (applicationString.empty())
		{
			// This presence event is from a client that doesn't set up the
			// Application string.  Do things the old-skool way.
			// NOTE: this will be needed to support people who aren't on the
			// 3010-class SDK yet.
			...
			{
				// If the user is online through SLim, their status will be
				// "Online-slc", "Away", or something else.
				// NOTE: we should never see this unless someone is running an
				// OLD version of SLim -- the versions that should be in use
				// now all set the application string.
		...
		{
			// This presence event is from something which is NOT the SL viewer
			// (assume it's SLim).
			if (stricmp("Unknown", statusString.c_str())== 0) 
			{
		...
		LL_DEBUGS("Voice") << "after: mOnlineSL = "
						   << (buddy->mOnlineSL ? "true" : "false")
						   << ", mOnlineSLim = "
						   << (buddy->mOnlineSLim ? "true" : "false")
						   << LL_ENDL;

		// HACK -- increment the internal change serial number in the
		// LLRelationship (without changing the actual status), so the UI
		// notices the change.
		LLAvatarTracker::instance().setBuddyOnline(buddy->mUUID,
												   LLAvatarTracker::instance().isBuddyOnline(buddy->mUUID));
		notifyFriendObservers();
	}
	else
	{
		LL_DEBUGS("Voice") << "Presence for unknown buddy " << uriString
						   << LL_ENDL;
	}
}
...
void LLVoiceClient::messageEvent(std::string& sessionHandle, 
								 ...
								 std::string& applicationString)
{
	LL_DEBUGS("Voice") << "Message event, session " << sessionHandle
					   << " from " << uriString << LL_ENDL;
...
//					   << messageBody << LL_ENDL;
	...
	if (messageHeader.find("text/html") != std::string::npos)
	{
...

		{
			...
			// Default to displaying the raw string, so the message gets
			// through.
...
//						   << LL_ENDL;
		...
		if (session)
		{
			...
			if (!chat.mMuted)
			{
				...
				{
					quiet_chat = true;
					// TODO: Question: Return busy mode response here ?
					//                 Or maybe when session is started
					//                 instead ?
				}
				...
				chat.mText = std::string("IM: ") + session->mName +
							 std::string(": ") + rawMessage;
				// If the chat should come in quietly (i.e. we're in busy
				// mode), pretend it's from a local agent.
											 ...
											 std::string& notificationType)
{
	...
	if (session)
	{
		...
		if (participant)
		{
			if (!stricmp(notificationType.c_str(), "Typing"))
			{
				...
				// TODO: The proper way to add a typing notification seems to
				// be LLIMMgr::processIMTypingStart().
			...
			else if (!stricmp(notificationType.c_str(), "NotTyping"))
			{
				...
				// TODO: The proper way to remove a typing notification seems
				// to be LLIMMgr::processIMTypingStop().
				// It requires an LLIMInfo for the message, which we don't have here.
			}
			else
			{
				LL_DEBUGS("Voice") << "Unknown notification type "
								   << notificationType << "for participant "
								   << uriString << " in session "
	...
	{
		LL_DEBUGS("Voice") << "Unknown session handle " << sessionHandle
						   << LL_ENDL;
	}
}
...
LLVoiceClient::participantState* LLVoiceClient::sessionState::addParticipant(const std::string& uri)
{
	...
	if (!result)
	{
		...
		// Try to do a reverse transform on the URI to get the GUID back.
		{
			...
			{
				// Create a UUID by hashing the URI, but do NOT set
				// mAvatarIDValid.
				// This tells both code in LLVoiceClient and code in
				// llfloateractivespeakers.cpp that the ID will not be in the
				// name cache.
				setUUIDFromStringHash(result->mAvatarID, uri);
			}
		}

		mParticipantsByUUID.insert(participantUUIDMap::value_type(&(result->mAvatarID),
								   result));
...
void LLVoiceClient::parcelChanged()
{
	if (getState() >= stateNoChannel)
	{
		...
		std::string url;
		url = gAgent.getRegion()->getCapability("ParcelVoiceInfoRequest");
		if (!url.empty())
		{
			LLSD data;
								  ...
								  std::string hash)
{
	...
	switch (getState())
	{
		...
		default:
		{
			if (mSessionTerminateRequested)
			{
				...
				{
					// mNextAudioSession is null -- this probably means we're
			...
			else
			{
				...
				else
				{
					if (!uri.empty())
					{
						// mAudioSession is null -- it's not clear what case
						// would cause this.
	...
	if (needsSwitch)
	{
		...
		if (getState() <= stateNoChannel)
		{
			// We're already set up to join a channel, just needed to fill in
			// the session URI
		}
		else
		{
			// State machine will come around and rejoin if uri/handle is not
			// empty.
			sessionTerminate();
		}
	}
...
bool LLVoiceClient::sendTextMessage(const LLUUID& participant_id,
									const std::string& message)
{
	...
	if (session)
	{
		...
		// Try to send queued messages (will do nothing if the session is not
		// open yet)
		sendQueuedTextMessages(session);
...
bool LLVoiceClient::isOnlineSIP(const LLUUID& id)
{
	...
	if (!result)
	{
		// This user isn't on the buddy list or doesn't show online status
		// through the buddy list, but could be a participant in an existing
		// session if they initiated a text IM.
		sessionState* session = findSession(id);
		if (session && !session->mHandle.empty())
		{
			if (session->mTextStreamState != streamStateUnknown ||
				session->mMediaStreamState > streamStateIdle)
			{
				LL_DEBUGS("Voice") << "Open session with " << id
								   << " found, returning SIP online state"
								   << LL_ENDL;
				// we have a p2p text session open with this user, so by
				// definition they're online.
				result = true;
			}
		}
	}
...
bool LLVoiceClient::isParticipantAvatar(const LLUUID& id)
{
	...
	{
		// Didn't find a matching session -- check the current audio session
		// for a matching participant
...
std::string LLVoiceClient::nameFromID(const LLUUID& uuid)
{
	...
	// If you need to transform a GUID to this form on the Mac OS X command
	// line, this will do so:
...
void LLVoiceClient::updatePosition(void)
{
	if (LLVoiceClient::instanceExists())
	{
		...
		if (region && isAgentAvatarValid())
		{
			...
			// TODO: If camera and avatar velocity are actually used by the
			// voice system, we could compute them here...
			// They're currently always set to zero.

			// Send the current camera position to the voice code
			LLViewerCamera* camera = LLViewerCamera::getInstance();
			rot.setRows(camera->getAtAxis(), camera->getLeftAxis(),
						camera->getUpAxis());
			pos = gAgent.getRegion()->getPosGlobalFromRegion(camera->getOrigin());
...
{
	static LLCachedControl<bool> enable_voice(gSavedSettings, "EnableVoiceChat");
	static LLCachedControl<bool> disable_voice(gSavedSettings, "CmdLineDisableVoice");
...
BOOL LLVoiceClient::getVoiceEnabled(const LLUUID& id)
{
	...
	if (participant)
	{
		...
		// For now, if we have any data about the user that came through the
		// chat channel, assume they're voice-enabled.
		result = TRUE;
	}
...
BOOL LLVoiceClient::getUsingPTT(const LLUUID& id)
{
	...
	if (participant)
	{
		...
		// For now, we know there's no PTT mechanism in place, so nobody is
		// using it.
	}
...
F32 LLVoiceClient::getUserVolume(const LLUUID& id)
{
	...
	if (participant)
	{
...
#if 0
			// Enable this when debugging voice slider issues. It's way too
			// spammy even for debug-level logging.
			LL_DEBUGS("Voice") << "mapping from mUserVolume " << ires
							   << LL_ENDL;
#endif
		}
		else if (participant->mVolume != -1)
		{
			...
			// Enable this when debugging voice slider issues. It's way too
			// spammy even for debug-level logging.
			LL_DEBUGS("Voice") << "mapping from mVolume "
							   << participant->mVolume << LL_ENDL;
#endif
...
#if 0
	// Enable this when debugging voice slider issues. It's way too spammy
	// even for debug-level logging.
...
void LLVoiceClient::recordingLoopStart(int seconds,
									   int deltaFramesPerControlFrame)
{
...
void LLVoiceClient::recordingLoopSave(const std::string& filename)
{
	...
	if (mAudioSession && !mAudioSession->mGroupHandle.empty())
	{
		std::ostringstream stream;
		stream << "<Request requestId=\"" << mCommandCookie++
			   << "\" action=\"SessionGroup.ControlRecording.1\">"
													   ...
													   const std::string& handle)
{
	...
	if (handle.empty())
	{
		...
		for (sessionIterator iter = sessionsBegin(); iter != sessionsEnd();
			 iter++)
		{
			sessionState* s = *iter;
	...
	if (!result)
	{
		...
		{
			mSessionsByHandle.insert(sessionMap::value_type(&(result->mHandle),
									 result));
		}
	}
	else
	{
		...
		if (handle != result->mHandle)
		{
			if (handle.empty())
			{
				// There's at least one race condition where where addSession
...
void LLVoiceClient::deleteAllBuddies(void)
{
	...
	// Don't want to correlate with friends list when we've emptied the buddy
	// list.
	mBuddyListMapPopulated = false;

	// Don't want to correlate with friends list when we've reset the block
	// rules.
	mBlockRulesListReceived = false;
	mAutoAcceptRulesListReceived = false;
}
...
void LLVoiceClient::addBlockRule(const std::string& blockMask,
								 const std::string& presenceOnly)
{
...
void LLVoiceClient::addAutoAcceptRule(const std::string& autoAcceptMask,
									  const std::string& autoAddAsBuddy)
{
	buddyListEntry* buddy = NULL;
...
void LLVoiceClient::accountListBlockRulesResponse(int statusCode,
												  const std::string& statusString)
{
	// Block list entries were updated via addBlockRule() during parsing.
	// Just flag that we're done.
	mBlockRulesListReceived = true;
}

void LLVoiceClient::accountListAutoAcceptRulesResponse(int statusCode,
													   const std::string& statusString)
{
	// Block list entries were updated via addBlockRule() during parsing.
	// Just flag that we're done.
		...
		else if (status == LLVoiceClientStatusObserver::STATUS_LEFT_CHANNEL)
		{
			switch (mAudioSession->mErrorStatusCode)
			{
				...
				case 408:	// REQUEST_TIMEOUT
					...
					// Reset the error code to make sure it won't be reused
					// later by accident.
					mAudioSession->mErrorStatusCode = 0;
				break;
			}
...
void LLVoiceClient::notifyFriendObservers()
{
		...
		it != mFriendObservers.end(); )
	{
		...
		it++;
		// The only friend-related thing we notify on is online/offline
		// transitions.
...
{
	// If the avatar whose name just resolved is on our friends list, resync
	// the friends list.
	...
	for (sessionIterator iter = sessionsBegin(); iter != sessionsEnd(); iter++)
	{
		...
		if (session->mCallerID == id)
		{
			...
			if (session->mTextInvitePending)
			{
				...
				// We don't need to call gIMMgr->addP2PSession() here. The
				// first incoming message will create the panel.
			}
