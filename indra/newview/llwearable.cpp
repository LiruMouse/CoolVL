...
BOOL LLWearable::importFile(LLFILE* file)
{
	...
	else
	{
		...
		fields_read = fscanf(file, "%2047[^\n]", text_buffer);	/* Flawfinder: ignore */
		if (fields_read != 1 || fgetc(file) != '\n')			/* Flawfinder: ignore */
		{
	...
	{
		llwarns << "Wearable parameter mismatch. Reading in " << num_parameters
				<< " from file, but created " << mVisualParamIndexMap.size()
				<< " from avatar parameters. type: " <<  mType << llendl;
	}
	...
	{
		S32 te = 0;
		fields_read = fscanf(file, "%d %2047s\n", &te, text_buffer);	/* Flawfinder: ignore */
		if (fields_read != 2)
		{
		...
		{
			llwarns << "Bad Wearable asset: bad texture uuid: " << text_buffer
					<< llendl;
			return FALSE;
		}
...
BOOL LLWearable::isOldVersion() const
{
	...
	{
		llerrs << "Wearable asset has newer version (" << mDefinitionVersion
			   << ") than XML (" << LLWearable::sCurrentDefinitionVersion
			   << ")" << llendl;
	}
...
void LLWearable::writeToAvatar()
{
	...
	for (LLVisualParam* param = gAgentAvatarp->getFirstVisualParam(); param;
		 param = gAgentAvatarp->getNextVisualParam())
	{
		// cross-wearable parameters are not authoritative, as they are driven
		// by a different wearable. So don't copy the values to the avatar
		// object if cross wearable. Cross wearable params get their values
		// from the avatar, they shouldn't write the other way.
		if (((LLViewerVisualParam*)param)->getWearableType() == mType &&
...
void LLWearable::setVisualParamWeight(S32 param_index, F32 value, BOOL upload_bake)
{
	...
	{
		llerrs << "LLWearable::setVisualParam passed invalid parameter index: "
			   << param_index << " for wearable type: " << this->getName()
			   << llendl;
	}
}
...
F32 LLWearable::getVisualParamWeight(S32 param_index) const
{
	...
	{
		llwarns << "LLWerable::getVisualParam passed invalid parameter index: "
				<< param_index << " for wearable type: " << this->getName()
				<< llendl;
	}
	return (F32)-1.0;
...
LLColor4 LLWearable::getClothesColor(S32 te) const
{
	LLColor4 color;
	U32 param_name[3];
	if (LLVOAvatar::teToColorParams((LLVOAvatarDefines::ETextureIndex)te,
									param_name))
	{
	...
	for (S32 te = 0; te < TEX_NUM_INDICES; ++te)
	{
		if (LLVOAvatarDictionary::getTEWearableType((ETextureIndex)te) == mType)
		{
			...
			if (iter != src.end())
			{
				...
				// this to populate the values to store in the destination image
				// map.
				lto = iter->second;
				image = lto->getImage();
				image_id = lto->getID();
			...
			{
				// there's already an entry in the destination map for the
			...
			if (lto)
			{
				// If we pulled values from a Local Texture Object in the source
				// map, make sure the proper flags are set in the new (or
				// updated) entry in the destination map.
...
// static
void LLWearable::onSaveNewAssetComplete(const LLUUID& new_asset_id,
