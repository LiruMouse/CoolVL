...
void process_logout_reply(LLMessageSystem* msg, void**)
{
	...
	for (S32 i = 0; i < count; ++i)
	{
		...
		{
			llinfos << "process_logout_reply item not found: " << item_id
					<< llendl;
		}
	}
...
// 	if (status < LLTS_OK)
// 	{
// 		llwarns << "Image download failed!" << llendl;
// 	}
// 	else
// 	{
...
public:
	...
	virtual void done()
	{
		...
		LLAppViewer::instance()->addOnIdleCallback(boost::bind(&LLInventoryModel::removeItem,
												   &gInventory, mObjectID));
		gInventory.removeObserver(this);
		delete this;
	}
		...
		if (check_offer_throttle(from_name, false))
		{
			...
			case LLAssetType::AT_NOTECARD:
				open_notecard((LLViewerInventoryItem*)item,
							  "Note: " + item->getName(), LLUUID::null, true,
							  LLUUID::null, FALSE);
				break;
...
LLOfferInfo::LLOfferInfo(const LLSD& sd)
{
	...
	mLogInChat = sd.has("log_in_chat") ? sd["log_in_chat"].asBoolean() : TRUE;
...
void LLOfferInfo::extractSLURL()
{
	...
	size_t i = msg.find(LLURLDispatcher::SLURL_PREFIX);
	if (i == std::string::npos)
	{
		i = msg.find(LLURLDispatcher::SLURL_ALT_PREFIX);
	}
	if (i != std::string::npos)
	{
...
//MK
	...
	if (gRRenabled)
	{
		...
		{
			chatHistory_string = gAgent.mRRInterface.getDummyName(chatHistory_string);
		}
	}
//mk
	...
	case IOR_ACCEPT:
		...
		break;
		default:
			llwarns << "inventory_offer_callback: unknown offer type"
					<< llendl;
			break;
		}	// end switch (mIM)
//MK
...
void inventory_offer_handler(LLOfferInfo* info, BOOL from_task)
{
	...
	if (muted)
	{
		...
		{
			llinfos << "Declining inventory offer from muted object/agent: "
	...
	{
		llwarns << "LLAssetType::lookupHumanReadable() returned NULL - probably bad asset type: "
				<< info->mType << llendl;
		args["OBJECTTYPE"] = "";
...
//mk
		{
			...
			if (!gIMMgr->hasSession(session_id))
			{
				...
				pack_instant_message(gMessageSystem, gAgentID, FALSE,
			...
			llinfos << "process_improved_im: session_id(" << session_id
	...
	case IM_GROUP_NOTICE_REQUESTED:
		{
			...
			if (has_inventory && !is_muted)
			{
				...
				// For requested notices, we don't want a chat decline message
	...
	case IM_TASK_INVENTORY_OFFERED:
		// Someone has offered us some inventory.
		{
			...
			else
			{
				...
				{
					llwarns << "Malformed inventory offer from object"
							<< llendl;
					break;
				}
...
//mk
	...
	case IM_BUSY_AUTO_RESPONSE:
		if (is_muted)
		{
			LL_DEBUGS("Messaging") << "Ignoring busy response from " << from_id
								   << LL_ENDL;
			return;
		}
		else
...
//MK
					// Accept the TP if we are forced to accept TPs from this
					// avatar or from everyone, even in busy mode.
					 && !(gRRenabled &&
...
//MK
				if (gRRenabled)
				{
					if (gAgent.mRRInterface.containsWithoutException("tplure", from_id.asString()) ||
						 ...
						 isAgentAvatarValid() && gAgentAvatarp->mIsSitting))
					{
						...
						pack_instant_message(gMessageSystem, gAgentID, FALSE,
											 gAgent.getSessionID(), from_id,
											 SYSTEM_FROM, response.c_str(),
	...
	case IM_GOTO_URL:
		{
			...
			{
				llwarns << "bad binary_bucket_size: " << binary_bucket_size
...
//mk
	...
	default:
		llwarns << "Instant message calling for unknown dialog " << (S32)dialog
				<< llendl;
		break;
	}
...
//mk
	...
	LLMuteList* ml = LLMuteList::getInstance();
	BOOL is_muted = ml &&
					(ml->isMuted(from_id, from_name, LLMute::flagTextChat) ||
...
//MK
		...
		{
			if ((chatter &&
				(chatter->isAvatar () || !chatter->isAttachment () || !chatter->permYouOwner ()))  // avatar, object or attachment that doesn't belong to me
				|| !chatter) /// or this may be a HUD (visible only to the other party) or an unrezzed avatar or object
			{
				if (gAgent.mRRInterface.containsWithoutException("recvchat",
																 from_id.asString()) ||
				...
				if (from_id != gAgentID && gAgent.mRRInterface.mContainsShownames)
				{
...
#if 0			// This can't work, because tabs are expanded into 4 spaces in the
				// script editor. Plus, it would break formatted text.
				else if (gRRenabled && mesg_str.length() >= 2 && mesg_str.substr (0, 2) == "\t")
				{
					// this is a remark, only visible to non-RLV users
					return;
				}
#endif
				else
...
void process_teleport_start(LLMessageSystem *msg, void**)
{
	...
	LL_DEBUGS("Messaging") << "Got TeleportStart with TeleportFlags="
	...
	{
		// *TODO: Translate
		gViewerWindow->setProgressCancelButtonVisible(TRUE, std::string("Cancel"));
	}
...
*/
	...
	llinfos << "process_teleport_finish() Enabling " << sim_host
...
void process_agent_movement_complete(LLMessageSystem* msg, void**)
{
	...
	if ((gAgentID != agent_id) || (gAgent.getSessionID() != session_id))
	{
	...
	{
		llwarns << "agent_movement_complete() with NULL avatar." << llendl;
	}
...
//mk
				...
				gSavedSettings.getBOOL("TeleportHistoryDeparture"))
			{
				...
				gFloaterTeleportHistory->addSourceEntry(gAgent.getTeleportSourceSLURL(),
														LLViewerParcelMgr::getInstance()->getAgentParcelName());
			}
		...
		if (gFloaterTeleportHistory)
		{
			gFloaterTeleportHistory->addPendingEntry(regionp->getName(),
													 (S16)agent_pos.mV[VX],
													 (S16)agent_pos.mV[VY],
													 (S16)agent_pos.mV[VZ]);
		}
...
void process_crossed_region(LLMessageSystem* msg, void**)
{
	...
	LLUUID session_id;
	msg->getUUIDFast(_PREHASH_AgentData, _PREHASH_SessionID, session_id);
	if (agent_id != gAgentID || gAgent.getSessionID() != session_id)
	{
		llwarns << "Incorrect id in process_crossed_region()" << llendl;
		return;
	}
...
void send_agent_update(BOOL force_send, BOOL send_reliable)
{
	if (gAgent.getTeleportState() != LLAgent::TELEPORT_NONE)
	{
		// We don't care if they want to send an agent update, they're not
		// allowed to until the simulator that's the target is ready to receive
		// them (after avatar_init_complete is received)
		return;
	}
	...
	// NOTA BENE: This is (intentionally?) using the small angle sine
	//            approximation to test for rotation. Plus, there is an extra
	//            0.5 in the mix since the perpendicular between last_camera_at
	//            and getAtAxis() bisects cam_rot_change. Thus, we're actually
	//            testing against 0.2 degrees
	...
	cam_rot_chg = last_camera_at - camera->getAtAxis();
	...
	else
	{
			...
			duplicate_count < AGENT_UPDATES_PER_SECOND)
		{
			...
			// adjustments when trying to aim an attached gun, so what we do
			// here (where we would normally skip sending an update when
			// nothing has changed) is gradually reduce the threshold to allow
			// a better update to eventually get sent... should update to
	...
	if (duplicate_count < DUP_MSGS && !gDisconnected)
	{
		...
		msg->addVector3Fast(_PREHASH_CameraAtAxis, camera->getAtAxis());
		msg->addVector3Fast(_PREHASH_CameraLeftAxis, camera->getLeftAxis());
		msg->addVector3Fast(_PREHASH_CameraUpAxis, camera->getUpAxis());
		...
		last_camera_at = camera->getAtAxis();
		last_camera_left = camera->getLeftAxis();
		last_camera_up = camera->getUpAxis();
...
void process_kill_object(LLMessageSystem *mesgsys, void **user_data)
{
	...
	for (i = 0; i < num_objects; i++)
	{
		...
		if (!(id == gAgentID))
		{
			...
			{
				llwarns << "Object in UUID lookup, but not on object list in kill !"
						<< llendl;
				gObjectList.mNumUnknownKills++;
			}
		}
...
void process_time_synch(LLMessageSystem *mesgsys, void **user_data)
{
	...
	static LLCachedControl<bool> sky_override_sim_sun_position(gSavedSettings,
															   "SkyOverrideSimSunPosition");
	if (!gNoRender && !sky_override_sim_sun_position || gSky.getOverrideSun())
	{
...
void process_sound_trigger(LLMessageSystem *msg, void **)
{
	...
	// to parcel "local audio only" setting or to maturity rating
	...
	LLMuteList* ml = LLMuteList::getInstance();
	if (ml &&
		(ml->isMuted(owner_id, LLMute::flagObjectSounds) ||	// muted resident
		 ml->isMuted(object_id) ||							// muted object
		 (parent_id.notNull() && ml->isMuted(parent_id))))	// muted parent (should be unnecessary now that the mutes act on root prims)
	{
		return;
	}
...
void process_avatar_animation(LLMessageSystem *mesgsys, void **user_data)
{
	...
	if (!avatarp)
	{
		...
		llwarns << "Received animation state for unknown avatar " << uuid
				<< llendl;
		return;
	}
...
{
	static LLCachedControl<bool> camera_ignore_collisions(gSavedSettings,
														  "CameraIgnoreCollisions");
	if (camera_ignore_collisions)
	{
		return;
	}
	LLVector4 cameraCollidePlane;
	mesgsys->getVector4Fast(_PREHASH_CameraCollidePlane, _PREHASH_Plane,
							cameraCollidePlane);
...
void process_avatar_sit_response(LLMessageSystem *mesgsys, void **user_data)
{
	...
	if (isAgentAvatarValid() &&
		dist_vec_squared(camera_eye, camera_at) > 0.0001f)
	{
	...
	if (object)
	{
		...
		{
			gAgent.startAutoPilotGlobal(gAgent.getPosGlobalFromAgent(sit_spot),
										"Sit", &sitRotation, near_sit_object,
										NULL, 0.5f);
		}
	}
	else
	{
		llwarns << "Received sit approval for unknown object " << sitObjectID
				<< llendl;
	}
}
...
void process_name_value(LLMessageSystem *mesgsys, void **user_data)
{
	...
	{
		llinfos << "Can't find object " << id << " to add name value pair"
				<< llendl;
	}
}
...
void process_remove_name_value(LLMessageSystem *mesgsys, void **user_data)
{
	...
	{
		llinfos << "Can't find object " << id << " to remove name value pair"
				<< llendl;
	}
}
												 ...
												 std::string desc)
{
	...
	if (is_name_group)
	{
		gCacheName->getGroup(name_id, boost::bind(&money_balance_group_notify,
												  _1, _2, _3, args, message));
	}
	else
	{
		LLAvatarNameCache::get(name_id, boost::bind(&money_balance_avatar_notify,
													_1, _2, args, message));										   
	}
}
...
void process_money_balance_reply(LLMessageSystem* msg, void**)
{
	 	...
	 	std::find(recent.rbegin(), recent.rend(), tid) == recent.rend())
	{
		...
		if (desc.empty())
		{
			...
			desc = LLTrans::getString(balance > old_balance ? "money_balance_increased"
															: "money_balance_decreased");
			LLSD args;
		...
		{
			// Old grids will not supply the TransactionInfo block,
...
bool handle_special_notification_callback(const LLSD& notification, const LLSD& response)
{
	...
	if (0 == option)
	{
		...
		U32 preferredMaturity = (U32)notification["payload"]["_region_access"].asInteger();
...
bool handle_special_notification(std::string notificationID, LLSD& llsdBlock)
{
	...
	// We're going to throw the LLSD in there in case anyone ever wants to
	// use it
	...
	if (regionAccess == SIM_ACCESS_MATURE)
	{
		...
		{
			LLNotifications::instance().add(notificationID + "_Change",
											llsdBlock, llsdBlock,
...
bool attempt_standard_notification(LLMessageSystem* msgsystem)
{
	...
	if (msgsystem->getNumberOfBlocksFast(_PREHASH_AlertInfo) > 0)
	{
		...
		if (llsdRaw.length())
		{
			...
			if (!LLSDSerialize::deserialize(llsdBlock, llsdData,
											llsdRaw.length()))
			{
		...
		if (notificationID == "RegionEntryAccessBlocked" ||
...
void process_mean_collision_alert_message(LLMessageSystem* msgsystem, void**)
{
	...
	for (i = 0; i < num; i++)
	{
		...
		if (!b_found)
		{
			LLMeanCollisionData* mcd = new LLMeanCollisionData(gAgentID, perp,
															   time, type,
															   mag);
			gMeanCollisionList.push_front(mcd);
			gCacheName->get(perp, false,
							boost::bind(&mean_name_callback, _1, _2, _3));
		}
	}
}
...
void process_economy_data(LLMessageSystem* msg, void**)
{
	...
	LL_INFOS_ONCE("Messaging") << "EconomyData message arrived; upload cost is L$"
							   << upload_cost << LL_ENDL;
	...
	if (gMenuHolder)
	{
		gMenuHolder->childSetLabelArg("Upload Image", "[COST]",
									  llformat("%d", upload_cost));
		gMenuHolder->childSetLabelArg("Upload Sound", "[COST]",
									  llformat("%d", upload_cost));
		gMenuHolder->childSetLabelArg("Upload Animation", "[COST]",
									  llformat("%d", upload_cost));
		gMenuHolder->childSetLabelArg("Bulk Upload", "[COST]",
									  llformat("%d", upload_cost));
	}
}
									  ...
									  BOOL granted)
{
	...
	if (orig_questions)
	{
		...
		std::string owner_name = notification["payload"]["owner_name"].asString();
//MK
		if (gRRenabled && gAgent.mRRInterface.mContainsShownames)
		{
...
//mk
		...
		BOOL foundpos = FALSE;
		LLViewerObject* vobj;
		vobj = gObjectList.findObject(notification["payload"]["task_id"].asUUID());
		if (vobj)
		{
			...
			LLViewerRegion* viewregion = vobj->getRegion();
			if (viewregion)
			{
...
bool script_question_cb(const LLSD& notification, const LLSD& response)
{
	...
	{
		LLMute mute(item_id, notification["payload"]["object_name"].asString(),
					LLMute::OBJECT);
		ml->add(mute);
		LLFloaterMute::showInstance();
		LLFloaterMute::getInstance()->selectMute(mute.mID);
	...
	if (response["Details"])
	{
		...
		LLNotifications::instance().add(notification["name"],
	...
	if (gRRenabled && gAgent.mRRInterface.contains("acceptpermission"))
	{
		auto_acceptable_permission = (questions	&
									  (LSCRIPTRunTimePermissionBits[1] |	//LSCRIPTRunTimePermissions::SCRIPT_PERMISSION_TAKE_CONTROLS
					   				   ...
					   				   LSCRIPTRunTimePermissionBits[11]));	//LSCRIPTRunTimePermissions::SCRIPT_PERMISSION_TELEPORT

		// security check: if there is any other permission contained in this
		// package, we can't automatically grant anything
		if (auto_acceptable_permission)
		{
			U32 other_perms = questions &
							  ~(LSCRIPTRunTimePermissionBits[1] |
								LSCRIPTRunTimePermissionBits[3] |
								LSCRIPTRunTimePermissionBits[4] |
								LSCRIPTRunTimePermissionBits[11]);
			if (other_perms)
			{
			...
			if (isAgentAvatarValid() && !gAgentAvatarp->mIsSitting &&
				(questions & LSCRIPTRunTimePermissionBits[3]))
			{
				auto_acceptable_permission = false;
			}
		}
...
//mk
		switch (question_throttle.noteAction(throttle_name))
		{
			case LLStringThrottle::THROTTLE_NEWLY_BLOCKED:
				llinfos << "process_script_question throttled owner_name:"
						<< owner_name << llendl;
				// Fall through
								 ...
								 void* data)
{
	...
	if (inventory->size() > 2)
	{
		...
		for ( ; it != end; ++it)
		{
			if ((*it)->getType() != LLAssetType::AT_CATEGORY)
			{
				...
				new_item = new LLViewerInventoryItem(item_id,
													 cat_id,
													 item->getPermissions(),
													 item->getAssetUUID(),
													 item->getType(),
	...
	else if (inventory->size() == 2)
	{
		...
		time_t creation_date_utc = time_corrected();
		LLPointer<LLViewerInventoryItem> new_item;
		new_item = new LLViewerInventoryItem(item_id, category,
											 item->getPermissions(),
											 item->getAssetUUID(),
											 item->getType(),
											 item->getInventoryType(),
...
{
	std::string timestr;
	timeToFormattedString(the_time,
	...
	if (msg->getNumberOfBlocksFast(_PREHASH_AlertInfo) > 0)
	{
		...
		if (llsd_raw.length())
		{
			...
			else
			{
				...
				if (handle_special_notification("RegionEntryAccessBlocked",
												llsd_block))
				{
					if (gAgent.getTeleportState() != LLAgent::TELEPORT_NONE)
					{
...
//		// allow the tp offer if and only if the avatar is allowed to send tp
//		// offers to ALL the avatars in the list
//		for (LLDynamicArray<LLUUID>::iterator it = ids.begin();
//			 it != ids.end(); ++it)
//		{
...
//mk
	if (gAgent.isGodlike())
	{
		LLNotifications::instance().add("OfferTeleportFromGod", edit_args,
										payload, handle_lure_callback);
	}
	else
	{
		LLNotifications::instance().add("OfferTeleport", edit_args, payload,
...
{
	pack_instant_message(gMessageSystem, gAgentID, FALSE,
						 gAgent.getSessionID(), to_id, name, message, offline,
						 dialog, id, 0, LLUUID::null,
...
void process_script_dialog(LLMessageSystem* msg, void**)
{
	...
	if (ml && !is_ours) // Do not apply to objects we own
	{
		...
		if (muted)
		{
			...
			{
				llinfos << "Muting scripted object dialog(s) from: "
	...
	else
	{
		...
		LLNotifications::instance().add(LLNotification::Params(dialog).substitutions(args).payload(payload).form_elements(form.asLLSD()));
	}
}
	...
	for (it = gLoadUrlList.begin(); it != gLoadUrlList.end(); )
	{
		...
		if (load_url_info["owner_id"].asUUID() == id)
		{
				...
				ml->isMuted(id, full_name, 0, is_group ? LLMute::GROUP : LLMute::AGENT))
			{
				...
				if (clock() - last_warning > 5 * CLOCKS_PER_SEC)
				{
					llinfos << "Ignoring load_url from muted owner "
							<< full_name << llendl;
					last_warning = clock();
					continue;
				}
			...
			std::string dialog = id == gAgentID ? "LoadWebPageOurs" : "LoadWebPage";
...
void process_script_teleport_request(LLMessageSystem* msg, void**)
{
	...
	if (gFloaterWorldMap)
	{
		gFloaterWorldMap->trackURL(sim_name, (S32)pos.mV[VX], (S32)pos.mV[VY],
								   (S32)pos.mV[VZ]);
...
void process_covenant_reply(LLMessageSystem* msg, void**)
{
	...
	{
		gAssetStorage->getEstateAsset(gAgent.getRegionHost(), gAgentID,
									  gAgent.getSessionID(), covenant_id,
					...
					void* user_data, S32 status, LLExtStat ext_status)
{
	...
	if (0 == status)
	{
		...
		{
			LLViewerTextEditor* editor = new LLViewerTextEditor(std::string("temp"),
																LLRect(0, 0, 0, 0),
																file_length + 1);
		...
		{
			llwarns << "Problem importing estate covenant: Covenant file format error."
					<< llendl;
...
void process_feature_disabled_message(LLMessageSystem* msg, void**)
{
	...
	llwarns << "Blacklisted Feature Response:" << messageText << llendl;
}
