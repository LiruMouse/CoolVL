												...
												BOOL deactivate_similar)
{
	...
	if (!gAssetStorage)
	{
		llwarns << "LLGestureManager::activateGestureWithAsset without valid gAssetStorage"
				<< llendl;
		return;
	}
...
//	}
	...
	if (asset_id.notNull())
	{
		...
		gAssetStorage->getAssetData(asset_id, LLAssetType::AT_GESTURE,
									onLoadComplete, (void*)info, high_priority);
	}
	else
	{
		notifyObservers();
...
// found. Generates a revised string that has the found tokens replaced by their
// replacement strings and (as a minor side effect) has multiple spaces in a row
...
//mk
			if (gChatBar)
			{
				gChatBar->sendChatFromViewer(chat_text, CHAT_TYPE_NORMAL,
											 animate);
			}
			gesture->mCurrentStep++;
			break;
...
void LLGestureManager::stopGesture(LLMultiGesture* gesture)
{
	...
	for (set_it = gesture->mRequestedAnimIDs.begin();
		 set_it != gesture->mRequestedAnimIDs.end(); ++set_it)
	{
		const LLUUID& anim_id = *set_it;
		gAgent.sendAnimationRequest(anim_id, ANIM_REQUEST_STOP);
	}
	for (set_it = gesture->mPlayingAnimIDs.begin();
		 set_it != gesture->mPlayingAnimIDs.end(); ++set_it)
	{
		const LLUUID& anim_id = *set_it;
		gAgent.sendAnimationRequest(anim_id, ANIM_REQUEST_STOP);
	}
