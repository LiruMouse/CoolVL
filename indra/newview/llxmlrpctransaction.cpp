...
private:
	...
	static size_t curlDownloadCallback(char* data, size_t size,
									   size_t nmemb,
									   void* user_data);
};
...
bool LLXMLRPCTransaction::Impl::process()
{
	...
	while (true)
	{
		...
		if (newmsg)
		{
			...
			mResponse = XMLRPC_REQUEST_FromXML(mResponseText.data(),
											   mResponseText.size(), NULL);
...
LLXMLRPCTransaction::Status LLXMLRPCTransaction::status(int* curlCode)
{
	if (curlCode)
	{
		*curlCode = (impl.mStatus == StatusCURLError) ? impl.mCurlCode
													  : CURLE_OK;
	}
