...
class LLPlaceAvatarTeleportResponder : public LLHTTPClient::Responder
{
public:
	...
	{
		llinfos << "LLPlaceAvatarTeleportResponder error in TP " << statusNum
				<< " " << reason << llendl;
	...
	void result(const LLSD& content)
	{
		...
		// Even though we have the pretty print of the complete content
		// returned, we still find it handy when scanning SecondLife.log to
		// have these laid out in this way. So they are still here.
		...
		llinfos << "All the LLSD PlaceAvatarTeleportResponder content: \n "
				<< ll_pretty_print_sd(content) << llendl; // OGPX

		// check "connect" to make sure place_avatar fully successful
		if (!content["connect"].asBoolean())
		{
			// place_avatar failed somewhere
			llinfos << "TP failed, connect false in TP PlaceAvatarResponder"
					<< llendl;
		...
		{
		    llinfos << "TP failed, could not resolve hostname for UDP messages."
					<< llendl;
			LLSD args;
...
:   LLFloater("floater_teleport")
{
	if (!sInstance)
	{
		...
		regioncombo->setSelectedByValue(LLSD(gSavedSettings.getString("CmdLineRegionURI")),
										TRUE);

		// TODO : decide if 'enter' when selecting something from the combox
		// box should *not* be sent to the floater (p.s. and figure out how to
		// change it)
...
void LLFloaterTeleport::onClickTeleport(void* userdata)
{
    ...
    LL_DEBUGS("OGPX") << "onClickTeleport! from using place_avatar cap "
					  << placeAvatarCap << " contains "<< text << LL_ENDL;
	...
	llinfos << "Args to placeavatar cap " << placeAvatarCap
			<< " on teleport: " << LLSDOStreamer<LLSDXMLFormatter>(args) << llendl;
...
{
	LLFloaterTeleport* self = (LLFloaterTeleport*)userdata;
	llinfos << "Teleport Cancel " << self->getName() << llendl;
	self->setVisible(FALSE);
}
