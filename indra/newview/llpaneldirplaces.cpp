...
{
#if LL_DARWIN			// Is this still needed at all ?...
	// MBW -- HACK!!!
	// This looks like some sort of compiler bug. We have a mysterious crash
	// during initialization on powerpc boxes. The crash seems unrelated to
	// this code, but the commit that introduced it was narrowed down to this
	// file. Adding llinfos calls to both the constructor and destructor here
	// makes the crash go away, even though they don't get called before the
	// point of the crash. This is wrong on many levels and scares the hell
	// out of me.
