...
:	mHideSelectedObjects(LLCachedControl<bool>(gSavedSettings, "HideSelectedObjects", FALSE)),
	...
	mEditLinkedParts(LLCachedControl<bool>(gSavedSettings, "EditLinkedParts", FALSE)),
	mSelectOwnedOnly(LLCachedControl<bool>(gSavedSettings, "SelectOwnedOnly", FALSE)),
	mSelectMovableOnly(LLCachedControl<bool>(gSavedSettings, "SelectMovableOnly", FALSE))
{
	mTEMode = FALSE;
...
BOOL LLSelectMgr::removeObjectFromSelections(const LLUUID &id)
{
	...
	if (!object_found)
	{
			 ...
			 iter != getSelection()->end(); )
		{
			...
			{
				// It's possible the item being removed has an avatar sitting
...
{
	if (!objectp ||
		(objectp->mID == gAgentID && !mAllowSelectAvatar) ||			// Can't select yourself
		(objectp->getPCode() == LLViewerObject::LL_VO_SURFACE_PATCH))	// Can't select land
	{
		mHoverObjects->deleteAllNodes();
		return NULL;
	}
...
void LLSelectMgr::highlightObjectOnly(LLViewerObject* objectp)
{
	...
	if ((mSelectOwnedOnly && !objectp->permYouOwner()) ||
		(mSelectMovableOnly && !objectp->permMove()))
	{
						  ...
						  LLVector3& scale)
{
	...
	else // GRID_MODE_WORLD or just plain default
	{
		...
		case SELECT_TYPE_ATTACHMENT:
			if (first_object && first_object->getRootEdit() &&
				first_object->getRootEdit()->mDrawable.notNull())
			{
				...
				mGridScale = LLVector3(1.f, 1.f, 1.f) * (F32)grid_resolution;
			}
			break;
...
void LLSelectMgr::remove(LLViewerObject *objectp, S32 te, BOOL undoable)
{
	...
	else if (0 <= te && te < SELECT_MAX_TES)
	{
		...
		{
			llerrs << "LLSelectMgr::remove - tried to remove TE " << te
				   << " that wasn't selected" << llendl;
			return;
		}
	...
	{
		// ...out of range face
		llerrs << "LLSelectMgr::remove - TE " << te << " out of range"
			   << llendl;
	}
...
void LLSelectMgr::promoteSelectionToRoot()
{
		 ...
		 iter != getSelection()->end(); )
	{
		...
		while (parentp->getParent() &&
			   !(parentp->isRootEdit() || parentp->isJointChild()))
		{
	...
	if (selection_changed)
	{
		...
		for (std::set<LLViewerObject*>::iterator
				set_iter = selection_set.begin(), end = selection_set.end();
			 set_iter != end; ++set_iter)
		{
...
void LLSelectMgr::dump()
{
		 ...
		 iter != getSelection()->end(); iter++)
	{
		...
		llinfos << "  "
				<< (drawablep&& drawablep->isVisible() ? "visible" : "invisible")
				<< llendl;
		llinfos << "  "
				<< (drawablep && drawablep->isState(LLDrawable::FORCE_INVISIBLE) ? "force_invisible" : "")
				<< llendl;
		count++;
	}
	...
	llinfos << mHighlightedObjects->getNumNodes()
			<< " objects currently highlighted." << llendl;
	...
	struct f : public LLSelectedTEFunctor
	{
		...
		bool apply(LLViewerObject* objectp, S32 te)
		{
			...
			else // not an inventory item
			{
				...
				objectp->setTEImage(te, LLViewerTextureManager::getFetchedTexture(mImageID,
																				  TRUE,
		 ...
		 iter != end; ++iter)
	{
			 ...
			 te_num++)
		{
			...
			if (planar == stretch)
			{
				...
				if (planar)
				{
					object->setTEScale(te_num,
									   scale_ratio.mV[s_axis] / object_scale.mV[s_axis],
								...
								LLObjectSelectionHandle handle)
{
	...
	if (option == 0)
	{
		...
		LLViewerStats::getInstance()->setStat(LLViewerStats::ST_OBJECT_DELETE_COUNT,
											  obj_delete_count);
	}
...
BOOL LLSelectMgr::selectGetAggregateTexturePermissions(LLAggregatePermissions& result_perm)
{
		 ...
		 iter != end; ++iter)
	{
		...
		t_perm = node->getObject()->permYouOwner() ? node->mAggregateTexturePermOwner
...
void LLSelectMgr::sendMultipleUpdate(U32 type)
{
	...
	sendListToRegions("MultipleObjectUpdate",
					  packAgentAndSessionID,
					  packMultipleUpdate,
					  &type,
					  send_type);
...
void LLSelectMgr::packBuyObjectIDs(LLSelectNode* node, void* data)
{
				  ...
				  object) == buy->mObjectsSent.end())
	{
		...
		gMessageSystem->addU32Fast(_PREHASH_ObjectLocalID,
								   object->getLocalID());
		gMessageSystem->addU8Fast(_PREHASH_SaleType,
								  buy->mSaleInfo.getSaleType());
		gMessageSystem->addS32Fast(_PREHASH_SalePrice,
								   buy->mSaleInfo.getSalePrice());
	}
}
...
void LLSelectMgr::deselectAllIfTooFar()
{
	...
	// Don't deselect when we're navigating an object's pie menu or when we are
	// quitting the application (with gPieObject deleted)
...
void LLSelectMgr::sendAttach(U8 attachment_point)
{
	...
	if (attachment_point == 0 ||
		get_if_there(gAgentAvatarp->mAttachmentPoints,
					 (S32)attachment_point,
					 (LLViewerJointAttachment*)NULL))
	{
		if (attachment_point != 0 && gAgentAvatarp->allowMultipleAttachments())
		{
									...
									ESendType send_type)
{
	...
	switch (send_type)
	{
		...
		case SEND_CHILDREN_FIRST:
			...
			break;
		default:
			llerrs << "Bad send type " << send_type
				   << " passed to SendListToRegions()" << llendl;
	}
	...
	while (node != NULL)
	{
		...
		if (current_region == last_region && !gMessageSystem->isSendFull(NULL)
	...
	//llinfos << "sendListToRegions " << message_name << " obj "
	//		<< objects_sent << " pkt " << packets_sent << llendl;
}
...
void LLSelectMgr::processObjectProperties(LLMessageSystem* msg, void** user_data)
{
	...
	for (i = 0; i < count; i++)
	{
		...
		if (size > 0)
		{
			...
			msg->getBinaryDataFast(_PREHASH_ObjectData, _PREHASH_TextureID,
								   packed_buffer, 0, i, SELECT_MAX_TES * UUID_BYTES);
	...
	if (gPopupMenuView && gPopupMenuView->getVisible())
	{
...
void LLSelectMgr::processObjectPropertiesFamily(LLMessageSystem* msg, void**)
{
	...
	{
		EReportType report_type = (COMPLAINT_REPORT_REQUEST & request_flags) ? COMPLAINT_REPORT
																			 : BUG_REPORT;
...
void LLSelectMgr::updateSilhouettes()
{
	...
	{
		mSilhouetteImagep = LLViewerTextureManager::getFetchedTextureFromFile("silhouette.j2c",
																			  TRUE,
			 ...
			 iter != end; ++iter)
		{
			...
			else
			{
					 ...
					 iter2 != end2; ++iter2)
				{
					...
					LLSelectNode* rect_select_node = new LLSelectNode(child_objectp,
																	  TRUE);
					 ...
					 objectp->getParent()->isChanged(LLXform::SILHOUETTE)))
				{
					if (num_sils_genned++ < MAX_SILS_PER_FRAME)
					{
						...
						changed_objects.push_back(objectp);
							 ...
							 objectp->getRootEdit()->mDrawable.notNull())
					{
						// RN: hack for orthogonal projection of HUD attachments
						LLViewerJointAttachment* attachment_pt;
						attachment_pt = (LLViewerJointAttachment*)objectp->getRootEdit()->mDrawable->getParent();
	...
	if (mHighlightedObjects->getNumNodes())
	{
		...
		for (S32 pass = 0; pass < 2; ++pass)
		{
			for (LLObjectSelection::iterator
					iter = mHighlightedObjects->begin(),
					end = mHighlightedObjects->end();
				 iter != end; ++iter)
			{
				LLSelectNode* node = *iter;
				...
				{
					LLColor4 highlight_color = objectp->isRoot() ? sHighlightParentColor
...
LLSelectNode::LLSelectNode(const LLSelectNode& nodep)
{
	...
	mSavedColors.clear();
	for (color_iter = nodep.mSavedColors.begin();
		 color_iter != nodep.mSavedColors.end(); ++color_iter)
	{
...
void LLSelectNode::renderOneWireframe(const LLColor4& color)
{
	...
	glColor4f(color.mV[VRED] * 2, color.mV[VGREEN] * 2, color.mV[VBLUE] * 2,
			  LLSelectMgr::sHighlightAlpha * 2);
...
void LLSelectNode::renderOneSilhouette(const LLColor4 &color)
{
...
#endif
	{
		...
		{
			for (S32 i = 0, count = mSilhouetteVertices.size(); i < count;
				 i += 2)
			{
				...
				gGL.color4f(color.mV[VRED], color.mV[VGREEN],
							color.mV[VBLUE], 0.0f); //LLSelectMgr::sHighlightAlpha);
...
S32 get_family_count(LLViewerObject *parent)
{
		 ...
		 iter != end; ++iter)
	{
		...
		else if (LLSelectMgr::getInstance()->canSelectObject(child))
		{
...
BOOL LLSelectMgr::canSelectObject(LLViewerObject* object)
{
	...
	// Can't select orphans, avatars or land
	...
	if ((mSelectOwnedOnly && !object->permYouOwner()) ||
		(mSelectMovableOnly && !object->permMove()))
	{
		// only select my own objects
		return FALSE;
	}
...
{
	//RN: don't currently support deletion of child objects, as that requires
...
void LLSelectMgr::sendSelectionMove()
{
		 ...
		 it != getSelection()->root_end(); ++it)
	{
		...
		if (curr_region != last_region || gMessageSystem->isSendFull(NULL) ||
			objects_in_this_packet >= MAX_OBJECTS_PER_PACKET)
		{
