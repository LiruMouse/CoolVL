...
{
public:
	LLGroupDropTarget(const std::string& name,
					  const LLRect& rect,
...
{
	return mAllowEdit && agentp->hasPowerInGroup(mGroupID, GP_NOTICES_SEND |
														   GP_NOTICES_RECEIVE);
}

BOOL LLPanelGroupNotices::postBuild()
{
	mNoticesList = getChild<LLScrollListCtrl>("notice_list", TRUE, FALSE);
	if (!mNoticesList) return FALSE;
	...
	// Create
	mCreateSubject = getChild<LLLineEditor>("create_subject", TRUE, FALSE);
	mCreateMessage = getChild<LLTextEditor>("create_message", TRUE, FALSE);
	if (!mCreateSubject || !mCreateMessage) return FALSE;

	mCreateInventoryName =  getChild<LLLineEditor>("create_inventory_name", TRUE, FALSE);
	if (!mCreateInventoryName) return FALSE;
	mCreateInventoryName->setTabStop(FALSE);
	mCreateInventoryName->setEnabled(FALSE);

	mCreateInventoryIcon = getChild<LLIconCtrl>("create_inv_icon", TRUE, FALSE);
	if (mCreateInventoryIcon) mCreateInventoryIcon->setVisible(FALSE);
	...
	mBtnRemoveAttachment = getChild<LLButton>("remove_attachment", TRUE, FALSE);
	...
	if (mViewMessage) mViewMessage->setParseHTML(TRUE);

	mViewInventoryName =  getChild<LLLineEditor>("view_inventory_name", TRUE, FALSE);
	...
	if (mViewInventoryIcon) mViewInventoryIcon->setVisible(FALSE);
	...
	mPanelCreateNotice = getChild<LLPanel>("panel_create_new_notice", TRUE, FALSE);
	mPanelViewNotice = getChild<LLPanel>("panel_view_past_notice", TRUE, FALSE);
	if (!mPanelCreateNotice || !mPanelViewNotice) return FALSE;

	// Must be in front of all other UI elements.
	LLPanel* dtv = getChild<LLPanel>("drop_target", TRUE, FALSE);
...
void LLPanelGroupNotices::onClickSendMessage(void* data)
{
	...
	send_group_notice(self->mGroupID,
...
{
	LL_DEBUGS("GroupPanel") << "LLPanelGroupNotices::onClickGetPastNotices" << LL_ENDL;
	LLPanelGroupNotices* self = (LLPanelGroupNotices*)data;
	if (!self || !self->mInitOK) return;
...
void LLPanelGroupNotices::onSelectNotice(LLUICtrl* ctrl, void* data)
{
	...
	LL_DEBUGS("GroupPanel") << "Item " << item->getUUID() << " selected." << LL_ENDL;
}
									 ...
									 LLOfferInfo* inventory_offer)
{
	...
	if (inventory_offer)
	{
		...
		if (mBtnOpenAttachment) mBtnOpenAttachment->setEnabled(TRUE);
	}
	else
	{
		if (mViewInventoryName) mViewInventoryName->clear();
		if (mViewInventoryIcon) mViewInventoryIcon->setVisible(FALSE);
		if (mBtnOpenAttachment) mBtnOpenAttachment->setEnabled(FALSE);
	}
}
