							 ...
							 const std::string& label)
{
	...
	F32 color_frac = 1.f;
	if (dist > 0.99f * camera->getFar())
	{
		color_frac = 0.4f;
		//pos_global = gAgent.getCameraPositionGlobal() + 0.99f * (camera->getFar() / dist) * to_vec;
	}
	else
	{
		color_frac = 1.f - 0.6f * (dist / camera->getFar());
	}
	...
	glPushMatrix();
		...
		LLVector3 x_axis = camera->getLeftAxis();
		F32 t = gRenderStartTime.getElapsedTimeF32();
		F32 dr = dist / camera->getFar();
...
void LLTracker::drawMarker(const LLVector3d& pos_global, const LLColor4& color)
{
		...
		|| LLViewerCamera::getInstance()->projectPosAgentToScreenEdge(pos_local, screen))
	{
		...
		if (dist > 0.f)
		{
			...
			F32 margin = 0.0;
			if (gToolBar && gToolBar->getVisible() && gChatBar &&
				gChatBar->getVisible())
			{
