													...
													const LLVolumeFace& vol_face, LLVOVolume* vobj)
{
		...
		(drawable && drawable->isState(LLDrawable::REBUILD_ALL)))
	{
		...
		static LLCachedControl<bool> mesh_force_deformer(gSavedSettings, "MeshForceDeformer");
		if (mesh_enable_deformer && (mesh_force_deformer || skin->mDeform))
		{
			LLDeformedVolume* deformed_volume = vobj->getDeformedVolume();
			bool success = deformed_volume->deform(volume, avatar, skin,
												   face->getTEOffset(), drawable);
			if (success)
			{
				which_volume = deformed_volume;
				avatar->setUseDeformer(true);
			}
		}
		face->getGeometryVolume(*which_volume, face->getTEOffset(), mat_vert,
