...
void LLInventoryViewFinder::draw()
{
	...
	mInventoryView->mActivePanel->setShowFolderState(getCheckShowEmpty() ? LLInventoryFilter::SHOW_ALL_FOLDERS
...
void LLInventoryView::toggleFindOptions()
{
	...
	if (!floater)
	{
		LLInventoryViewFinder* finder;
		finder = new LLInventoryViewFinder(std::string("Inventory Finder"),
										   LLRect(getRect().mLeft - INV_FINDER_WIDTH,
												  getRect().mTop,
												  getRect().mLeft,
...
void LLInventoryView::onFilterSelected(void* userdata, bool from_click)
{
	...
	if (filter->isActive())
	{
		// If our filter is active we may be the first thing requiring a fetch
		// so we better start it here.
		LLInventoryModelBackgroundFetch::instance().start();
	}
