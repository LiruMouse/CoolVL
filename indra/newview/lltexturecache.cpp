...
public:
	LLTextureCacheLocalFileWorker(LLTextureCache* cache, U32 priority,
								  const std::string& filename, const LLUUID& id,
								  U8* data, S32 datasize, S32 offset,
								  S32 imagesize, // for writes
								  LLTextureCache::Responder* responder)
...
bool LLTextureCacheLocalFileWorker::doRead()
{
	...
	if (local_size > 0 && mFileName.size() > 4)
	{
		...
		{
// 			llwarns << "Unrecognized file extension " << extension
//					<< " for local texture " << mFileName << llendl;
			mDataSize = 0; // no data
			return true;
		}
	}
...
// 						<< " / " << mDataSize << llendl;
	...
	S32 bytes_read = LLAPRFile::readEx(mFileName, mReadData, mOffset,
...
bool LLTextureCacheRemoteWorker::doWrite()
{
	...
	if (!done && (mState == HEADER))
	{
		...
		{
			// We need to write a full record in the header cache so, if the
			// amount of data is smaller than a record, we need to transfer the
			// data to a buffer padded with 0 and write that
		...
		{
			// Write the header record (== first TEXTURE_CACHE_ENTRY_SIZE bytes
			// of the raw file) in the header file
			bytes_written = LLAPRFile::writeEx(mCache->mHeaderDataFileName,
...
// 			llinfos << "Writing Body: " << filename << " Bytes: " << file_offset+file_size << llendl;
			S32 bytes_written = LLAPRFile::writeEx(filename,
												   ...
												   0,
												   file_size, mCache->getLocalAPRFilePool());
			if (bytes_written <= 0)
			{
				llwarns << "LLTextureCacheWorker: "  << mID
...
:	LLWorkerThread("TextureCache", threaded),
	...
	mDoPurge(FALSE)
{
	LL_DEBUGS("TextureCache") << "Texture cache local APR pool: "
							  << getLocalAPRFilePool() << LL_ENDL;
}
...
std::string LLTextureCache::getTextureFileName(const LLUUID& id)
{
	...
	std::string filename = mTexturesDirName + delem + idstr[0] + delem +
						   idstr + ".texture";
	return filename;
}
...
void LLTextureCache::purgeCache(ELLPath location)
{
	...
	if (!mReadOnly)
	{
		...
		mTexturesDirName = gDirUtilp->getExpandedFilename(location,
														  old_textures_dirname);
		if (LLFile::isdir(mTexturesDirName))
		{
			std::string file_name = gDirUtilp->getExpandedFilename(location,
																   entries_filename);
...
S64 LLTextureCache::initCache(ELLPath location, S64 max_size,
							  BOOL texture_cache_mismatch)
{
	// should not start accessing the texture cache before initialized.
...
S32 LLTextureCache::openAndReadEntry(const LLUUID& id, Entry& entry, bool create)
{
	...
	if (idx < 0)
	{
		if (create && !mReadOnly)
		{
			...
			else
			{
				...
				for (std::set<LLUUID>::iterator iter2 = mLRU.begin();
					 iter2 != mLRU.end(); )
				{
					...
					{
						idx = iter3->second;
						 // remove the existing cached texture to release the entry index.
	...
	else
	{
		...
		// it happens on 64-bit systems, do not know why
...
// 		llinfos << "ENTRY: " << entry.mTime << " TEX: " << entry.mID
//				<< " IDX: " << idx << " Size: " << entry.mImageSize
//				<< llendl;
		if (entry.mImageSize > entry.mBodySize)
		{
			mHeaderIDMap[entry.mID] = idx;
...
void LLTextureCache::updatedHeaderEntriesFile()
{
	if (!mReadOnly && !mUpdatedEntryMap.empty() && mHeaderAPRFile)
	{
			 ...
			 iter != end; ++iter)
		{
			...
			bytes_written = mHeaderAPRFile->write((void*)(&iter->second),
												  entry_size);
...
//			else // entries are not changed, nothing here.
		}
	}
	mHeaderMutex.unlock();
}
...
{
	llwarns << "the texture cache is corrupted, need to be cleared."
			<< llendl;
...
// 				llinfos << "TIME: " << entries[idx].mTime
//						<< " TEX: " << entries[idx].mID
//						<< " IDX: " << idx << " Size: "
//						<< entries[idx].mImageSize << llendl;
			}
			else
			{
	...
	const U32 FRACTION = 8;	// 256 / 8 = 32
	U32 validate_idx = 0;
	if (validate)
	{
		validate_idx = (gSavedSettings.getU32("CacheValidateCounter") / FRACTION) * FRACTION;
		U32 next_idx = (validate_idx + FRACTION) % 256;
		gSavedSettings.setU32("CacheValidateCounter", next_idx);
		LL_DEBUGS("TextureCache") << "TEXTURE CACHE: Validating indexes "
								  << validate_idx << " to "
								  << validate_idx + FRACTION - 1 << LL_ENDL;
	}
		 ...
		 iter != end; ++iter)
	{
		...
		else if (validate)
		{
			...
			if (uuididx >= validate_idx && uuididx < validate_idx + 4)
			{
				...
				{
					llwarns << "TEXTURE CACHE BODY HAS BAD SIZE: " << bodysize
	...
	if (purge_count > 0)
	{
		...
		llinfos << "TEXTURE CACHE: Purged: " << purge_count
				<< " - Entries: " << num_entries
	...
	{
		llinfos << "TEXTURE CACHE: nothing to purge." << llendl;
	}
										...
										S32 imagesize, S32 datasize)
{
	...
	if (idx < 0) // retry
	{
		...
		mHeaderMutex.lock();
		llassert_always(!mLRU.empty() ||
						mHeaderEntriesInfo.mEntries < sCacheMaxEntries);
		mHeaderMutex.unlock();

		// assert above ensures no inf. recursion
...
LLTextureCache::handle_t LLTextureCache::readFromCache(const std::string& filename,
													   const LLUUID& id,
													   U32 priority,
													   S32 offset,
													   S32 size,
													   ReadResponder* responder)
{
	...
	worker = new LLTextureCacheLocalFileWorker(this, priority, filename, id,
											   NULL, size, offset, 0,
											   responder);
													   ...
													   ReadResponder* responder)
{
	...
	LLMutexLock lock(&mWorkersMutex);
	LLTextureCacheWorker* worker;
	worker = new LLTextureCacheRemoteWorker(this, priority, id, NULL, size,
											offset, 0, responder);
	handle_t handle = worker->read();
	mReaders[handle] = worker;
	return handle;
													  ...
													  WriteResponder* responder)
{
	...
	static LLCachedControl<bool> cache_purge_time_sliced(gSavedSettings,
														 "CachePurgeTimeSliced");
	purgeTextureFilesTimeSliced(!cache_purge_time_sliced);

	LLMutexLock lock(&mWorkersMutex);
	LLTextureCacheWorker* worker;
	worker = new LLTextureCacheRemoteWorker(this, priority, id, data, datasize,
											0, imagesize, responder);
