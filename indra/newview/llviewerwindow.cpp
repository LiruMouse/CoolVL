...
public:
	...
	void update()
	{
		...
		{
			const U32 y_inc2 = 15;
			for (std::map<S32,LLFrameTimer>::reverse_iterator
					iter = gDebugTimers.rbegin(), rend = gDebugTimers.rend();
				 iter != rend; ++iter)
			{
				S32 idx = iter->first;
		...
		if (debug_show_render_info)
		{
			...
			if (gMeshRepo.meshRezEnabled())
			{
				...
				addText(xpos, ypos, llformat("%d/%d Mesh LOD Pending/Processing",
		...
		if (debug_show_mesh_queue && !gMeshRepo.mUploads.empty())
		{
	...
	if (activated)
	{
		...
		if (mWindow->getFullscreen() && !mIgnoreActivate)
		{
			if (!LLApp::isExiting())
			{
				...
				{
					// if we're in world, show a progress bar to hide reloading
					// of textures
...
void LLViewerWindow::initBase()
{
	...
	// Console
	llassert(!gConsole);
	gConsole = new LLConsole("console", getChatConsoleRect(),
...
void LLViewerWindow::initWorldUI()
{
	...
	if (gToolBar == NULL)	// Don't re-enter if objects are alreay created
	{
		...
		gStatusBar->reshape(root_rect.getWidth(),
							gStatusBar->getRect().getHeight(), TRUE);
...
void LLViewerWindow::shutdownViews()
{
	...
	cleanup_menus();
	llinfos << "Menus destroyed" << llendflush;

	delete gFloaterTeleportHistory;
	gFloaterTeleportHistory = NULL;

	delete gFloaterWorldMap;
	gFloaterWorldMap = NULL;
	llinfos << "World map destroyed" << llendflush;

	delete gFloaterTools;
	gFloaterTools = NULL;
	...
	if (mRootView)
	{
		delete mRootView;
		mRootView = NULL;
		llinfos << "Root view destroyed." << llendflush;
	}
	else
	{
		llwarns << "Root view was already destroyed." << llendflush;
	}
	...
	llinfos << "Global pointers to deleted children of RootView set to NULL."
			<< llendflush;
}
...
void LLViewerWindow::shutdownGL()
{
	...
	stop_glerror();
	llinfos << "Fonts destroyed" << llendflush;

	gSky.cleanup();
	stop_glerror();
	llinfos << "Sky cleaned up" << llendflush;

	gPipeline.cleanup();
	stop_glerror();
	llinfos << "Pipeline cleaned up" << llendflush;

	// MUST clean up pipeline before cleaning up wearables
	LLWearableList::instance().cleanup();
	llinfos << "Wearables cleaned up" << llendflush;

	gTextureList.shutdown();
	stop_glerror();
	llinfos << "Texture list shut down" << llendflush;

	gBumpImageList.shutdown();
	stop_glerror();
	llinfos << "Cleaned up bump map images" << llendflush;

	LLWorldMapView::cleanupTextures();
	llinfos << "Cleaned up LLWorldMapView textures" << llendflush;

	LLViewerTextureManager::cleanup();
	LLImageGL::cleanupClass();
	llinfos << "Cleaned up textures and GL images" << llendflush;

	LLSelectMgr::getInstance()->cleanup();
	llinfos << "Cleaned up select manager" << llendflush;

	llinfos << "Stopping GL during shutdown" << llendflush;
	if (!gNoRender)
	{
		stopGL(FALSE);
		stop_glerror();
	...
	gGL.shutdown();
	llinfos << "GL shutdown" << llendflush;

	LLVertexBuffer::cleanupClass();
	llinfos << "LLVertexBuffer cleaned." << llendflush;
}
...
BOOL LLViewerWindow::handleKey(KEY key, MASK mask)
{
		...
		!gFocusMgr.getKeystrokesOnly())
	{
		...
		if (key < 0x80)
		{
			// Not a special key, so likely (we hope) to generate a character.
			// Let it fall through to character handler first.
	...
	// *FIXME: Having this special-cased right here (just so this can be
	//         invoked from the login screen) sucks.
	...
	if (keyboard_focus)
	{
		...
		if (gChatBar && gChatBar->inputEditorHasFocus())
		{
				...
				gSavedSettings.getBOOL("ArrowKeysMoveAvatar"))
			{
				...
				case KEY_PAGE_DOWN:
				case KEY_HOME:
					// when chatbar is empty or ArrowKeysMoveAvatar set, pass
					// arrow keys on to avatar...
					return FALSE;
				default:
					break;
				}
...
BOOL LLViewerWindow::handleUnicodeChar(llwchar uni_char, MASK mask)
{
		...
		(uni_char == 3 && mask == MASK_NONE))
	{
		return gViewerKeyboard.handleKey(KEY_RETURN, mask,
										 gKeyboard->getKeyRepeated(KEY_RETURN));
	}
...
BOOL LLViewerWindow::handlePerFrameHover()
{
	...
	if (!gFloaterView || !gKeyboard) return TRUE;
...
void LLViewerWindow::renderSelections(BOOL for_gl_pick, BOOL pick_parcel_walls, BOOL for_hud)
{
	...
	LLObjectSelectionHandle selection = selectmgr->getSelection();

	if (!for_hud && !for_gl_pick)
	{
		// Call this once and only once
		selectmgr->updateSilhouettes();
	}
	...
	{
		selectmgr->renderSilhouettes(for_hud);

		stop_glerror();

		// setup HUD render
		if (selection->getSelectType() == SELECT_TYPE_HUD &&
			selectmgr->getSelection()->getObjectCount())
		{
		...
		if (LLSelectMgr::sRenderLightRadius && LLToolMgr::getInstance()->inEdit())
		{
			...
			selectmgr->getSelection()->applyToObjects(&func);
		...
		if (tool)
		{
			...
			{
				if (!selectmgr->getSelection()->isEmpty())
				{
					...
					for (LLObjectSelection::iterator
							iter = selectmgr->getSelection()->begin(),
							end = selectmgr->getSelection()->end();
						 iter != end; ++iter)
					{
...
//mk
					...
					if (tool == LLToolCompTranslate::getInstance() &&
						(!moveable_object_selected || !all_selected_objects_move))
					{
						draw_handles = FALSE;
					}
					...
					if (!all_selected_objects_modify &&
...
BOOL LLViewerWindow::clickPointOnSurfaceGlobal(const S32 x, const S32 y,
											   LLViewerObject *objectp,
											   LLVector3d &point_global) const
{
...
//	U8 shape = objectp->mPrimitiveCode & LL_PCODE_BASE_MASK;
	if (!intersect)
	{
		...
		LL_DEBUGS("Window") << "approx intersection at "
							<<  objectp->getPositionGlobal() - point_global
							<< LL_ENDL;
	}
	else
	{
		LL_DEBUGS("Window") << "good intersection at "
							<<  objectp->getPositionGlobal() - point_global
							<< LL_ENDL;
	}
...
void LLViewerWindow::pickAsync(S32 x, S32 y_from_bot, MASK mask,
							   ...
							   BOOL pick_transparent, BOOL get_surface_info)
{
	if (gNoRender)
	{
		return;
...
void LLViewerWindow::performPick()
{
	...
	if (!mPicks.empty())
	{
		for (std::vector<LLPickInfo>::iterator pick_it = mPicks.begin(),
											   end = mPicks.end();
			 pick_it != end; ++pick_it)
		{
											   ...
											   LLVector3* intersection)
{
	...
	return LLHUDIcon::lineSegmentIntersectAll(mouse_world_start,
											  mouse_world_end, intersection);
}
											 ...
											 const LLVector3 &plane_normal_global)
{
	...
	if (llabs(plane_mouse_dot) < 0.00001)
	{
		...
		mouse_look_at_scale = plane_origin_camera_rel.magVec() /
...
BOOL LLViewerWindow::mousePointOnLandGlobal(const S32 x, const S32 y,
											LLVector3d* land_position_global)
{
	...
	for (mouse_dir_scale = FIRST_PASS_STEP;
		 mouse_dir_scale < gAgent.mDrawDistance;
		 mouse_dir_scale += FIRST_PASS_STEP)
	{
		...
		regionp = LLWorld::getInstance()->resolveRegionGlobal(probe_point_region,
															  probe_point_global);
		...
		{
			//llinfos << "probe_point is out of region" << llendl;
			continue;
		}
	...
	if (hit_land)
	{
		...
		// will stop the loop before the last increment...
		// X - 1.0 + 0.1 + 0.1 + ... + 0.1 != X
...
/*	if (high_res)
	...
	LLViewerCamera* camera = LLViewerCamera::getInstance();
	F32 depth_conversion_factor_1 = (camera->getFar() + camera->getNear()) / (2.f * camera->getFar() * camera->getNear());
	F32 depth_conversion_factor_2 = (camera->getFar() - camera->getNear()) / (2.f * camera->getFar() * camera->getNear());

	gObjectList.generatePickList(*camera);

	// Subimages are in fact partial rendering of the final view. This happens
	// when the final view is bigger than the screen.
	// In most common cases, scale_factor is 1 and there's no more than 1
	// iteration on x and y
	for (int subimage_y = 0; subimage_y < scale_factor; ++subimage_y)
	{
		S32 subimage_y_offset = llclamp(buffer_y_offset - (subimage_y * window_height),
										0, window_height);
		// handle fractional columns
		U32 read_height = llmax(0, window_height - subimage_y_offset -
						  		   llmax(0, window_height * (subimage_y + 1) -
		...
		for (int subimage_x = 0; subimage_x < scale_factor; ++subimage_x)
		{
			...
			S32 subimage_x_offset = llclamp(buffer_x_offset - (subimage_x * window_width),
											0, window_width);
			// handle fractional rows
			U32 read_width = llmax(0, window_width - subimage_x_offset -
									  llmax(0, window_width * (subimage_x + 1) -
			...
			if (read_width && read_height)
			{
				...
				{
					S32 output_buffer_offset = raw->getComponents() *
					...
					else // SNAPSHOT_TYPE_DEPTH
					{
						...
						for (S32 i = 0; i < (S32)read_width; i++)
						{
							...
							U8 depth_byte = F32_to_U8(linear_depth_float, camera->getNear(), camera->getFar());
...
void LLViewerWindow::drawMouselookInstructions()
{
	...
	static LLCachedControl<U32> fade_mouselook_exit_tip(gSavedSettings,
...
{
	LLViewerCamera::getInstance()->setPerspective(NOT_FOR_SELECTION, 0, 0,
												  mWindowRect.getWidth(),
												  mWindowRect.getHeight(),
												  FALSE,
...
void LLViewerWindow::getTargetWindow(BOOL& fullscreen, S32& width, S32& height) const
{
	...
	if (mWindow && mWindow->getFullscreen() == mWantFullscreen)
	{
...
void LLPickInfo::fetchResults()
{
	...
	LLHUDIcon* hit_icon = gViewerWindow->cursorIntersectIcon(mMousePt.mX,
															 mMousePt.mY,
															 512.f,
															 &intersection);
	F32 icon_dist = 0.f;
	...
	LLViewerObject* hit_object = gViewerWindow->cursorIntersect(mMousePt.mX,
																mMousePt.mY,
																512.f,
																NULL, -1,
																mPickTransparent,
	...
	else if (objectp)
	{
		if (objectp->getPCode() == LLViewerObject::LL_VO_SURFACE_PATCH)
		{
			...
			LLVector3d land_pos;
			if (!gViewerWindow->mousePointOnLandGlobal(mPickPt.mX, mPickPt.mY,
													   &land_pos))
			{
