...
void LLViewerMediaFocus::cleanupClass()
{
	...
	if (self)
	{
		...
		/* Richard says:
			all widgets are supposed to be destroyed at the same time you
			shouldn't hold on to pointer to them outside of ui code you can use
			the LLHandle approach if you want to be type safe, you'll need to
			add a LLRootHandle to whatever derived class you are pointing to
			look at llview::gethandle: its our version of a weak pointer
		*/
...
void LLViewerMediaFocus::setFocusFace(BOOL b,
