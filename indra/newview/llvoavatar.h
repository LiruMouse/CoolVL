...
public:
	...
	void					setUseDeformer(bool deformed)	{ mUseDeformer = deformed; }
...
public:
	...
	static S32		sMaxVisible;			// affected by control "RenderAvatarMaxVisible"
...
public:
	// Made this one public instead of protected, to allow rebuilding our avatar
	void			buildCharacter();
protected:
	static BOOL		parseSkeletonFile(const std::string& filename);
	virtual BOOL	loadAvatar();

	BOOL			setupBone(const LLVOAvatarBoneInfo* info,
							  LLViewerJoint* parent,
							  S32 &current_volume_num,
							  S32 &current_joint_num);
...
private:
	...
	S32			mUpdatePeriod;
	S32			mNumInitFaces;	// number of faces generated when creating the avatar drawable, does not include splitted faces due to long vertex buffer.
...
protected:
	...
	//--------------------------------------------------------------------
protected:
	virtual void	setLocalTexture(LLVOAvatarDefines::ETextureIndex type,
									LLViewerTexture* tex,
									BOOL baked_version_exits,
									U32 index = 0);
	virtual void	addLocalTextureStats(LLVOAvatarDefines::ETextureIndex type,
	...
	virtual void	setBakedReady(LLVOAvatarDefines::ETextureIndex type,
...
protected:
	...
	void			addBakedTextureStats(LLViewerFetchedTexture* imagep,
										 F32 pixel_area,
										 F32 texel_area_ratio,
										 S32 boost_level);
...
private:
	...
	//--------------------------------------------------------------------
public:
	void			setClothesColor(LLVOAvatarDefines::ETextureIndex te,
									const LLColor4& new_color,
									BOOL upload_bake);
	LLColor4		getClothesColor(LLVOAvatarDefines::ETextureIndex te);
	static BOOL		teToColorParams(LLVOAvatarDefines::ETextureIndex te,
									U32 *param_name);
...
private:
	...
	// speaks, it puts a voice symbol over the avatar's head) and gesticulations
...
protected: // Shared with LLVOAvatarSelf
	...
	struct LLVOAvatarXmlInfo
	{
		...
		struct LLVOAvatarMeshInfo
		{
			...
			~LLVOAvatarMeshInfo()
			{
				...
				for (iter = mPolyMorphTargetInfoList.begin();
					 iter != mPolyMorphTargetInfoList.end(); iter++)
				{
					delete iter->first;
				}
	...
	struct LLMaskedMorph
	{
		LLMaskedMorph(LLPolyMorphTarget *morph_target,
					  BOOL invert,
					  std::string layer)
