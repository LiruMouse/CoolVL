...
public:
	...
	void fire(const LLUUID& inv_item)
	{
		...
		if (!item)
		{
			llwarns << "Item add reported, but not found in inventory!: "
					<< inv_item << llendl;
		}
		else
		{
...
{
	if (ext_char >= LLTextEditor::FIRST_EMBEDDED_CHAR &&
		ext_char <= LLTextEditor::LAST_EMBEDDED_CHAR)
	{
		item_map_t::iterator iter = sEntries.find(ext_char);
		if (iter != sEntries.end())
		{
...
void LLEmbeddedItems::removeUnusedChars()
{
	...
	{
		llwchar wc = wtext[i];
		if (wc >= LLTextEditor::FIRST_EMBEDDED_CHAR &&
			wc <= LLTextEditor::LAST_EMBEDDED_CHAR)
		{
			used.erase(wc);
		}
	}
...
void LLEmbeddedItems::bindEmbeddedChars(const LLFontGL* font) const
{
	...
	for (std::set<llwchar>::const_iterator iter1 = mEmbeddedUsedChars.begin();
		 iter1 != mEmbeddedUsedChars.end(); ++iter1)
	{
		llwchar wch = *iter1;
...
BOOL LLViewerTextEditor::handleHover(S32 x, S32 y, MASK mask)
{
	...
	if (hasMouseCapture())
	{
		...
		else if (mDragItem)
		{
			...
			S32 screen_y;
			localPointToScreen(x, y, &screen_x, &screen_y);
			if (LLToolDragAndDrop::getInstance()->isOverThreshold(screen_x,
																  screen_y))
			{
				LLToolDragAndDrop::getInstance()->beginDrag(LLViewerAssetType::lookupDragAndDropType(mDragItem->getType()),
															mDragItem->getUUID(),
										   ...
										   std::string& tooltip_msg)
{
	...
	source = LLToolDragAndDrop::getInstance()->getSource();
	...
	if (mTakesNonScrollClicks)
	{
		...
		handled = TRUE;
		lldebugst(LLERR_USER_INPUT) << "dragAndDrop handled by LLViewerTextEditor "
									<< getName() << llendl;
	}
...
{
	std::string title = "  " + LLLandmarkBridge::prefix() + item->getName();
...
void LLViewerTextEditor::showUnsavedAlertDialog(LLInventoryItem* item)
{
	...
	LLNotifications::instance().add("ConfirmNotecardSave", LLSD(), payload,
									...
									LLUICtrlFactory* factory)
{
	...
	LLViewerTextEditor* text_editor = new LLViewerTextEditor(name, rect,
															 max_text_length,
															 LLStringUtil::null,
															 font,
															 allow_embedded_items);
