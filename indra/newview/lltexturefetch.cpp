							  ...
							  const LLIOPipe::buffer_ptr_t& buffer)
	{
		...
		if (worker)
		{
			...
			if (!success)
			{
				worker->setGetStatus(status, reason);
 				llwarns << "CURL get failed for " << mID << " with status: "
						<< status << llendl;
			}
		...
		else
		{
			mFetcher->removeFromHTTPQueue(mID);
 			llwarns << "Worker not found for " << mID << llendl;
		}
	}
...
bool LLTextureFetchWorker::doWork(S32 param)
{
	...
	if (mState == INIT)
	{
		...
		LL_DEBUGS("Texture") << mID << ": Priority: "
							 << llformat("%8.0f", mImagePriority)
							 << " Desired Discard: " << mDesiredDiscard
							 << " Desired Size: " << mDesiredSize << LL_ENDL;
		// fall through
	}
	...
	if (mState == CACHE_POST)
	{
		...
		if ((mCachedSize >= mDesiredSize) || mHaveAllData)
		{
			...
			mState = DECODE_IMAGE;
			mWriteToCacheState = NOT_WRITE;
			LL_DEBUGS("Texture") << mID << ": Cached. Bytes: "
								 << mFormattedImage->getDataSize() << " Size: "
								 << llformat("%dx%d",
...
// 		if (mHost != LLHost::invalid) use_http = false;
	...
	if (mState == LOAD_FROM_SIMULATOR)
	{
		...
		{
			LL_DEBUGS("Texture") << mID << ": Loaded from Sim. Bytes: "
								 << mFormattedImage->getDataSize() << LL_ENDL;
...
// 				llwarns << "processSimulatorPackets() failed to load buffer" << llendl;
	...
	if (mState == SEND_HTTP_REQ)
	{
		if (mCanUseHTTP)
		{
			...
			if (!mUrl.empty())
			{
				...
				mGetStatus = 0;
				mGetReason.clear();
				LL_DEBUGS("Texture") << "HTTP GET: " << mID << " Offset: "
									 << offset << " Bytes: " << mRequestedSize
									 << " Bandwidth(kbps): "
									 << mFetcher->getTextureBandwidth()
									 << "/" << mFetcher->mMaxBandwidth
									 << LL_ENDL;
				setPriority(LLWorkerThread::PRIORITY_LOW | mWorkPriority);
	...
	if (mState == DECODE_IMAGE)
	{
		...
		LL_DEBUGS("Texture") << mID << ": Decoding. Bytes: "
							 << mFormattedImage->getDataSize() << " Discard: "
							 << discard << " All Data: " << mHaveAllData
							 << LL_ENDL;
		mDecodeHandle = mFetcher->mImageDecodeThread->decodeImage(mFormattedImage,
																  image_priority,
																  discard,
																  mNeedsAux,
...
// 					llwarns << "UNABLE TO LOAD TEXTURE: " << mID << " RETRIES: " << mRetryAttempt << llendl;
			...
			else
			{
				...
				LL_DEBUGS("Texture") << mID << ": Decoded. Discard: "
										   ...
										   bool partial, bool success)
{
	...
	if (success)
	{
		...
		LL_DEBUGS("Texture") << "HTTP RECEIVED: " << mID.asString()
							 << " Bytes: " << data_size << LL_ENDL;
		if (data_size > 0)
		{
			...
			{
				LL_DEBUGS("Texture") << "Extra data received: got "
									 << data_size << " bytes (requested: "
									 << mRequestedSize << " ) for texture: "
									 << mID.asString() << LL_ENDL;
...
// 		llwarns << "Decode callback for " << mID << " with state = " << mState << llendl;
	...
	if (success)
	{
		...
		mDecodedDiscard = mFormattedImage->getDiscardLevel();
 		LL_DEBUGS("Texture") << mID << ": Decode Finished. Discard: "
							 << mDecodedDiscard << " Raw Image: "
							 << llformat("%dx%d", mRawImage->getWidth(),
										 mRawImage->getHeight()) << LL_ENDL;
	}
	else
	{
		if (mFormattedImage.notNull())
		{
			llwarns << "DECODE FAILED: id = " << mID << ", Discard = "
					<< (S32)mFormattedImage->getDiscardLevel() << llendl;
		}
		else
		{
			llwarns << "DECODE FAILED: id = " << mID
					<< ", mFormattedImage is Null !" << llendl;
		}
		removeFromCache();
...
LLTextureFetch::LLTextureFetch(LLTextureCache* cache,
							   LLImageDecodeThread* imagedecodethread,
							   bool threaded)
	...
	mCurlGetRequest(NULL)
{
	...
	mTextureInfo.setUpLogging(gSavedSettings.getBOOL("LogTextureDownloadsToViewerLog"),
							  gSavedSettings.getBOOL("LogTextureDownloadsToSimulator"),
							  gSavedSettings.getU32("TextureLoggingThreshold"));
}
...
// 				llwarns << "Adding work for inactive worker: " << id << llendl;
		...
		else if (worker->checkWork())
		{
			...
			res = true;
			LL_DEBUGS("Texture") << id << ": Request Finished. State: "
...
S32 LLTextureFetch::update(F32 max_time_ms)
{
	...
	if (!mThreaded)
	{
		...
		if (processed > 0)
		{
			LL_DEBUGS("Texture") << "processed: " << processed << " messages."
								 << LL_ENDL;
		}
	}
...
void LLTextureFetch::sendRequestListToSimulators()
{
	...
	work_request_map_t requests;
	{
		...
		for (queue_t::iterator iter = mNetworkQueue.begin();
			 iter != mNetworkQueue.end(); )
		{
			...
			if (!req)
			{
				// This happens when a request was removed from mRequestMap in
				// a race with adding it to mNetworkQueue by doWork (see
				// SNOW-196).
				...
				req->mState != LLTextureFetchWorker::LOAD_FROM_SIMULATOR)
			{
				...
				llwarns << "Worker: " << req->mID
						<< " in mNetworkQueue but in wrong state: "
						<< req->mState << llendl;
				mNetworkQueue.erase(curiter);
				continue;
			}
...
bool LLTextureFetchWorker::insertPacket(S32 index, U8* data, S32 size)
{
	...
	{
		//llwarns << "Received Image Packet " << index << " > max: "
		//		<< mTotalPackets << " for image: " << mID << llendl;
		return false;
	}
	if (index > 0 && index < mTotalPackets-1 && size != MAX_IMG_PACKET_SIZE)
	{
 		//llwarns << "Received bad sized packet: " << index << ", " << size
		//		<< " != " << MAX_IMG_PACKET_SIZE << " for image: " << mID
		//		<< llendl;
		return false;
	}
	...
	{
		//llwarns << "Received duplicate packet: " << index << " for image: "
		//		<< mID << llendl;
		return false;
	}
