...
void LLViewerParcelMgr::renderParcelCollision()
{
	...
	static LLCachedControl<bool> show_ban_lines(gSavedSettings, "ShowBanLines");
	if (mRenderCollision && show_ban_lines)
	{
...
void optionally_start_music(LLParcel* parcel)
{
	if (gSavedSettings.getBOOL("AudioStreamingMusic"))
	{
		...
		// now only play music when you enter a new parcel if the control is
		// in PLAY state
...
void LLViewerParcelMgr::deedLandToGroup()
{
	...
	if (mCurrentParcel->getContributeWithDeed())
	{
		...
		LLNotifications::instance().add("DeedLandToGroupWithContribution",args,
										LLSD(), deedAlertCB);
	}
	else
	{
		LLNotifications::instance().add("DeedLandToGroup",args, LLSD(),
										deedAlertCB);
	}
}
...
*/
	...
	LLSD args;
	args["AREA"] = llformat("%d",mCurrentParcel->getArea());
	LLNotifications::instance().add("ReleaseLandWarning", args, LLSD(),
									releaseAlertCB);
}
...
void LLViewerParcelMgr::startDivideLand()
{
	...
	LLNotifications::instance().add("LandDivideWarning", LLSD(), payload,
									callbackDivideLand);
}
...
void LLViewerParcelMgr::startJoinLand()
{
	...
	LLNotifications::instance().add("JoinLandWarning", LLSD(), payload,
									callbackJoinLand);
}
