...
LLFolderViewItem* LLFolderViewItem::getNextOpenNode(BOOL include_children)
{
	...
	LLFolderViewItem* itemp = mParentFolder->getNextFromChild(this,
															  include_children);
	while (itemp && !itemp->getVisible())
	{
		LLFolderViewItem* next_itemp;
		next_itemp = itemp->mParentFolder->getNextFromChild(itemp,
															include_children);
		if (itemp == next_itemp) 
		{
			// hit last item
...
LLFolderViewItem* LLFolderViewItem::getPreviousOpenNode(BOOL include_children)
{
	...
	LLFolderViewItem* itemp = mParentFolder->getPreviousFromChild(this,
																  include_children);
	while (itemp && !itemp->getVisible())
	{
		LLFolderViewItem* next_itemp;
		next_itemp = itemp->mParentFolder->getPreviousFromChild(itemp,
																include_children);
		if (itemp == next_itemp) 
		{
			// hit first item
...
BOOL LLFolderViewItem::potentiallyVisible()
{
	...
	return getLastFilterGeneration() < getRoot()->getFilter()->getMinRequiredGeneration() ||
		   getFiltered();
}
...
// This function clears the currently selected item, and records the specified
...
{
	mIndentation = mParentFolder ? mParentFolder->getIndentation() + LEFT_INDENTATION
								 : 0;
										 ...
										 std::string& tooltip_msg)
{
	...
	if (handled)
	{
		lldebugst(LLERR_USER_INPUT) << "dragAndDrop handled by LLFolderViewItem"
									<< llendl;
	}
...
void LLFolderViewItem::draw()
{
	...
	if (possibly_has_children/* && mControlLabel[0] != '\0'*/)
	{
		if (sArrowImage)
		{
			gl_draw_scaled_rotated_image(mIndentation,
										 getRect().getHeight() - ARROW_SIZE - TEXT_PAD,
	...
	if (!mLabel.empty())
	{
		...
		if (debug_filters)
		{
			...
			LLColor4 filter_color = mLastFilterGeneration >= getRoot()->getFilter()->getCurrentGeneration() ? LLColor4(0.5f, 0.8f, 0.5f, 1.f)
		...
		{
			// *TODO: Translate
			sFont->renderUTF8(std::string("Loading... "), 0, text_left, y,
							  sSearchStatusColor,
				...
				combined_string_upper.find(mRoot->getFilterSubString()) == mStringMatchOffset)
			{
				...
				F32 match_string_left = text_left +
										sFont->getWidthF32(combined_string, 0,
														   mStringMatchOffset);
				F32 y = (F32)getRect().getHeight() - sFont->getLineHeight() -
						(F32)TEXT_PAD;
				...
				iit != mItems.end(); ++iit)
			{
				...
				if (itemp->getVisible())
				{
					...
					target_height += itemp->arrange(&child_width, &child_height,
													filter_generation);
					// don't change width, as this item is as wide as its parent
										 ...
										 LLDynamicArray<LLFolderViewItem*>& selected_items)
{
	...
	if (found_last_selected && found_selection)
	{
		...
		for (S32 index = reverse_select ? items_to_select.getLength() - 1 : 0; 
			 reverse_select ? index >= 0 : index < items_to_select.getLength();
...
bool LLFolderViewFolder::isTrash() const
{
	...
	{
		mAmTrash = mListener->getUUID() == gInventory.findCategoryUUIDForType(LLFolderType::FT_TRASH, false) ? LLFolderViewFolder::TRASH
																											 : LLFolderViewFolder::NOT_TRASH;
	}
...
BOOL LLFolderViewFolder::addItem(LLFolderViewItem* item)
{
	...
	items_t::iterator it = std::lower_bound(mItems.begin(), mItems.end(), item,
											mSortFunction);
										   ...
										   std::string& tooltip_msg)
{
	...
	if (!handled)
	{
		...
		lldebugst(LLERR_USER_INPUT) << "dragAndDrop handled by LLFolderViewFolder"
									<< llendl;
	}
	...
	mDragAndDropThisFrame(FALSE)
{
	...
	mRenamer = new LLLineEditor(std::string("ren"), getRect(),
								LLStringUtil::null, sFont,
	...
	LLMenuGL* menu = LLUICtrlFactory::getInstance()->buildMenu("menu_inventory.xml",
															   parent_view);
	if (!menu)
	{
						  ...
						  S32 filter_generation)
{
		 ...
		 iter != mFolders.end(); )
	{
		...
		{
			folderp->setVisible(show_folder_state == LLInventoryFilter::SHOW_ALL_FOLDERS || // always show folders ?
								folderp->getFiltered(filter_generation) || 					// passed filter
								folderp->hasFilteredDescendants(filter_generation));		// or has descendants that passed filter
		}
		if (folderp->getVisible())
		{
			S32 child_height = 0;
...
void LLFolderView::sanitizeSelection()
{
	...
	{
		// ...select first available parent of original selection, or
		// "My Inventory" otherwise
...
BOOL LLFolderView::getSelectionList(std::set<LLUUID> &selection)
{
	...
	for (item_it = mSelectedItems.begin(); item_it != mSelectedItems.end();
		 ++item_it)
	{
...
BOOL LLFolderView::startDrag(LLToolDragAndDrop::ESource source)
{
	...
	if (!mSelectedItems.empty())
	{
			 ...
			 ++item_it)
		{
			...
			can_drag = can_drag && (*item_it)->getListener()->startDrag(&type,
																		&id);
...
void LLFolderView::draw()
{
	if (mDebugFilters)
	{
		std::string current_filter_string = llformat("Current Filter: %d, Least Filter: %d, Auto-accept Filter: %d",
													 mFilter.getCurrentGeneration(),
													 mFilter.getMinRequiredGeneration(),
													 mFilter.getMustPassGeneration());
		sSmallFont->renderUTF8(current_filter_string, 0, 2, 
							   getRect().getHeight() - sSmallFont->getLineHeight(),
							   LLColor4(0.5f, 0.5f, 0.8f, 1.f), 
							   LLFontGL::LEFT, LLFontGL::BOTTOM,
							   LLFontGL::NORMAL, S32_MAX, S32_MAX, NULL,
							   FALSE);
	}
	...
	if (hasVisibleChildren() ||
		getShowFolderState() == LLInventoryFilter::SHOW_ALL_FOLDERS)
	{
		mStatusText.clear();
	}
	else
	{
			...
			mCompletedFilterGeneration < mFilter.getMinRequiredGeneration())
		{
			...
			sFont->renderUTF8(mStatusText, 0, 2, 1, sSearchStatusColor,
							  LLFontGL::LEFT, LLFontGL::TOP, LLFontGL::NORMAL,
	...
	if (getVisible() && getEnabled())
	{
				...
				if (parent->removeItem(item_to_delete))
				{
					...
					if (new_selection)
					{
						setSelectionFromRoot(new_selection,
											 new_selection->isOpen(),
											 gFocusMgr.childHasKeyboardFocus(this));
					}
					else
					{
						setSelectionFromRoot(NULL,
											 gFocusMgr.childHasKeyboardFocus(this));
					}
				}
			}
		...
		else if (count > 1)
		{
			...
			for (S32 i = 0; i < count; ++i)
			{
				...
				if (listener &&
					listeners.find(listener) == LLDynamicArray<LLFolderViewEventListener*>::FAIL)
				{
...
void LLFolderView::openSelectedItems(void)
{
	if (getVisible() && getEnabled())
	{
		...
		else
		{
			...
			multi_previewp = new LLMultiPreview(LLRect(left, top, left + 300,
													   top - 100));
			gFloaterView->getNewFloaterPosition(&left, &top);
			...
			multi_propertiesp = new LLMultiProperties(LLRect(left, top,
															 ...
															 top - 100));
			selected_items_t::iterator item_it;
			for (item_it = mSelectedItems.begin();
				 item_it != mSelectedItems.end(); ++item_it)
			{
				...
				bool is_prop = listener &&
							   (listener->getInventoryType() == LLInventoryType::IT_OBJECT ||
								listener->getInventoryType() == LLInventoryType::IT_ATTACHMENT);
				if (is_prop)
				{
			...
			// *NOTE: LLMulti* will safely auto-delete when opened without any
			// children.
			multi_previewp->open();
...
BOOL LLFolderView::autoOpenTest(LLFolderViewFolder* folder)
{
	if (folder && mAutoOpenCandidate == folder)
	{
		if (mAutoOpenTimer.getStarted())
		{
			...
			{
				mAutoOpenCandidate->setAutoOpenCountdown(clamp_rescale(mAutoOpenTimer.getElapsedTimeF32(),
																	   0.f, sAutoOpenTime, 0.f, 1.f));
			}
...
BOOL LLFolderView::canCopy() const
{
	...
	for (selected_items_t::const_iterator selected_it = mSelectedItems.begin();
		 selected_it != mSelectedItems.end(); ++selected_it)
	{
...
BOOL LLFolderView::canCut() const
{
		 ...
		 selected_it != mSelectedItems.end(); ++selected_it)
	{
		...
		if (!item->getListener()->isItemMovable())
		{
			return FALSE;
		}
	}
...
void LLFolderView::paste()
{
	if (getVisible() && getEnabled())
	{
		...
		for (selected_it = mSelectedItems.begin();
			 selected_it != mSelectedItems.end(); ++selected_it)
		{
			LLFolderViewItem* item = *selected_it;
...
BOOL LLFolderView::handleKeyHere(KEY key, MASK mask)
{
	...
	case KEY_DOWN:
		if ((mSelectedItems.size() > 0) && mScrollContainer)
		{
			...
			if (mask & MASK_SHIFT)
			{
				// don't shift select down to children of folders (they are
				// implicitly selected through parent)
...
BOOL LLFolderView::handleUnicodeCharHere(llwchar uni_char)
{
	...
	if (uni_char > 0x7f)
	{
		llwarns << "LLFolderView::handleUnicodeCharHere - Don't handle non-ascii yet, aborting"
				<< llendl;
		return FALSE;
	}
...
BOOL LLFolderView::handleRightMouseDown(S32 x, S32 y, MASK mask)
{
	...
	if (handled && (count > 0) && menu)
	{
		...
		for (item_itor = mSelectedItems.begin();
			 item_itor != mSelectedItems.end(); ++item_itor)
		{
...
void LLFolderView::scrollToShowItem(LLFolderViewItem* item)
{
	...
	if (item && mScrollContainer)
	{
		...
		S32 max_height_to_show = gFocusMgr.childHasKeyboardFocus(this) ? (llmax(icon_height, label_height) + ICON_PAD)
		...
		item_scrolled_rect.mRight = llmin(item_scrolled_rect.mLeft + MIN_ITEM_WIDTH_VISIBLE,
										  item_scrolled_rect.mRight);
		...
		item->localPointToOtherView(item->getIndentation(),
									...
									mScrollContainer);
		mLastScrollOffset.set(-mScrollContainer->getBorderWidth() - new_item_left_top.mX,
							  mScrollContainer->getRect().getHeight() - new_item_left_top.mY - 1);
	}
}
...
void LLFolderView::doIdle()
{
	...
	static LLCachedControl<bool> debug_filters(gSavedSettings,
											   "DebugInventoryFilters");
	if (mDebugFilters != debug_filters)
	{
	...
	BOOL filter_modified_and_active = mFilter.isNotDefault() &&
	...
	// Do this every frame until user puts keyboard focus into the inventory
	// window signaling the end of the automatic update.
	...
	if (mNeedsAutoSelect)
	{
		...
		LLFolderViewItem* selected_itemp = mSelectedItems.empty() ? NULL
																  : mSelectedItems.back();
		...
		else if (!mAutoSelectOverride &&
				 (!selected_itemp || !selected_itemp->getFiltered()))
		{
	...
	if (mSelectedItems.size() && mNeedsScroll)
	{
		...
		if ((!needsArrange() || !is_visible) &&
			getCompletedFilterGeneration() >= mFilter.getMinRequiredGeneration())
		{
			mNeedsScroll = FALSE;
		}
	}
...
BOOL LLInventoryFilter::check(LLFolderViewItem* item) 
{
	...
	if (!(0x1 << object_type & mFilterOps.mFilterTypes) &&
		object_type != LLInventoryType::IT_NONE)
	{
		return FALSE;
	}

	mSubStringMatchOffset = mFilterSubString.size() ? item->getSearchableLabel().find(mFilterSubString)
													: std::string::npos;
	if (mFilterSubString.size() &&  mSubStringMatchOffset == std::string::npos)
	{
		return FALSE;
	}
	...
	if ((listener->getPermissionMask() & mFilterOps.mPermissions) != mFilterOps.mPermissions)
	{
		return FALSE;
	}
	...
	static LLCachedControl<bool> hide_empty_system_folders(gSavedSettings,
														   "HideEmptySystemFolders");
	if (object_type == LLInventoryType::IT_CATEGORY &&
		hide_empty_system_folders)
	{
...
BOOL LLInventoryFilter::isNotDefault()
{
	return mFilterOps.mFilterTypes != mDefaultFilterOps.mFilterTypes ||
		   mFilterSubString.size() || mFilterWorn ||
...
void LLInventoryFilter::setFilterSubString(const std::string& string)
{
	if (mFilterSubString != string)
	{
		...
		BOOL less_restrictive = mFilterSubString.size() >= string.size() &&
								!mFilterSubString.substr(0, string.size()).compare(string);
		// appending new characters
		BOOL more_restrictive = mFilterSubString.size() < string.size() &&
								!string.substr(0, mFilterSubString.size()).compare(mFilterSubString);
...
void LLInventoryFilter::setFilterPermissions(PermissionMask perms)
{
	if (mFilterOps.mPermissions != perms)
	{
		...
		{
			// target must have all requested permission bits, so more
			// bits == more restrictive
			setModified(FILTER_MORE_RESTRICTIVE);
		}
		else if (fewer_bits_set)
		{
...
void LLInventoryFilter::setHoursAgo(U32 hours)
{
	if (mFilterOps.mHoursAgo != hours)
	{
		...
		BOOL less_restrictive = (mFilterOps.mMinDate == time_min() &&
								 mFilterOps.mMaxDate == time_max() &&
								 hours > mFilterOps.mHoursAgo);
		BOOL more_restrictive = (mFilterOps.mMinDate == time_min() &&
								 mFilterOps.mMaxDate == time_max() &&
								 hours <= mFilterOps.mHoursAgo);
...
void LLInventoryFilter::setModified(EFilterBehavior behavior)
{
	...
	if (isNotDefault())
	{
		...
		case FILTER_MORE_RESTRICTIVE:
			mMinRequiredGeneration = mFilterGeneration;
			// must have passed either current filter generation (meaningless,
			// as it hasn't been run yet) or some older generation, so keep the
			// value
