...
void LLHUDText::renderText()
{
	...
	if (mDoFade && mLastDistance > mFadeDistance)
	{
		alpha_factor = llmax(0.f,
							 1.f - (mLastDistance - mFadeDistance) / mFadeRange);
		text_color.mV[3] = text_color.mV[3] * alpha_factor;
	}
	if (text_color.mV[3] < 0.01f)
	{
	...
	{
		LLViewerCamera::getInstance()->getPixelVectors(mPositionAgent,
													   y_pixel_vec,
													   x_pixel_vec);
	}
	...
	LLCoordGL screen_pos;
	LLViewerCamera::getInstance()->projectPosAgentToScreen(mPositionAgent,
														   screen_pos, FALSE);
	...
	if (mUseBubble)
	{
		...
		LLUI::pushMatrix();
		{
			...
			if (mLabelSegments.size())
			{
				LLUI::pushMatrix();
				{
					gGL.color4f(text_color.mV[VX], text_color.mV[VY], text_color.mV[VZ],
	...
	// Render label
	{
		...
		for (std::vector<LLHUDTextSegment>::iterator
				segment_iter = mLabelSegments.begin(),
				end = mLabelSegments.end();
			 segment_iter != end; ++segment_iter)
		{
			const LLFontGL* fontp = segment_iter->mStyle == LLFontGL::BOLD ? mBoldFontp
																		   : mFontp;
			...
			hud_render_text(segment_iter->getText(), render_position, *fontp,
							segment_iter->mStyle, x_offset, y_offset,
	...
	// Render text
	{
		...
		for (std::vector<LLHUDTextSegment>::iterator
				segment_iter = mTextSegments.begin() + start_segment,
				end = mTextSegments.end();
			 segment_iter != end; ++segment_iter)
		{
		...
		while (iter != tokens_end)
		{
			...
			do
			{
				S32 segment_length = mFontp->maxDrawableChars(iter->substr(line_length).c_str(),
															  ...
															  wline.length(),
															  TRUE);
				mTextSegments.push_back(LLHUDTextSegment(iter->substr(line_length, segment_length),
														 style, color));
		...
		while (iter != tokens_end)
		{
			...
			do
			{
				S32 segment_length = mFontp->maxDrawableChars(iter->substr(line_length).c_str(),
															  ...
															  wstr.length(),
															  TRUE);
				mLabelSegments.push_back(LLHUDTextSegment(iter->substr(line_length, segment_length),
...
void LLHUDText::setColor(const LLColor4 &color)
{
	mColor = color;
	for (std::vector<LLHUDTextSegment>::iterator
			segment_iter = mTextSegments.begin(), end = mTextSegments.end();
		 segment_iter != end; ++segment_iter)
	{
...
LLVector2 LLHUDText::updateScreenPos(LLVector2 &offset)
{
	...
	if (!camera->projectPosAgentToScreen(world_pos, screen_pos, FALSE) && mVisibleOffScreen)
	{
		...
		LLVector2 window_center(gViewerWindow->getWindowDisplayWidth() * 0.5f,
								gViewerWindow->getWindowDisplayHeight() * 0.5f);
	...
	screen_center.mV[VX] = llclamp((F32)screen_pos_vec.mV[VX], mWidth * 0.5f,
								   (F32)gViewerWindow->getWindowDisplayWidth() - mWidth * 0.5f);

	if (mVertAlignment == ALIGN_VERT_TOP)
	{
		screen_center.mV[VY] = llclamp((F32)screen_pos_vec.mV[VY], (F32)bottom,
		...
		mSoftScreenRect.setLeftTopAndSize(screen_center.mV[VX] - (mWidth + BUFFER_SIZE) * 0.5f,
										  screen_center.mV[VY] + (mHeight + BUFFER_SIZE),
										  mWidth + BUFFER_SIZE,
										  mHeight + BUFFER_SIZE);
	}
	else
...
void LLHUDText::updateAll()
{
	...
	std::vector<LLPointer<LLHUDText> >::reverse_iterator r_end = sVisibleTextObjects.rend();
	for (r_it = sVisibleTextObjects.rbegin(); r_it != r_end; ++r_it)
	{
	...
	VisibleTextObjectIterator end = sVisibleTextObjects.end();

	for (S32 i = 0; i < NUM_OVERLAP_ITERATIONS; ++i)
	{
		for (src_it = sVisibleTextObjects.begin(); src_it != end; ++src_it)
		{
	...
	for (this_object_it = sVisibleTextObjects.begin(); this_object_it != end;
		 ++this_object_it)
	{
		...
		(*this_object_it)->mPositionOffset = lerp((*this_object_it)->mPositionOffset,
												  (*this_object_it)->mTargetPositionOffset,
...
void LLHUDText::renderAllHUD()
{
...

	{
		...
		VisibleTextObjectIterator end = sVisibleHUDTextObjects.end();

		for (text_it = sVisibleHUDTextObjects.begin(); text_it != end;
			 ++text_it)
		{
			(*text_it)->renderText();
		}
...
{
	TextObjectIterator text_it;
	TextObjectIterator end = sTextObjects.end();
...
{
	//this might put an object on the pick list a second time, overriding it's
	// mGLName, which is ok
	// *FIX: we should probably cull against pick frustum
	VisibleTextObjectIterator text_it;
	VisibleTextObjectIterator end = sVisibleTextObjects.end();
	for (text_it = sVisibleTextObjects.begin(); text_it != end; ++text_it)
	{
...
void LLHUDText::reshape()
{
	...
	for (text_it = sTextObjects.begin(); text_it != end; ++text_it)
	{
		...
		std::vector<LLHUDTextSegment>::iterator end2 = textp->mTextSegments.end();
		for (segment_iter = textp->mTextSegments.begin();
			 segment_iter != end2; ++segment_iter)
		{
			segment_iter->clearFontWidthMap();
		}
		end2 = textp->mLabelSegments.end();
