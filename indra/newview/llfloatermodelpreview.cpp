...
// Does not seem needed any more and causes crashes for rigged meshes
#undef FIRST_LOAD_HACK
...
void LLFloaterModelPreview::createSmoothComboBox(LLComboBox* combo_box, float min, float max)
{
	...
	{
		std::string label = (++ilabel == SMOOTH_VALUES_NUMBER ? "10 (max)"
															  : llformat("%.1d", ilabel));
		combo_box->add(label, value, ADD_BOTTOM, true);
	}
}
...
bool LLModelLoader::doLoadModel()
{
		...
		if (i != std::string::npos)
		{
			...
			{	//slm file exists
					...
					dae_status.st_mtime < slm_status.st_mtime)
				{
					...
					{	// slm successfully loaded, if this fails, fall through
						// and try loading from dae
	...
	U32 controllerCount = (U32)db->getElementCount(NULL, "controller");
	bool result = false;
					...
					if (model)
					{
						...
						else	// Has Skeleton
						{
							...
							if (pSkeletonRootNode)
							{
								...
								const S32 jointCnt = mJointMap.size();
								...
								// If anything failed in regards to extracting
								// the skeleton, joints or translation id,
								// mention it
						...
						{
							llwarns << "No joint names found in skin info." << llendl;
						}
...
void LLFloaterModelPreview::onUpload(void* userdata)
{
	...
	self->mModelPreview->saveUploadData(upload_skinweights, upload_joint_positions);
