			   ...
			   std::string type, LLFolderBridge *self = NULL)
{
	...
	else if ("shirt" == type)
	{
		...
		LLAgentWearables::createWearable(LLWearableType::WT_SHIRT, parent_id);
	}
	else if ("pants" == type)
	{
		LLUUID parent_id = self ? self->getUUID()
								: gInventory.findCategoryUUIDForType(LLFolderType::FT_CLOTHING);
		LLAgentWearables::createWearable(LLWearableType::WT_PANTS, parent_id);
	}
	else if ("shoes" == type)
	{
		LLUUID parent_id = self ? self->getUUID()
								: gInventory.findCategoryUUIDForType(LLFolderType::FT_CLOTHING);
		LLAgentWearables::createWearable(LLWearableType::WT_SHOES, parent_id);
	}
	else if ("socks" == type)
	{
		LLUUID parent_id = self ? self->getUUID()
								: gInventory.findCategoryUUIDForType(LLFolderType::FT_CLOTHING);
		LLAgentWearables::createWearable(LLWearableType::WT_SOCKS, parent_id);
	}
	else if ("jacket" == type)
	{
		LLUUID parent_id = self ? self->getUUID()
								: gInventory.findCategoryUUIDForType(LLFolderType::FT_CLOTHING);
		LLAgentWearables::createWearable(LLWearableType::WT_JACKET, parent_id);
	}
	else if ("skirt" == type)
	{
		LLUUID parent_id = self ? self->getUUID()
								: gInventory.findCategoryUUIDForType(LLFolderType::FT_CLOTHING);
		LLAgentWearables::createWearable(LLWearableType::WT_SKIRT, parent_id);
	}
	else if ("gloves" == type)
	{
		LLUUID parent_id = self ? self->getUUID()
								: gInventory.findCategoryUUIDForType(LLFolderType::FT_CLOTHING);
		LLAgentWearables::createWearable(LLWearableType::WT_GLOVES, parent_id);
	}
	else if ("undershirt" == type)
	{
		LLUUID parent_id = self ? self->getUUID()
								: gInventory.findCategoryUUIDForType(LLFolderType::FT_CLOTHING);
		LLAgentWearables::createWearable(LLWearableType::WT_UNDERSHIRT, parent_id);
	}
	else if ("underpants" == type)
	{
		LLUUID parent_id = self ? self->getUUID()
								: gInventory.findCategoryUUIDForType(LLFolderType::FT_CLOTHING);
		LLAgentWearables::createWearable(LLWearableType::WT_UNDERPANTS, parent_id);
	}
	else if ("alpha" == type)
	{
		LLUUID parent_id = self ? self->getUUID()
								: gInventory.findCategoryUUIDForType(LLFolderType::FT_CLOTHING);
		LLAgentWearables::createWearable(LLWearableType::WT_ALPHA, parent_id);
	}
	else if ("tattoo" == type)
	{
		LLUUID parent_id = self ? self->getUUID()
								: gInventory.findCategoryUUIDForType(LLFolderType::FT_CLOTHING);
		LLAgentWearables::createWearable(LLWearableType::WT_TATTOO, parent_id);
	}
	else if ("physics" == type)
	{
		LLUUID parent_id = self ? self->getUUID()
								: gInventory.findCategoryUUIDForType(LLFolderType::FT_CLOTHING);
		LLAgentWearables::createWearable(LLWearableType::WT_PHYSICS, parent_id);
	}
	else if ("shape" == type)
	{
		LLUUID parent_id = self ? self->getUUID()
								: gInventory.findCategoryUUIDForType(LLFolderType::FT_BODYPART);
		LLAgentWearables::createWearable(LLWearableType::WT_SHAPE, parent_id);
	}
	else if ("skin" == type)
	{
		LLUUID parent_id = self ? self->getUUID()
								: gInventory.findCategoryUUIDForType(LLFolderType::FT_BODYPART);
		LLAgentWearables::createWearable(LLWearableType::WT_SKIN, parent_id);
	}
	else if ("hair" == type)
	{
		LLUUID parent_id = self ? self->getUUID()
								: gInventory.findCategoryUUIDForType(LLFolderType::FT_BODYPART);
		LLAgentWearables::createWearable(LLWearableType::WT_HAIR, parent_id);
	}
	else if ("eyes" == type)
	{
		LLUUID parent_id = self ? self->getUUID()
								: gInventory.findCategoryUUIDForType(LLFolderType::FT_BODYPART);
		LLAgentWearables::createWearable(LLWearableType::WT_EYES, parent_id);
	}
...
class LLAttachObject : public inventory_panel_listener_t
{
	bool handleEvent(LLPointer<LLEvent> event, const LLSD& userdata)
	{
		...
		{
			LLAppearanceMgr::instance().rezAttachment(item, attachmentp);
		}
...
void init_inventory_actions(LLInventoryView* floater)
{
	...
	(new LLHideEmptySystemFolders())->registerListener(floater, "Inventory.HideEmptySystemFolders");
