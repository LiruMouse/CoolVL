...
namespace
{
	...
	// API.
	typedef boost::signals2::signal<void (const std::string& output)> console_reply_signal_t;
	...
	public:
		...
		void error(U32 status, const std::string& reason)
		{
			if (mOutput)
			{
				mOutput->appendText(UNABLE_TO_SEND_COMMAND + PROMPT, false, false);
			}
		}
		...
		void result(const LLSD& content)
		{
			if (mOutput)
			{
				mOutput->appendText(content.asString() + PROMPT, false, false);
			}
		}
...
: LLFloater(std::string("Region Debug Console"), std::string("FloaterRegionDebugConsoleRect"), LLStringUtil::null), mOutput(NULL)
{
	LLUICtrlFactory::getInstance()->buildFloater(this, "floater_region_debug_console.xml");
	sInstance = this;
	mReplySignalConnection = sConsoleReplySignal.connect(boost::bind(
			&LLFloaterRegionDebugConsole::onReplyReceived, this, _1));
}

LLFloaterRegionDebugConsole::~LLFloaterRegionDebugConsole()
{
	mReplySignalConnection.disconnect();
	gSavedSettings.setRect("FloaterRegionDebugConsoleRect", getRect());
	sInstance = NULL;
}
...
//static
void LLFloaterRegionDebugConsole::showInstance()
{
    if (!sInstance)
	{
...
void LLFloaterRegionDebugConsole::onInput(LLUICtrl* ctrl, void *userdata)
{
	...
	if (url.empty())
	{
		...
		else
		{
			...
			LLHTTPClient::post(url,	LLSD(input->getText()), new ConsoleResponder(self->mOutput));
		}
	}
	else
	{
		// Using SimConsoleAsync
		LLHTTPClient::post(url,	LLSD(input->getText()),	new AsyncConsoleResponder);
	}
...
LLHTTPRegistration<ConsoleResponseNode> gHTTPRegistrationMessageDebugConsoleResponse("/message/SimConsoleResponse");
