						 ...
						 const BOOL orthographic)
{
	...
	hud_render_text(wstr, pos_agent, font, style, x_offset, y_offset, color,
					orthographic);
}
					 ...
					 const BOOL orthographic)
{
	...
	if (wstr.empty() ||
		(!orthographic && dir_vec * camera->getAtAxis() <= 0.f))
	{
		return;
	}
	...
	gl_state_for_2d(gViewerWindow->getWindowDisplayWidth(),
					gViewerWindow->getWindowDisplayHeight());
	gViewerWindow->setupViewport();

	LLUI::loadIdentity();
	glLoadIdentity();
	LLUI::translate((F32)winX / LLFontGL::sScaleX,
					(F32)winY / LLFontGL::sScaleY,
					-((F32)winZ * 2.0f - 1.0f));
	F32 right_x;
