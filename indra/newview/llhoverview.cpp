...
void LLHoverView::updateHover(LLTool* current_tool)
{
	...
	mUseHover = !gAgent.cameraMouselook() && picking_tool && !mTyping;
	if (mUseHover)
	{
...
//MK
							if (gRRenabled &&
								gAgent.mRRInterface.mContainsShownames)
							{
...
//mk
				...
				if (object &&
					(object->usePhysics() || object->flagScripted() ||
					 object->flagHandleTouch() ||
					 (parent && parent->flagHandleTouch()) ||
					 object->flagTakesMoney() ||
					 (parent && parent->flagTakesMoney()) ||
					 object->flagAllowInventoryAdd() ||
					 object->flagTemporary() || object->flagPhantom()))
				{
					line.clear();
					...
					if (object->usePhysics() || object->flagHandleTouch() ||
						(parent && parent->flagHandleTouch()))
					{
						line = LLTrans::getString("TooltipFlagRightClickMenu");
						mText.push_back(line);
					}
				}
