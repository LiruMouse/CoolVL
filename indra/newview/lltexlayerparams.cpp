...
void LLTexLayerParamAlpha::setWeight(F32 weight, BOOL upload_bake)
{
	...
	if (cur_u8 != new_u8)
	{
		...
		// visual param.
		if ((mAvatar->getSex() & getSex()) && mAvatar->isSelf() && !mIsDummy)
		{
			if (isAgentAvatarValid() && !gAgentAvatarp->isUsingBakedTextures())
			{
				upload_bake = FALSE;
			}
			mAvatar->invalidateComposite(mTexLayer->getTexLayerSet(),
										 upload_bake);
...
BOOL LLTexLayerParamAlpha::getSkip() const
{
	...
	{
		F32 effective_weight = (avatar->getSex() & getSex()) ? mCurWeight
															 : getDefaultWeight();
	...
	if (type != LLWearableType::WT_INVALID &&
		!avatar->isWearingWearableType(type))
	{
		return TRUE;
	}
...
BOOL LLTexLayerParamAlpha::render(S32 x, S32 y, S32 width, S32 height)
{
	...
	F32 effective_weight = (mTexLayer->getTexLayerSet()->getAvatar()->getSex() & getSex()) ? mCurWeight
																						   : getDefaultWeight();
	...
	if (!info->mStaticImageFileName.empty() && !mStaticImageInvalid)
	{
		...
		{
//			llinfos << "Building Cached Alpha: " << mName << ": ("
//					<< mStaticImageRaw->getWidth() << ", "
//					<< mStaticImageRaw->getHeight() << ") "
//					<< effective_weight << llendl;
			mCachedEffectiveWeight = effective_weight;

			if (!mCachedProcessedTexture)
			{
				mCachedProcessedTexture = LLViewerTextureManager::getLocalTexture(image_tga_width,
																				  image_tga_height,
																				  1, FALSE);
...
LLColor4 LLTexLayerParamColor::getNetColor() const
{
	...
	F32 effective_weight = mAvatar && (mAvatar->getSex() & getSex()) ? mCurWeight
																	 : getDefaultWeight();
