...
void LLFloaterInspect::refresh()
{
		 ...
		 iter != end; ++iter)
	{
		...
		std::string time;
		timeToFormattedString(timestamp, format, time);
		...
		gCacheName->getFullName(obj->mPermissions->getLastOwner(),
								last_owner_name);
										...
										InventoryObjectList* inv, S32, void*)
{
	...
	if (id == mQueuedInventoryRequest)
	{
		...
		for (InventoryObjectList::const_iterator it = inv->begin(),
												 end = inv->end();
			 it != end; ++it)
		{
			type = (*it)->getType();
...
LLUUID LLFloaterInspect::getSelectedUUID()
{
	if (sInstance)
	{
		if (sInstance->mObjectList->getAllSelected().size() > 0)
		{
			...
			first_selected = sInstance->mObjectList->getFirstSelected();
			if (first_selected)
			{
...
void LLFloaterInspect::show(void* ignored)
{
	...
	sInstance->open();
	LLToolMgr::getInstance()->setTransientTool(LLToolCompInspect::getInstance());
	LLSelectMgr::getInstance()->setForceSelection(forcesel);	// restore previous value
...
{
	LLFloaterObjectWeights::show(sInstance);
}
...
//static
void LLFloaterInspect::onSelectObject(LLUICtrl*, void*)
{
	if (LLFloaterInspect::getSelectedUUID() != LLUUID::null)
	{
		sInstance->mButtonOwner->setEnabled(TRUE);
		sInstance->mButtonCreator->setEnabled(TRUE);
		sInstance->mButtonWeights->setEnabled(TRUE);
	}
}
