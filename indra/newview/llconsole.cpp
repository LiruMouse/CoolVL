...
{
	S32 new_width = llmax(MIN_CONSOLE_WIDTH, llmin(getRect().getWidth(),
						  gViewerWindow->getWindowWidth()));
	S32 line_height = mFont ? llfloor(mFont->getLineHeight()) : 0;
    S32 new_height = llclamp(line_height + 15, getRect().getHeight(),
...
void LLConsole::draw()
{
	...
	static LLCachedControl<F32> console_background_opacity(gSavedSettings,
														   "ConsoleBackgroundOpacity");
	F32 console_opacity = llclamp((F32)console_background_opacity, 0.f, 1.f);
	static LLCachedControl<LLColor4U> console_background(gColors,
														 "ConsoleBackground");
	...
	static LLCachedControl<bool> console_box_per_message(gSavedSettings,
														 "ConsoleBoxPerMessage");
	if (!console_box_per_message)
	{
		// This section makes a single huge black box behind all the text.
		S32 bkg_height = 4;
		static LLCachedControl<bool> disable_messages_spacing(gSavedSettings,
															  "DisableMessagesSpacing");
		...
		S32 bkg_width = 0;
		for (paragraph_t::reverse_iterator paragraph_it = mParagraphs.rbegin(),
										   rend = mParagraphs.rend();
			 paragraph_it != rend; ++paragraph_it)
		{
...
// Generate highlight color segments for this paragraph. Pass in default color
// of paragraph.
void LLConsole::Paragraph::makeParagraphColorSegments (const LLColor4& color)
{
	...
	highlight->parseFullLineHighlights(wstring_to_utf8str(mParagraphText),
									   &lcolor);
	paragraph_color_segments = highlight->parsePartialLineHighlights(wstring_to_utf8str(mParagraphText),
																	 lcolor);

	for (LLSD::array_const_iterator
			color_segment_it = paragraph_color_segments.beginArray(),
			end = paragraph_color_segments.endArray();
		color_segment_it != end; ++color_segment_it)
	{
...
void LLConsole::Paragraph::updateLines(F32 screen_width, LLFontGL* font,
									   bool force_resize)
{
	if (!force_resize && mMaxWidth >= 0.0f && mMaxWidth < screen_width)
	{
		return;			// No resize required.
	}

	if (mParagraphText.empty() || mParagraphColorSegments.empty() ||
		font == NULL)
	{
	...
	paragraph_color_segments_t::iterator end_segments = mParagraphColorSegments.end();
	...
	LLWString::size_type paragraph_text_length = mParagraphText.length();
	while (paragraph_offset < (S32)paragraph_text_length)
	{
		...
		U32 drawable = font->maxDrawableChars(mParagraphText.c_str() + paragraph_offset,
											  screen_width,
		...
		if (drawable != 0 || found_newline)
		{
			...
			{
				LLWString color_text = mParagraphText.substr(paragraph_offset + drawn,
