...
:	LLViewerDynamicTexture(width, height, 4, LLViewerDynamicTexture::ORDER_LAST, TRUE), 	// ORDER_LAST => must render these after the hints are created.
...
{
	llinfos << "Composite System GL Buffers: "
			<< LLTexLayerSetBuffer::sGLByteCount / 1024 << "KB" << llendl;
}
...
BOOL LLTexLayerSetBuffer::needsRender()
{
	...
	// Don't render if we are trying to create a shirt texture but aren't
	// wearing a skirt.
	...
	if (upload_now)
	{
		...
		else
		{
			...
			else
			{
				...
				mNeedsUpload = FALSE;
				mNeedsUploadTimer.pause();
				mTexLayerSet->getAvatar()->setNewBakedTexture(mTexLayerSet->getBakedTexIndex(),
															  IMG_INVISIBLE);
			}
		}
	}
...
BOOL LLTexLayerSetBuffer::isReadyToUpload() const
{
	...
	{
		// If we requested an upload and have the final LOD ready, upload
	...
	{
		// Upload if we've hit a timeout. Upload is a pretty expensive process
		// so we need to make sure we aren't doing uploads too frequently.
...
BOOL LLTexLayerSetBuffer::isReadyToUpdate() const
{
	...
	// If we haven't done an update yet, then just do one now regardless of
	// state of textures.
	if (mNumLowresUpdates == 0) return TRUE;

	// Update if we've hit a timeout. Unlike for uploads, we can make this
	// timeout fairly small since render unnecessarily doesn't cost much.
	...
	if (texture_timeout != 0)
	{
		...
		const BOOL is_update_textures_timeout = mNeedsUpdateTimer.getElapsedTimeF32() >= (F32)texture_timeout;
		const BOOL has_lower_lod = mTexLayerSet->isLocalTextureDataAvailable();
...
{
	llinfos << "Uploading baked " << mTexLayerSet->getBodyRegionName()
			<< llendl;
	LLViewerStats::getInstance()->incStat(LLViewerStats::ST_TEX_BAKES);

	// Don't need caches since we're baked now (note: we won't *really* be baked
	// until this image is sent to the server and the Avatar Appearance message
	// is received).
	...
	if (compressedImage->encode(baked_image, comment_text))
	{
							   ...
							   gVFS, asset_id, LLAssetType::AT_TEXTURE))
		{
			...
			S32 file_size = 0;
			U8* data = LLVFile::readFile(gVFS, asset_id,
										 LLAssetType::AT_TEXTURE, &file_size);
			if (data)
			{
			...
			if (valid)
			{
					...
					mUploadFailCount < BAKE_UPLOAD_ATTEMPTS - 1) // Try last ditch attempt via asset store if cap upload is failing.
				{
					...
					LLHTTPClient::post(url, body,
									   new LLSendTexLayerResponder(body,
																   mUploadID,
			...
			{
				// The read back and validate operation failed.
												  ...
												  LLExtStat ext_status) // StoreAssetData callback (not fixed)
{
		...
		baked_upload_data->mTexLayerSet->hasComposite())
	{
		...
		else if (baked_upload_data->mID == layerset_buffer->mUploadID)
		{
			...
			{
				++failures;
				S32 max_attempts = baked_upload_data->mIsHighestRes ? BAKE_UPLOAD_ATTEMPTS
...
BOOL LLTexLayerSetInfo::parseXml(LLXmlTreeNode* node)
{
	...
	// <layer>
	for (LLXmlTreeNode* child = node->getChildByName("layer"); child;
...
void LLTexLayerSetInfo::createVisualParams(LLVOAvatar* avatar)
{
	...
	for (layer_info_list_t::iterator layer_iter = mLayerInfoList.begin();
		 layer_iter != mLayerInfoList.end(); layer_iter++)
	{
...
BOOL LLTexLayerSet::setInfo(const LLTexLayerSetInfo* info)
{
	...
	for (LLTexLayerSetInfo::layer_info_list_t::const_iterator iter = info->mLayerInfoList.begin(); 
		 iter != info->mLayerInfoList.end(); iter++)
	{
		...
		// - make sure you add the parameters to the avatar
...
void LLTexLayerSet::gatherMorphMaskAlpha(U8* data, S32 width, S32 height)
{
	...
	{
		LLTexLayerInterface* layer = *iter;
		layer->gatherAlphaMasks(data, mComposite->getOriginX(),
								mComposite->getOriginY(), width, height);
	}

	// Set alpha back to that of our alpha masks.
	renderAlphaMaskTextures(mComposite->getOriginX(), mComposite->getOriginY(),
							width, height, true);
}

void LLTexLayerSet::renderAlphaMaskTextures(S32 x, S32 y, S32 width, S32 height,
										    bool forceClear)
{
	...
	// will still have an effect even if mClearAlpha is set or the alpha
								 ...
								 LLWearable* wearable)
{
	...
	if (mInfo != NULL) 
	{
		// Not a critical warning, but could be useful for debugging later
		// issues. -Nyx
	...
	for (param_color_info_list_t::const_iterator iter = mInfo->mParamColorInfoList.begin(); 
		 iter != mInfo->mParamColorInfoList.end(); iter++)
	{
...
LLViewerVisualParam* LLTexLayerInterface::getVisualParamPtr(S32 index) const
{
	...
	for (param_color_list_t::const_iterator color_iter = mParamColorList.begin();
		 color_iter != mParamColorList.end() && !result; ++color_iter)
	{
	...
	for (param_alpha_list_t::const_iterator alpha_iter = mParamAlphaList.begin();
		 alpha_iter != mParamAlphaList.end() && !result; ++alpha_iter)
	{
...
const U8* LLTexLayer::getAlphaData() const
{
	...
	return (iter2 == mAlphaCache.end()) ? NULL : iter2->second;
}

BOOL LLTexLayer::findNetColor(LLColor4* net_color) const
{
	// Color is either:
	//	* one or more color parameters (weighted colors)  (which may make use
	//    of a global color or fixed color)
	//	* a reference to a global color
	//	* a fixed color with non-zero alpha
	//	* opaque white (the default)
...
BOOL LLTexLayer::renderMorphMasks(S32 x, S32 y, S32 width, S32 height, const LLColor4 &layer_color)
{
	...
	if (!getInfo()->mStaticImageFileName.empty())
	{
		LLViewerTexture* tex = LLTexLayerStaticImageList::getInstance()->getTexture(getInfo()->mStaticImageFileName,
...
void LLTexLayerSet::cloneTemplates(LLLocalTextureObject* lto,
								   LLVOAvatarDefines::ETextureIndex tex_index,
								   LLWearable* wearable)
{
...
// Note: in general, for a given image image we'll call either getImageTga()
// or getTexture(). We call getImageTga() if the image is used as an alpha
...
// subsequent requests.
LLViewerTexture* LLTexLayerStaticImageList::getTexture(const std::string& file_name,
													   BOOL is_mask)
{
