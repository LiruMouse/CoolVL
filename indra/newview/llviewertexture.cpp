		...
		if (avail_virtual_mem_kb < main_memory_safety_margin * 1024)
		{
			...
			if (last_memory_check - last_draw_distance_reduce > 20.0f * discard_delta_time)
			{
				...
				else if (!warned)
				{
					...
					warned = true;	// Don't spam: warn only once and wait till the memory usage decreases
				}
			}
											   ...
											   BOOL pause)
{
	...
	LLLoadedCallbackEntry* entryp = new LLLoadedCallbackEntry(loaded_callback,
															  discard_level,
															  keep_imageraw,
															  userdata,
															  src_callback_list,
	...
	{
		// We need aux data, but we've already loaded the image, and it didn't
		// have any
		static LLUUID last_id;
		if (getID() != last_id)
		{
			last_id = getID();
			llwarns << "No aux data available for callback for image:"
					<< last_id << llendl;
		}
	}
...
bool LLViewerFetchedTexture::doLoadedCallbacks()
{
	...
	// If we don't have a legit GL image, set it to be lower than the worst
	// discard level
	if (gl_discard == -1)
	{
...
{
	// Don't stop the media impl playing here -- this breaks non-inworld media
	// (login screen, search, and media browser).
//	if(mMediaImplp)
//	{
//		mMediaImplp->stop();
//	}
