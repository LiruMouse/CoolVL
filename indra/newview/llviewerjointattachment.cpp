...
//MK
	bool was_empty = mAttachedObjects.size() == 0;
//mk
	...
	{
		gAgentAvatarp->setUseDeformer(false);
	...
	if (gRRenabled)
	{
		...
		if (was_empty)
		{
			...
			if (!gAgent.mRRInterface.canAttach(object, name))
			{
				...
				if (!just_reattaching)
				{
					llinfos << "Illegally attached to a locked point: "
							<< item_id << ", detaching." << llendl;
					...
					// Now notify that this object has been attached and will
					// be detached right away
