...
#include "llviewermenufile.h"		// for upload_new_resource()
...
BOOL LLFloaterAnimPreview::postBuild()
{
	...
	mInWorld = isAgentAvatarValid() && gSavedSettings.getBOOL("PreviewAnimInWorld");
		...
		loaderp->getDuration() <= MAX_ANIM_DURATION)
	{
		...
		if (success)
		{
			...
			childSetValue("loop_in_point",
						  LLSD(motionp->getLoopIn() / motionp->getDuration() * 100.f));
			childSetValue("loop_out_point",
						  LLSD(motionp->getLoopOut() / motionp->getDuration() * 100.f));
...
void LLFloaterAnimPreview::resetMotion()
{
	...
	if (mInWorld)
	{
		avatarp = isAgentAvatarValid() ? gAgentAvatarp.get() : NULL;
	}
	else
	{
...
void LLFloaterAnimPreview::onBtnPlay(void* user_data)
{
	...
	if (!previewp || !previewp->getEnabled())
	{
		return;
	}
...
void LLFloaterAnimPreview::onCommitEmote(LLUICtrl* ctrl, void* data)
{
	...
	if (previewp && previewp->getEnabled())
	{
		previewp->resetMotion(); // sets emote
	}
}
...
BOOL LLFloaterAnimPreview::validateEaseIn(LLUICtrl* spin, void* data)
{
	...
	if (!motionp->getLoop())
	{
		F32 new_ease_in = llclamp((F32)previewp->childGetValue("ease_in_time").asReal(),
								  0.f,
								  motionp->getDuration() - motionp->getEaseOutDuration());
...
BOOL LLFloaterAnimPreview::validateEaseOut(LLUICtrl* spin, void* data)
{
	...
	if (!motionp->getLoop())
	{
		F32 new_ease_out = llclamp((F32)previewp->childGetValue("ease_out_time").asReal(),
								   0.f,
								   motionp->getDuration() - motionp->getEaseInDuration());
...
void LLFloaterAnimPreview::onBtnOK(void* userdata)
{
	...
	if ((!floaterp->mInWorld && floaterp->mAnimPreview) ||
		(floaterp->mInWorld && isAgentAvatarValid()))
	{
		...
		{
			LLVFile file(gVFS, motionp->getID(), LLAssetType::AT_ANIMATION,
						 LLVFile::APPEND);
			...
			if (file.write((U8*)buffer, size))
			{
				...
				upload_new_resource(floaterp->mTransactionID, // tid
									LLAssetType::AT_ANIMATION, name, desc, 0,
									LLFolderType::FT_ANIMATION,
									LLInventoryType::IT_ANIMATION,
									LLFloaterPerms::getNextOwnerPerms(),
									LLFloaterPerms::getGroupPerms(),
									LLFloaterPerms::getEveryonePerms(),
									name, callback, expected_upload_cost,
									data);
			}
			else
			{
...
:	LLViewerDynamicTexture(width, height, 3, ORDER_MIDDLE, FALSE)
{
	...
	mDummyAvatar = (LLVOAvatar*)gObjectList.createObjectViewer(LL_PCODE_LEGACY_AVATAR,
															   gAgent.getRegion());
...
BOOL LLPreviewAnimation::render()
{
	...
	camera->setOriginAndLookAt(target_pos + ((LLVector3(mCameraDistance, 0.f, 0.f) + mCameraOffset) * av_rot),	// camera
							   LLVector3::z_axis,																// up
							   target_pos + (mCameraOffset * av_rot));											// point of interest

	camera->setView(camera->getDefaultFOV() / mCameraZoom);
	camera->setPerspective(FALSE, mOrigin.mX, mOrigin.mY, mFullWidth,
						   mFullHeight, FALSE);
