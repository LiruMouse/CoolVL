...
//-----------------------------------------------------------------------------
class LLBodyNoiseMotion : public LLMotion
{
public:
	// Constructor
...
//-----------------------------------------------------------------------------
class LLBreatheMotionRot : public LLMotion
{
public:
	// Constructor
...
//-----------------------------------------------------------------------------
class LLPelvisFixMotion : public LLMotion
{
public:
	// Constructor
	...
	mUseDeformer(false)
{
	...
	// Currently, this HUD effect doesn't need to pack and unpack data to do
	// its job
	const bool needsSendToSim = false;
	// mVoiceVisualizer is created by the hud effects manager and uses the HUD
	// Effects pipeline
	mVoiceVisualizer = (LLVoiceVisualizer*)LLHUDManager::getInstance()->createViewerEffect(LLHUDObject::LL_HUD_EFFECT_VOICE_VISUALIZER,
																						   needsSendToSim);

	LL_DEBUGS("Avatar") << "LLVOAvatar Constructor (0x" << this << ") id:"
						<< mID << LL_ENDL;
...
LLVOAvatar::~LLVOAvatar()
{
	...
	std::for_each(mAttachmentPoints.begin(), mAttachmentPoints.end(),
				  DeletePairedPointer());
...
void LLVOAvatar::dumpBakedStatus()
{
		 ...
		 iter != end; ++iter)
	{
		...
		else
		{
			...
			for (LLVOAvatarDictionary::BakedTextures::const_iterator
					iter = LLVOAvatarDictionary::getInstance()->getBakedTextures().begin(),
					end = LLVOAvatarDictionary::getInstance()->getBakedTextures().end();
				 iter != end; ++iter)
			{
				...
				{
					llcont << " "
						   << LLVOAvatarDictionary::getInstance()->getTexture(index)->mName;
				}
			}
...
{ 
	...
	if (!skeleton_node)
	{
		llerrs << "No skeleton in avatar configuration file: " << xmlFile
			   << llendl;
		return;
	}
...
void LLVOAvatar::initInstance(void)
{
	...
	for (LLVOAvatarDictionary::Meshes::const_iterator
			iter = LLVOAvatarDictionary::getInstance()->getMeshes().begin(),
			end = LLVOAvatarDictionary::getInstance()->getMeshes().end();
		 iter != end; ++iter)
	{
		 ...
		 iter != end; ++iter)
	{
		...
		for (std::vector<LLViewerJointMesh* >::iterator
				iter = mMeshLOD[mesh_index]->mMeshParts.begin(),
...
void LLVOAvatar::getSpatialExtents(LLVector4a& newMin, LLVector4a& newMax)
{
	...
	for (polymesh_map_t::iterator i = mMeshes.begin(), end = mMeshes.end();
		 i != end; ++i)
	{
		LLPolyMesh* mesh = i->second;
		for (S32 joint_num = 0, count = mesh->mJointRenderData.count();
		 ...
		 iter != end; ++iter)
	{
		...
		for (LLViewerJointAttachment::attachedobjs_vec_t::iterator
				...
				end2 = attachment->mAttachedObjects.end();
			 attachment_iter != end2; ++attachment_iter)
		{
			...
			if (attached_object && !attached_object->isHUDAttachment())
			{
				...
				if (drawable && !drawable->isState(LLDrawable::RIGGED))
				{
					...
					if (bridge)
					{
						...
						// Only add the prim to spatial extents calculations if
						// it isn't a megaprim.
						// max_attachment_span calculated at the start of the
						// function (currently 5 times our max prim size) 
						if (lt == 0x7)
						{
...
void LLVOAvatar::renderCollisionVolumes()
{
	...
	{
		LLVector3 unused;
		mNameText->lineSegmentIntersect(LLVector3(0, 0, 0), LLVector3(0, 0, 1),
										unused, TRUE);
	}
}
									  ...
									  LLVector3* bi_normal)
{
	...
	if (lineSegmentBoundingBox(start, end))
	{
		for (S32 i = 0; i < mNumCollisionVolumes; ++i)
		{
			...
			if (linesegment_sphere(LLVector3(p1.v), LLVector3(p2.v),
								   LLVector3(0, 0, 0), 1.f, position, norm))
			{
	...
	if (lineSegmentBoundingBox(start, end))
	{
			 ...
			 iter != end; ++iter)
		{
				 ...
				 attachment_iter != end2; ++attachment_iter)
			{
				...
				if (attached_object->lineSegmentIntersect(start, local_end,
														  face,
														  pick_transparent,
														  face_hit,
														  &local_intersection,
...
BOOL LLVOAvatar::parseSkeletonFile(const std::string& filename)
{
	...
	// parse the file
	//-------------------------------------------------------------------------
	if (!sSkeletonXMLTree.parseFile(filename, FALSE))
	{
...
// setupBone()
//-----------------------------------------------------------------------------
BOOL LLVOAvatar::setupBone(const LLVOAvatarBoneInfo* info,
						   ...
						   S32 &joint_num)
{
	...
	// setup children
	for (LLVOAvatarBoneInfo::child_list_t::const_iterator
...
BOOL LLVOAvatar::buildSkeleton(const LLVOAvatarSkeletonInfo* info)
{
	...
	if (info->mNumCollisionVolumes)
	{
		...
		{
			llerrs << "Can't allocate " << info->mNumCollisionVolumes
	...
	for (LLVOAvatarSkeletonInfo::bone_info_list_t::const_iterator
			iter = info->mBoneInfoList.begin(),
			end = info->mBoneInfoList.end();
		 iter != end; ++iter)
	{
...
void LLVOAvatar::buildCharacter()
{
	...
	for (std::vector<LLViewerJoint*>::iterator jointIter = mMeshLOD.begin(),
		 ...
		 jointIter != end; ++jointIter)
	{
		LLViewerJoint* joint = (LLViewerJoint*) *jointIter;
		for (std::vector<LLViewerJointMesh*>::iterator
				meshIter = joint->mMeshParts.begin(),
				end2 = joint->mMeshParts.end();
			 meshIter != end2; ++meshIter)
		{
...
void LLVOAvatar::updateMeshData()
{
	if (mDrawable.notNull())
	{
		...
		S32 f_num = 0;
		 // small number of this means each part of an avatar has its own vertex buffer.
		const U32 VERTEX_NUMBER_THRESHOLD = 128;
		const S32 num_parts = mMeshLOD.size();

		// this order is determined by number of LODS
		// if a mesh earlier in this list changed LODs while a later mesh
		// doesn't, the later mesh's index offset will be inaccurate
		for (S32 part_index = 0; part_index < num_parts; )
		{
			...
			while (part_index < num_parts && num_vertices < VERTEX_NUMBER_THRESHOLD)
			{
				...
				mMeshLOD[part_index++]->updateFaceSizes(num_vertices,
														num_indices,
														mAdjustedPixelArea);
			}
			...
			// the case of more than one avatar in the pool (thus > 0 instead
			// of >= 0)
			if (facep->getGeomIndex() > 0)
			{
				llerrs << "non-zero geom index: " << facep->getGeomIndex()
					   << " in LLVOAvatar::restoreMeshData" << llendl;
			}
			...
			for (S32 k = j; k < part_index; ++k)
			{
				...
				{	// eyeballs can't have terse updates since they're never
					// rendered with the hardware skinning shader
					rigid = true;
				}
				mMeshLOD[k]->updateFaceData(facep, mAdjustedPixelArea,
											k == MESH_ID_HAIR,
											terse_update && !rigid);
			}
		...
		if (isSelf())
		{
			...
			if (lastGesticulationLevel != mCurrentGesticulationLevel)
			{
				if (mCurrentGesticulationLevel != VOICE_GESTICULATION_LEVEL_OFF)
				{
					...
					{
						gestureString = "/voicelevel1";
					}
					else if (mCurrentGesticulationLevel == 1)
					{
						gestureString = "/voicelevel2";
					}
					else if (mCurrentGesticulationLevel == 2)
					{
						gestureString = "/voicelevel3";
					}
					else
					{
						llinfos << "oops - CurrentGesticulationLevel can be only 0, 1, or 2"
								<< llendl;
					}

					// this is the call that Karl S. created for triggering
					// gestures from within the code.
	...
	if (detailed_update || !sUseImpostors)
	{
			 ...
			 iter != end; ++iter)
		{
				 ...
				 attachment_iter != end2; ++attachment_iter)
			{
				...
				if (visibleAttachment && attached_object &&
					!attached_object->isDead() && attachment->getValid())
				{
					LLObjectSelectionHandle selection = LLSelectMgr::getInstance()->getSelection();
					// if selecting any attachments, update all of them as
					// non-damped
...
void LLVOAvatar::idleUpdateNameTag(const LLVector3& root_pos_last)
{
	...
	static LLCachedControl<LLColor4U> avatar_name_color(gColors, "AvatarNameColor");
...
//mk
	...
	{
		bool new_name = false;
		if (visible_chat != (bool)mVisibleChat)
		{
		...
		// If alpha > 0, create mNameText if necessary, otherwise delete it
		{
			...
			if (alpha > 0.f)
			{
				...
				LLColor4 name_color = LLColor4(avatar_name_color);
				...
				camera->getPixelVectors(root_pos_last, pixel_up_vec, pixel_right_vec);
				LLVector3 camera_to_av = root_pos_last - camera->getOrigin();
				camera_to_av.normalize();
				LLVector3 local_camera_at = camera_to_av * ~root_rot;
				LLVector3 local_camera_up = camera_to_av % camera->getLeftAxis();
		...
		if (mNameText.notNull() && firstname && lastname)
		{
			...
			AnimIterator end_signaled_anims = mSignaledAnimations.end();
			bool is_away = mSignaledAnimations.find(ANIM_AGENT_AWAY) != end_signaled_anims;
			bool is_busy = mSignaledAnimations.find(ANIM_AGENT_BUSY) != end_signaled_anims;
			bool is_appearance = mSignaledAnimations.find(ANIM_AGENT_CUSTOMIZE) != end_signaled_anims;
			...
			if (mNameString.empty() || new_name || complete_name != mCompleteName ||
				...
				(show_typing && !chat_muted && (bool)mTyping != mNameTyping))
			{
				std::string line;
			...
			if (visible_chat)
			{
				...
				LLColor4 new_chat = LLColor4(avatar_name_color);
				...
				{
					F32 chat_fade_amt = llclamp((F32)((LLFrameTimer::getElapsedSeconds() - chat_iter->mTime) / CHAT_FADE_TIME),
												0.f, 4.f);
...
{
	for (std::vector<LLCharacter*>::iterator
			it = LLCharacter::sInstances.begin(),
			end =  LLCharacter::sInstances.end();
		 it != end; ++it)
	{
...
BOOL LLVOAvatar::updateCharacter(LLAgent &agent)
{
	...
	if (LLVOAvatar::sShowAnimationDebug)
	{
			 ...
			 iter != end; ++iter)
		{
			...
			if (motionp->getMinPixelArea() < getPixelArea())
			{
				...
				{
					output = llformat("%s - %d", motionp->getID().asString().c_str(),
	...
	if (visible && !isSelf() && !mIsDummy && sUseImpostors &&
		!mNeedsAnimUpdate && !sFreezeCounter)
	{
		...
		{	// max visible avatars are not impostored
			// also, don't impostor avatars whose bounding box may be
			// penetrating the impostor camera near clip plane
			mUpdatePeriod = 1;
		}
	...
	// don't early out for your own avatar, as we rely on your animations
...
//		llinfos << "Setting timestep to " << time_quantum * pixel_area_scale << llendl;
	...
	else if (!getParent() && mIsSitting &&
			 !isMotionActive(ANIM_AGENT_SIT_GROUND_CONSTRAINED))
	{
		getOffObject();
	}
	...
	if (!(mIsSitting && getParent()))
	{
									...
									NUM_AGENT_NO_ROTATE_ANIMS))
		{
			...
			if (isSelf())
			{
				primDir = agent.getAtAxis() - projected_vec(agent.getAtAxis(),
															agent.getReferenceUpVector());
				primDir.normalize();
			}
			else
			{
			...
			F32 pelvis_rot_threshold_slow = PELVIS_ROT_THRESHOLD_SLOW;
			if (isSelf())
			{
				static LLCachedControl<bool> use_in_mouse_look(gSavedSettings, "MouseLookUseRotDeviation");
				static LLCachedControl<U32> max_rot_deviation(gSavedSettings, "CameraToPelvisRotDeviation");
				if (use_in_mouse_look || !self_in_mouselook)
				{
					pelvis_rot_threshold_slow = llclamp((F32)max_rot_deviation,
														PELVIS_ROT_THRESHOLD_FAST,
														PELVIS_ROT_THRESHOLD_SLOW);
				}
			}
			LLVector3 pelvisDir(mRoot.getWorldMatrix().getFwdRow4().mV);
			F32 pelvis_rot_threshold = clamp_rescale(speed, 0.1f, 1.0f,
													 pelvis_rot_threshold_slow,
													 PELVIS_ROT_THRESHOLD_FAST);
			...
			if (root_roll < 1.f * DEG_TO_RAD && root_pitch < 5.f * DEG_TO_RAD)
			{
				...
				LLVector3 correction_vector = (pelvisDir - fwdDir) *
											  clamp_rescale(angle,
															pelvis_rot_threshold * 0.75f,
															pelvis_rot_threshold,
															1.0f, 0.0f);
	...
	//-------------------------------------------------------------------------
	static const LLUUID AGENT_FOOTSTEP_ANIMS[] = { ANIM_AGENT_WALK, ANIM_AGENT_RUN, ANIM_AGENT_LAND };
	static const S32 NUM_AGENT_FOOTSTEP_ANIMS = LL_ARRAY_SIZE(AGENT_FOOTSTEP_ANIMS);
...
//							AUDIO_STEP_LO_GAIN, AUDIO_STEP_HI_GAIN);
			...
			{
				gAudiop->triggerSound(step_sound_id, getID(), STEP_VOLUME,
				 ...
				 iter != end; ++iter)
			{
					 ...
					 attachment_iter != end2; ++attachment_iter)
				{
					if (LLViewerObject* attached_object = *attachment_iter)
					{
						...
						{
							llinfos << attachment->getName() << " visible"
									<< llendl;
						}
						else
						{
							llinfos << attachment->getName()
									...
									<< " and radius " << mDrawable->getRadius()
									<< llendl;
						}
					}
				}
...
{
	const bool should_alpha_mask = mSupportsAlphaLayers &&
								    // Don't alpha mask if "Highlight Transparent" checked
...
U32 LLVOAvatar::renderSkinned(EAvatarRenderPass pass)
{
	...
	if (sShowFootPlane && mDrawable.notNull())
	{
		...
		gGL.begin(LLRender::LINES);
		{
			...
			gGL.vertex3f(collide_point.mV[VX] - SQUARE_SIZE,
						 collide_point.mV[VY] - SQUARE_SIZE,
						 collide_point.mV[VZ]);
			gGL.vertex3f(collide_point.mV[VX] + SQUARE_SIZE,
						 collide_point.mV[VY] - SQUARE_SIZE,
						 collide_point.mV[VZ]);
			...
			gGL.vertex3f(collide_point.mV[VX],
						 collide_point.mV[VY],
						 collide_point.mV[VZ]);
			gGL.vertex3f(collide_point.mV[VX] + mFootPlane.mV[VX],
						 collide_point.mV[VY] + mFootPlane.mV[VY],
						 collide_point.mV[VZ] + mFootPlane.mV[VZ]);
		}
		gGL.end();
		gGL.flush();
	}
	...
	if (pass == AVATAR_RENDER_PASS_SINGLE)
	{
		...
		if (!LLDrawPoolAvatar::sSkipOpaque)
		{
			...
			{
				num_indices += mMeshLOD[MESH_ID_UPPER_BODY]->render(mAdjustedPixelArea,
																	first_pass, mIsDummy);
				first_pass = FALSE;
			}
			...
			{
				num_indices += mMeshLOD[MESH_ID_LOWER_BODY]->render(mAdjustedPixelArea,
																	first_pass, mIsDummy);
				first_pass = FALSE;
			}
		}
...
U32 LLVOAvatar::renderTransparent(BOOL first_pass)
{
	...
	if (!isSelf() || gAgent.needsRenderHead() || LLPipeline::sShadowRender)
	{
		...
		// Can't test for baked hair being defined, since that won't always be
...
void LLVOAvatar::updateTextures()
{
		 ...
		 ++texture_index)
	{
		...
		for (U32 wearable_index = 0; wearable_index < num_wearables;
			 ++wearable_index)
		{
			imagep = LLViewerTextureManager::staticCastToFetchedTexture(getImage(texture_index,
																				 wearable_index),
																		TRUE);
			if (imagep)
			{
		...
		if (isIndexBakedTexture((ETextureIndex) texture_index) && render_avatar)
		{
			...
			imagep = LLViewerTextureManager::staticCastToFetchedTexture(getImage(texture_index,0),
																		TRUE);
...
void LLVOAvatar::checkTextureLoading()
{
	...
	for	(LLLoadedCallbackEntry::source_callback_list_t::iterator
			iter = mCallbackTextureList.begin(),
...
void LLVOAvatar::addBakedTextureStats(LLViewerFetchedTexture* imagep,
									  ...
									  S32 boost_level)
{
	// Note: if this function is not called for the last
	// MAX_TEXTURE_VIRTURE_SIZE_RESET_INTERVAL frames, the texture pipeline
	// will stop fetching this texture.
...
{
	LLViewerObject* obj;
	LLWorld::getInstance()->resolveStepHeightGlobal(this, start_pt, end_pt,
													out_pos, out_norm, &obj);
}
									 ...
									 LLVector3 &outNorm)
{
	...
	LLViewerObject* obj;
	LLWorld::getInstance()->resolveStepHeightGlobal(this, p0, p1, outPos,
													outNorm, &obj);
	if (!obj)
	{
		mStepOnLand = TRUE;
...
void LLVOAvatar::processAnimationStateChanges()
{
	...
	for (AnimIterator anim_it = mPlayingAnimations.begin(),
	...
	for (AnimIterator anim_it = mSignaledAnimations.begin();
		 anim_it != end_signaled_anims; ++anim_it)
	{
		AnimIterator found_anim = mPlayingAnimations.find(anim_it->first);

		// signaled but not playing, or different sequence id, start motion
		if (found_anim == mPlayingAnimations.end() ||
			found_anim->second != anim_it->second)
		{
	...
	if (isSelf())
	{
		for (AnimSourceIterator source_it = mAnimationSources.begin(),
								end = mAnimationSources.end();
			 source_it != end; )
		{
			if (mSignaledAnimations.find(source_it->second) == end_signaled_anims)
			{
...
BOOL LLVOAvatar::processSingleAnimationStateChange(const LLUUID& anim_id, BOOL start)
{
	...
	if (start) // start animation
	{
		...
		{
			if (gAudiop && gSavedSettings.getBOOL("PlayTypingSound"))
			{
...
#if 0				// RN: anable this to play on typing sound at fixed volume
					// once sound engine is fixed to support both spatialized
					// and non-spatialized instances of the same sound
					if (isSelf())
					{
						gAudiop->triggerSound(sound_id, 1.0f,
											  LLAudioEngine::AUDIO_TYPE_UI);
					}
					else
#endif
					{
						gAudiop->triggerSound(sound_id, getID(), 1.0f,
...
void LLVOAvatar::getGround(const LLVector3 &in_pos_agent, LLVector3 &out_pos_agent, LLVector3 &outNorm)
{
	...
	LLWorld::getInstance()->resolveStepHeightGlobal(this, p0_global, p1_global,
													out_pos_global, outNorm,
													&obj);
	out_pos_agent = gAgent.getPosAgentFromGlobal(out_pos_global);
}
...
{
	if ((S32)num >= mNumJoints || (S32)num < 0)
	{
		return NULL;
	}
...
// 	LLFastTimer t(LLFastTimer::FTM_LOAD_AVATAR);
	...
	for (LLVOAvatarXmlInfo::morph_info_list_t::iterator
	...
	for (LLVOAvatarXmlInfo::driver_info_list_t::iterator
		 ...
		 iter != end; ++iter)
	{
		...
		{
			delete driver_param;
			llwarns << "avatar file: driver_param->parseData() failed"
					<< llendl;
			return FALSE;
		}
	}
...
BOOL LLVOAvatar::loadSkeletonNode()
{
	...
	{
		for (LLVOAvatarXmlInfo::skeletal_distortion_info_list_t::iterator
	...
	// ATTACHMENTS
	{
		for (LLVOAvatarXmlInfo::attachment_info_list_t::iterator
			 ...
			 iter != end; ++iter)
		{
			...
			{
				llwarns << "Attachment point out of range [1-255]: "
						<< attachmentID << " on attachment point "
			...
			if (!parentJoint)
			{
				llwarns << "No parent joint by name " << info->mJointName
						<< " found for attachment point " << info->mName
						<< ", skipping." << llendl;
				continue;
			}

			LLViewerJointAttachment* attachment = get_if_there(mAttachmentPoints,
													   		   attachmentID,
															   (LLViewerJointAttachment*)NULL);
			if (attachment)
			{
				llwarns << "Attachment point redefined with id "
						<< attachmentID << " on attachment point "
						<< info->mName << llendl;
			}
			else
			...
			if (group >= 0)
			{
				if (group < 0 || group >= 9)
				{
					llwarns << "Invalid group number (" << group
...
{
	for (LLVOAvatarXmlInfo::mesh_info_list_t::const_iterator
			meshinfo_iter = sAvatarXmlInfo->mMeshInfoList.begin(),
			end = sAvatarXmlInfo->mMeshInfoList.end();
		 meshinfo_iter != end; ++meshinfo_iter)
	{
		...
		/* if (type == "hairMesh")
			switch (lod)
			  case 0:
				mesh = &mHairMesh0; */
		for (LLVOAvatarDictionary::Meshes::const_iterator
				mesh_iter = LLVOAvatarDictionary::getInstance()->getMeshes().begin(),
				end2 = LLVOAvatarDictionary::getInstance()->getMeshes().end();
			 mesh_iter != end2; ++mesh_iter)
		{
		...
		if (found_mesh_id)
		{
			...
			{
				llwarns << "Avatar file: <mesh> has invalid lod setting "
						<< lod << llendl;
				return FALSE;
			}
		}
		...
		// If this isn't set to white (1.0), avatars will *ALWAYS* be darker
		...
		for (LLVOAvatarXmlInfo::LLVOAvatarMeshInfo::morph_info_list_t::const_iterator
				xmlinfo_iter = info->mPolyMorphTargetInfoList.begin(),
				end2 = info->mPolyMorphTargetInfoList.end();
			 xmlinfo_iter != end2; ++xmlinfo_iter)
		{
...
{
	BOOL success = TRUE;
	for (LLVOAvatarXmlInfo::layer_info_list_t::const_iterator
			layerset_iter = sAvatarXmlInfo->mLayerInfoList.begin(),
			end = sAvatarXmlInfo->mLayerInfoList.end();
		 layerset_iter != end; ++layerset_iter)
	{
		// Construct a layerset for each one specified in avatar_lad.xml and
...
void LLVOAvatar::updateVisualParams()
{
	...
	LL_DEBUGS("Avatar") << "LLVOAvatar::updateVisualParams() called for avatar: "
						<< getID() << LL_ENDL;
	...
	// Note: rebuilding rigged mesh attachments can cause a "jumping" effect
	// for attachments which origin is not 0, 0, 0. Let's only do it when
	// needed (currently only by the mesh deformer code).
	static LLCachedControl<bool> mesh_enable_deformer(gSavedSettings,
													  "MeshEnableDeformer");
	if (mesh_enable_deformer && mUseDeformer)
	{
...
BOOL LLVOAvatar::updateJointLODs()
{
...

	{
		if (isSelf())
		{
			if (gFloaterCustomize || gAgent.cameraMouselook())
			{
...
BOOL LLVOAvatar::updateGeometry(LLDrawable* drawable)
{
	...
	if (!drawable)
	{
		llerrs << "LLVOAvatar::updateGeometry() called with NULL drawable"
			   << llendl;
	}
...
LLViewerJointAttachment* LLVOAvatar::getTargetAttachmentPoint(LLViewerObject* viewer_object)
{
	...
	if (!attachment)
	{
		...
		{
			attachment = get_if_there(mAttachmentPoints, 1,
									  (LLViewerJointAttachment*)NULL); // Arbitrary using 1 (chest)
		}
	}
...
{
	LL_DEBUGS("Avatar") << "LLVOAvatar::rebuildRiggedAttachments() called for avatar: "
						<< getID() << LL_ENDL;
	for (attachment_map_t::iterator iter = mAttachmentPoints.begin(),
									end = mAttachmentPoints.end();
		 iter != end; ++iter)
	{
		...
		for (LLViewerJointAttachment::attachedobjs_vec_t::iterator
				attachmentIter = pAttachment->mAttachedObjects.begin(),
				attachmentIterEnd = pAttachment->mAttachedObjects.end();
		 ...
		 iter != end; ++iter)
	{
		...
		if (attachment->isObjectAttached(viewer_object))
		{
			...
			LL_DEBUGS("Attachment") << "Detaching object "
									<< viewer_object->mID << " from "
									<< attachment->getName() << LL_ENDL;
			return TRUE;
		}
	}
									  ...
									  BOOL upload_bake)
{
	...
	else if (global_color == mTexHairColor)
	{
		...
		if (!isTextureDefined(mBakedTextureDatas[BAKED_HAIR].mTextureIndex))
		{
			...
			{
				mBakedTextureDatas[BAKED_HAIR].mMeshes[i]->setColor(color.mV[VX],
																	color.mV[VY],
																	color.mV[VZ],
																	color.mV[VW]);
			}
...
BOOL LLVOAvatar::processFullyLoadedChange(bool loading)
{
	...
	BOOL changed = (mFullyLoaded != mPreviousFullyLoaded ||			// if the value is different from the previous call
...
void LLVOAvatar::updateMeshTextures()
{
	...
	for (U32 i = 0, count = getNumTEs(); i < count; ++i)
	{
			...
			te_image->getID() == IMG_DEFAULT)
		{
			...
			setImage(i, LLViewerTextureManager::getFetchedTexture(i == TEX_HAIR ? IMG_DEFAULT
																				: IMG_DEFAULT_AVATAR),
					 0);
		}
	}

	const BOOL self_customizing = isSelf() && gFloaterCustomize; // During face edit mode, we don't use baked textures
	...
	if (!target_host.isOk())
	{
		llwarns << "updateMeshTextures: invalid host for object: " << getID()
				<< llendl;
	}

	for (U32 i = 0; i < mBakedTextureDatas.size(); ++i)
	{
		if (!self_customizing && use_lkg_baked_layer[i])
		{
		...
		else if (!self_customizing && is_layer_baked[i])
		{
			...
			{
				// Even though the file may not be finished loading, we'll
				// consider it loaded and use it (rather than doing compositing).
		...
		else if (mBakedTextureDatas[i].mTexLayerSet && !other_culled)
		{
	...
	// This can happen while loading hair for yourself, or for clients that did
	// not bake a hair texture.
	// Still needed for yourself after 1.22 is deprecated.
	...
	{
		for (LLVOAvatarDictionary::BakedTextures::const_iterator
				baked_iter = LLVOAvatarDictionary::getInstance()->getBakedTextures().begin(),
				end = LLVOAvatarDictionary::getInstance()->getBakedTextures().end();
			 baked_iter != end; ++baked_iter)
		{
				 ...
				 local_tex_iter != end2; ++local_tex_iter)
			{
				...
				const BOOL is_baked_ready = (is_layer_baked[baked_index] &&
											 mBakedTextureDatas[baked_index].mIsLoaded);
...
//-----------------------------------------------------------------------------
// virtual
void LLVOAvatar::setLocalTexture(ETextureIndex type,
								 LLViewerTexture* in_tex,
								 BOOL baked_version_ready,
								 U32 index)
{
...
//virtual 
void LLVOAvatar::setBakedReady(LLVOAvatarDefines::ETextureIndex type,
							  BOOL baked_version_exists,
							  U32 index)
{
	// invalid for anyone but self
	llassert(0);
...
void LLVOAvatar::addChat(const LLChat& chat)
{
	...
	while ((chat_length > MAX_BUBBLE_CHAT_LENGTH ||
			mChats.size() > MAX_BUBBLE_CHAT_UTTERANCES) &&
...
void LLVOAvatar::addMaskedMorph(EBakedTextureIndex index,
...
void LLVOAvatar::applyMorphMask(U8* tex_data,
								S32 width,
								S32 height,
								S32 num_components,
								LLVOAvatarDefines::EBakedTextureIndex index)
{
	if (index >= BAKED_NUM_INDICES)
	{
		 ...
		 iter != end; ++iter)
	{
		...
		maskedMorph->mMorphTarget->applyMask(tex_data, width, height,
											 num_components,
											 maskedMorph->mInvert);
	}
}
...
{
	// ! BACKWARDS COMPATIBILITY !
	// Detect if the baked hair texture actually wasn't sent, and if so set to
	// default
	if (isTextureDefined(TEX_HAIR_BAKED) &&
	...
	{
		const LLVOAvatarDictionary::BakedEntry* bakedDicEntry;
		bakedDicEntry = LLVOAvatarDictionary::getInstance()->getBakedTexture((EBakedTextureIndex)baked_index);
		// skip if this is a skirt and av is not wearing one, or if we don't
		...
		for (U8 texture = 0; texture < bakedDicEntry->mLocalTextures.size();
			 ++texture)
		{
...
BOOL LLVOAvatar::isWearingWearableType(LLWearableType::EType type) const
{
	...
	for (LLVOAvatarDictionary::Textures::const_iterator
			tex_iter = LLVOAvatarDictionary::getInstance()->getTextures().begin(),
			end = LLVOAvatarDictionary::getInstance()->getTextures().end();
		 tex_iter != end; ++tex_iter)
	{
		const LLVOAvatarDictionary::TextureEntry* texture_dict = tex_iter->second;
		if (texture_dict->mWearableType == type)
		{
			// You are checking another avatar's clothing and you don't have
		 ...
		 iter != end; ++iter)
	{
		...
		if (attachment->getIsHUDAttachment())
		{
				 ...
				 attachment_iter != end2; ++attachment_iter)
			{
				...
				for (LLViewerObject::child_list_t::const_iterator
						iter = child_list.begin(), end3 = child_list.end();
					 iter != end3; ++iter)
				{
	     ...
	     param = getNextVisualParam())
	{
		if (param->isTweakable())
		{
			...
			if (param->getWeight() != param->getDefaultWeight() &&
			    // we have to not care whether skirt weights are default, if
				// we're not actually wearing a skirt
			    (is_wearing_skirt || !is_skirt_param))
			{
				//llinfos << "param '" << param->getName() << "'="
				//		 << param->getWeight() << " which differs from default="
				//		 << param->getDefaultWeight() << llendl;
				rtn = false;
				break;
			}
		}
...
//	dumpAvatarTEs("POST processAvatarAppearance()");
	...
	{
		llwarns << "AvatarAppearance msg received without any parameters, object: "
				<< getID() << llendl;
		...
		if (visualParamWeightsAreDefault())
		{
			if (sendAvatarTexturesRequest())
			{
				...
				// shape next time
				llinfos << "Re-requested AvatarAppearance for object: "
						<< getID() << llendl;
			}
		}
		else
	...
	// If all of the avatars are completely baked, release the global image
	// caches to conserve memory.
										   ...
										   void* userdata)
{
							...
							discard_level < maskData->mLastDiscardLevel - 2))
	{
		if (aux_src && aux_src->getComponents() == 1)
		{
			if (!aux_src->getData())
			{
				llerrs << "No auxiliary source data for onBakedTextureMasksLoaded"
					   << llendl;
				return;
			}
			...
			for (LLVOAvatarDictionary::Textures::const_iterator
					iter = LLVOAvatarDictionary::getInstance()->getTextures().begin(),
					end = LLVOAvatarDictionary::getInstance()->getTextures().end();
				 iter != end; ++iter)
			{
				const LLVOAvatarDictionary::TextureEntry* texture_dict = iter->second;
			...
			if (!found_texture_id)
			{
				...
				{
					last_id = id;
					llinfos << "onBakedTextureMasksLoaded(): unexpected image id: "
							<< id << llendl;
				}
			}
			self->dirtyMesh();
	...
	for (U32 i = 0, count = mBakedTextureDatas.size(); i < count; ++i)
	{
		...
		if (id == image_baked->getID())
		{
			...
			if (isSelf())
			{
				...
				for (texture_vec_t::const_iterator
						local_tex_iter = baked_dict->mLocalTextures.begin(),
						end = baked_dict->mLocalTextures.end();
					 local_tex_iter != end; ++local_tex_iter)
				{
			...
			// ! BACKWARDS COMPATIBILITY !
			// Workaround for viewing avatars from old viewers that haven't
			// baked hair textures. This is paired with similar code in
			// updateMeshTextures that sets hair mesh color.
			if (i == BAKED_HAIR)
			{
				for (U32 i = 0, count2 = mBakedTextureDatas[BAKED_HAIR].mMeshes.size();
					 i < count2; ++i)
				{
...
void LLVOAvatar::cullAvatarsByPixelArea()
{
		 ...
		 iter != end; ++iter)
	{
		...
		{
			inst->mCulled = culled;
			LL_DEBUGS("Avatar") << "avatar " << inst->getID()
								<< (culled ? " start culled" : " start not culled")
								<< LL_ENDL;
...
{
	//LLViewerObject::updateRegion(regionp);	// no operation
}
...
std::string LLVOAvatar::getFullname() const
{
	...
	if (first && last)
	{
		...
		if (!last_name.empty() &&
			!(LLAvatarName::sOmitResidentAsLastName && last_name == "Resident"))
		{
			name += " ";
			name += last_name;
		}
...
void LLVOAvatar::updateImpostors() 
{
		 ...
		 iter != end; ++iter)
	{
		...
		if (!avatar->isDead() && avatar->needsImpostorUpdate() &&
			avatar->isVisible() && avatar->isImpostor())
		{
								   ...
								   F32& distance) const
{
	...
	LLViewerCamera* camera = LLViewerCamera::getInstance();
	LLVector3 at = camera->getOrigin() - (getRenderPosition() + mImpostorOffset);
	distance = at.normalize();
	F32 da = 1.f - (at * camera->getAtAxis());
	angle.mV[0] = camera->getYaw() * da;
	angle.mV[1] = camera->getPitch() * da;
	angle.mV[2] = da;
}
				...
				if (drawable)
				{
					...
					if (volume)
					{
						...
						for (LLVOVolume::texture_cost_t::iterator
								iter = textures.begin(), end3 = textures.end();
							 iter != end3; ++iter)
						{
...
// virtual
BOOL LLVOAvatar::isTextureDefined(LLVOAvatarDefines::ETextureIndex te,
								  U32 index) const
{
	...
	const LLUUID id = getImage(te, index)->getID();
	return (id != IMG_DEFAULT_AVATAR && id != IMG_DEFAULT);
}

// virtual
BOOL LLVOAvatar::isTextureVisible(LLVOAvatarDefines::ETextureIndex type,
								  U32 index) const
{
