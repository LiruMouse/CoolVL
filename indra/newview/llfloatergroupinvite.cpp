										...
										LLView* parent)
{
	...
	LLFloaterGroupInvite* fgi = get_if_there(impl::sInstances, group_id,
											 (LLFloaterGroupInvite*)NULL);
	if (!fgi)
	{
		fgi = new LLFloaterGroupInvite("groupinfo", FGI_RECT, FLOATER_TITLE,
									   group_id);
		...
		{
			gFloaterView->getParentFloater(parent)->addDependentFloater(fgi);
		}
