...
void LLStatGraph::draw()
{
	...
	if (mUpdateTimer.getElapsedTimeF32() > 0.5f)
	{
		...
		tmp_str = llformat(format_str.c_str(), mLabel.c_str(),
						   mValue, mUnits.c_str());
		setToolTip(tmp_str);
	...
	static LLCachedControl<LLColor4U> menu_default_bg_color(gColors,
															"MenuDefaultBgColor");
	LLColor4 color = LLColor4(menu_default_bg_color);
	...
	gGL.color4fv(color.mV);
	gl_rect_2d(1, llround(frac * getRect().getHeight()),
