...
void LLSDMessage::EventResponder::result(const LLSD& data)
{
	...
	{
		llinfos << "'" << mMessage << "' to '" << mTarget << "' succeeded"
				<< llendl;
	}
}

void LLSDMessage::EventResponder::errorWithContent(U32 status,
...
bool LLSDMessage::ResponderAdapter::listener(const LLSD& payload, bool success)
{
	...
	{
		mResponder->errorWithContent(payload["status"].asInteger(),
