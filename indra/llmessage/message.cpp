...
{
	virtual bool validate(const std::string& name, LLSD& context) const	{ return true; }
	...
	mLastMessageFromTrustedMessageService(false)
{
	...
	// default to blocking trusted connections on a public interface if one is
	// specified
...
LLMessageSystem::~LLMessageSystem()
{
	...
	for_each(mMessageNumbers.begin(), mMessageNumbers.end(),
			 DeletePairedPointer());
...
BOOL LLMessageSystem::poll(F32 seconds)
{
	S32 num_socks;
	apr_status_t status;
	status = apr_poll(&(mPollInfop->mPollFD), 1, &num_socks,
...
{
	message_template_name_map_t::const_iterator iter = findTemplate(mMessageTemplates,
																	name);
...
BOOL LLMessageSystem::checkMessages(S64 frame_count)
{
	...
	do
	{
		...
		else
		{
			...
			if (buffer[0] & LL_RESENT_FLAG)
			{
				...
				if (cdp && cdp->isDuplicateResend(mCurrentRecvPacketID))
				{
					...
					if (mVerboseLog)
					{
						...
						str << "MSG: <- " << host;
						std::string tbuf;
						tbuf = llformat("\t%6d\t%6d\t%6d ", receive_size,
			...
			// UseCircuitCode can be a valid, off-circuit packet. But we don't
			...
			if (valid_packet)
			{
				logValidMsg(cdp, host, recv_reliable, recv_resent,
							(BOOL)(acks > 0));
				valid_packet = mTemplateMessageReader->readMessage(buffer,
																   host);
			}

			// It's possible that the circuit went away, because ANY message
...
// blow away the last block of a message, return FALSE if that leaves no blocks
// or there wasn't a block to remove.
	...
	if (ping_based_timeout)
	{
	    ...
	    if (cdp)
	    {
		    timeout = llmax(LL_MINIMUM_RELIABLE_TIMEOUT_SECONDS,
							LL_RELIABLE_TIMEOUT_FACTOR * cdp->getPingDelayAveraged());
	    }
	    else
	    {
		    timeout = llmax(LL_MINIMUM_RELIABLE_TIMEOUT_SECONDS,
...
LLHTTPClient::ResponderPtr LLMessageSystem::createResponder(const std::string& name)
{
	if (mSendReliable)
	{
		return new LLFnPtrResponder(mReliablePacketParams.mCallback,
									mReliablePacketParams.mCallbackData, name);
	}
	else
	{
		...
		//		<< llendl;
		return new LLFnPtrResponder(NULL, NULL, name);
	}
}
...
S32 LLMessageSystem::sendMessage(const LLHost& host)
{
	...
	// if port and ip are zero, don't bother trying to send the message
	if (!host.isOk())
	{
		return 0;
	}
	...
	if (!cdp)
	{
		...
		if (mbProtected)
		{
			...
			if (mVerboseLog)
			{
				LL_INFOS_ONCE("Messaging") << "MSG: -> " << host
										   << "\tUNKNOWN CIRCUIT:\t"
	...
	*((S32*)&mSendBuffer[PHL_PACKET_ID]) = htonl(cdp->getPacketOutID());
	...
	BOOL success;
	success = mPacketRing.sendPacket(mSocket, (char*)buf_ptr, buffer_length,
									 host);
											   ...
											   BOOL recv_reliable)
{
	if (mVerboseLog)
	{
		...
		str << "MSG: <- " << host;
		std::string buffer;
		buffer = llformat("\t%6d\t%6d\t%6d ", mMessageReader->getMessageSize(),
						  (mIncomingCompressedSize ? mIncomingCompressedSize
	...
	// nope!
	// cout << "Rejecting unexpected message " << mCurrentMessageTemplate->mName
	//		<< " from " << hex << ip << " , " << dec << port << endl;
...
void LLMessageSystem::disableCircuit(const LLHost& host)
{
	...
	// Don't need to do this, as we're removing the circuit info anyway
	// - djs 01/28/03

	// Don't clean up 0 circuit code entries because many hosts (neighbor sims,
	// etc) can have the 0 circuit
	if (code)
	{
	...
	else
	{
		...
		llinfos << "Couldn't find circuit code for " << host << ", ignoring..."
				<< llendl;
	}
}
...
BOOL LLMessageSystem::checkCircuitBlocked(const U32 circuit)
{
	...
	if (!host.isOk())
	{
		LL_DEBUGS("Messaging") << "checkCircuitBlocked: Unknown circuit "
							   << circuit << LL_ENDL;
		return TRUE;
	}
	...
	{
		llinfos << "checkCircuitBlocked(circuit): Unknown host - " << host
				<< llendl;
		return FALSE;
	}
}
...
BOOL LLMessageSystem::checkCircuitAlive(const U32 circuit)
{
	...
	if (!host.isOk())
	{
		LL_DEBUGS("Messaging") << "checkCircuitAlive: Unknown circuit "
							   << circuit << LL_ENDL;
		return FALSE;
	}
	...
	{
		llinfos << "checkCircuitAlive(circuit): Unknown host - " << host
				<< llendl;
		return FALSE;
	}
}
...
BOOL LLMessageSystem::checkCircuitAlive(const LLHost& host)
{
	...
	{
		LL_DEBUGS("Messaging") << "checkCircuitAlive(host): Unknown host - "
							   << host << LL_ENDL;
		return FALSE;
	}
}
...
#if 0
			LL_DEBUGS("Messaging") << "ack recvd' from " << host
...
void process_create_trusted_circuit(LLMessageSystem *msg, void **)
{
		...
		untrusted_interface == last_interface)
	{
		...
		{
			llwarns << "Ignoring CreateTrustedCircuit on public interface from host: "
					<< msg->getSender() << llendl;
			return;
		}
		else
		{
			llwarns << "Processing CreateTrustedCircuit on public interface from host: "
					<< msg->getSender() << llendl;
		}
	}
...
void process_deny_trusted_circuit(LLMessageSystem *msg, void **)
{
		...
		untrusted_interface == last_interface)
	{
		if (msg->getBlockUntrustedInterface())
		{
			llwarns << "Ignoring DenyTrustedCircuit on public interface from host: "
					<< msg->getSender() << llendl;
			return;
		}
		else
		{
			llwarns << "Processing DenyTrustedCircuit on public interface from host: "
					<< msg->getSender() << llendl;
		}
	}
			 ...
			 iter != end; ++iter)
		{
			...
			if (mt->mReceiveCount > 0)
			{
				llinfos << "Num: " << std::setw(3) << mt->mReceiveCount
						...
						<< " " << mt->mName << " "
						<< llround(100 * mt->mDecodeTimeThisFrame / mReceiveTime)
						<< "%" << llendl;
			}
		}
	}
								  ...
								  LLMessageSystem* msg)
{
	...
	if (iter == mMessageTemplates.end())
	{
		llwarns << "LLMessageSystem::callHandler: unknown message " << name
				<< llendl;
		return false;
	}
	...
	{
		llwarns << "LLMessageSystem::callHandler: banned message " << name
				<< " from " << (trustedSource ? "trusted " : "untrusted ")
				<< "source" << llendl;
		return false;
	}
													  ...
													  const LLUUID& id2) const
{
	...
	{
		llerrs << "Trying to generate complex digest on a machine without a shared secret !"
			   << llendl;
	}
														...
														const LLUUID& id2) const
{
	...
	if (shared_secret.empty())
	{
		llerrs << "Trying to compare complex digests on a machine without a shared secret !"
			   << llendl;
	}
											  ...
											  const S32 window) const
{
	...
	if (shared_secret.empty())
	{
		llerrs << "Trying to generate simple digest on a machine without a shared secret !"
			   << llendl;
	}
												...
												S32 const window) const
{
	...
	if (shared_secret.empty())
	{
		llerrs << "Trying to compare simple digests on a machine without a shared secret !"
			   << llendl;
	}
											   ...
											   const LLUUID& id2)
{
	...
	if (id1.isNull())
	{
		llwarns << "Can't send CreateTrustedCircuit to " << host
				<< " because we don't have the local end point ID" << llendl;
		return;
	}
	if (id2.isNull())
	{
		llwarns << "Can't send CreateTrustedCircuit to " << host
				<< " because we don't have the remote end point ID" << llendl;
		return;
	}
												  ...
												  S64 frame_count)
{
	...
	if (shared_secret.empty())
	{
		llerrs << "Trying to establish bidirectional trust on a machine without a shared secret !"
			   << llendl;
	}
	LLTimer timeout;
...
{
	message_template_name_map_t::iterator itt;
	itt = mMessageTemplates.find(LLMessageStringTable::getInstance()->getString(name.c_str()));
