					...
					const LLSD& headers = LLSD())
{
	...
	LL_DEBUGS("HTTPClient") << LLURLRequest::actionAsVerb(method) << " " << url
							<< " " << headers << LL_ENDL;
    ...
    if (headers.isMap())
    {
        ...
        for ( ; iter != end; ++iter)
        {
            ...
            LL_DEBUGS("HTTPClient") << "header = " << header.str() << LL_ENDL;
...
static LLSD blocking_request(const std::string& url,
							 ...
							 const F32 timeout = 5)
{
	LL_DEBUGS("HTTPClient") << "blockingRequest of " << url << LL_ENDL;
	...
	LL_DEBUGS("HTTPClient") << "HTTP body: " << body_str << LL_ENDL;
	...
	else
	{
		...
		LL_DEBUGS("HTTPClient") << "CURL response: " << http_buffer.asString()
								<< LL_ENDL;
	}
...
void LLHTTPClient::postRaw(const std::string& url,
						   const U8* data,
						   S32 size,
						   ResponderPtr responder,
						   const LLSD& headers,
