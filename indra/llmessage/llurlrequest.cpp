...
void LLURLRequest::useProxy(bool use_proxy)
{
    ...
    LL_DEBUGS("Proxy") << "use_proxy = " << (use_proxy ? 'Y' : 'N')
					   << ", env_proxy = "
					   << (env_proxy ? env_proxy : "(null)")
					   << LL_ENDL;

    if (use_proxy && env_proxy)
    {
											 ...
											 LLPumpIO* pump)
{
	...
	if (pump && mDetail->mByteAccumulator > MIN_ACCUMULATION)
	{
		//LLFastTimer t(LLFastTimer::FTM_URL_ADJUST_TIMEOUT);
		 ...
		 LL_DEBUGS("URLRequest") << "LLURLRequest adjustTimeoutSeconds for request: "
								 << mDetail->mURL << LL_ENDL;
	...
	case STATE_PROCESSING_RESPONSE:
	{
		...
		while (true)
		{
			...
			LL_DEBUGS("URLRequest") << this << "Setting context to "
									<< context << LL_ENDL;
			switch(result)
			{
				case CURLE_OK:
	...
	case STATE_HAVE_RESPONSE:
		...
		context[CONTEXT_REQUEST][CONTEXT_TRANSFERED_BYTES] = mRequestTransferedBytes;
		context[CONTEXT_RESPONSE][CONTEXT_TRANSFERED_BYTES] = mResponseTransferedBytes;
		LL_DEBUGS("URLRequest") << this << "Setting context to " << context
								<< LL_ENDL;
		return STATUS_DONE;
