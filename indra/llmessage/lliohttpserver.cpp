...
LLIOPipe::EStatus LLHTTPPipe::process_impl(
    ...
    LLPumpIO* pump)
{
	//LLFastTimer t(LLFastTimer::FTM_PROCESS_HTTP_PIPE);
	PUMP_DEBUG;
    LL_DEBUGS("IOHTTPServer") << "LLSDHTTPServer::process_impl" << LL_ENDL;
	...
	if (mState == STATE_INVOKE)
	{
		...
		LL_DEBUGS("IOHTTPServer") << verb << " "
								  << context[CONTEXT_REQUEST]["path"].asString()
								  << " " << mStatusCode << " "
								  << mStatusMessage << " " << delta << "s"
								  << LL_ENDL;
...
LLIOPipe::EStatus LLHTTPResponseHeader::process_impl(
	...
	LLPumpIO* pump)
{
	//LLFastTimer t(LLFastTimer::FTM_PROCESS_HTTP_HEADER);
	PUMP_DEBUG;
	...
	S32& len)
{
	...
	if(!newline)
	{
		if(len)
		{
			LL_DEBUGS("IOHTTPServer") << "readHeaderLine failed - too long maybe?"
									  << LL_ENDL;
...
LLIOPipe::EStatus LLHTTPResponder::process_impl(
	...
	LLPumpIO* pump)
{
	//LLFastTimer t(LLFastTimer::FTM_PROCESS_HTTP_RESPONDER);
	PUMP_DEBUG;
	LLMemType m1(LLMemType::MTYPE_IO_HTTP_SERVER);
		...
		if (readHeaderLine(channels, buffer, (U8*)buf, len))
		{
			...
			if(mVerb.empty())
			{
				   ...
				   || (HTTP_VERB_OPTIONS == mVerb))
				{
					...
					LL_DEBUGS("IOHTTPServer") << "http request: " << mVerb
											  << " " << mAbsPathAndQuery
											  << " " << mVersion << LL_ENDL;
				...
				else
				{
					...
					parse_all = false;
					LL_DEBUGS("IOHTTPServer") << "unknown http verb: " << mVerb
											  << LL_ENDL;
					markBad(channels, buffer);
				}
			}
			if(parse_all)
			{
				...
				while(keep_parsing)
				{
					...
					{
						keep_parsing = false;
						LL_DEBUGS("IOHTTPServer") << "bad header: " << buf
												  << LL_ENDL;
						markBad(channels, buffer);
						break;
					}
					...
					{
						LL_DEBUGS("IOHTTPServer") << "Content-Length: "
												  << value << LL_ENDL;
	...
	if(STATE_DONE == mState)
	{
		...
		if(node)
		{
			...
			if (protocolHandler)
			{
				LL_DEBUGS("IOHTTPServer") << "HTTP context: " << context
										  << LL_ENDL;
...
// static
LLHTTPNode& LLIOHTTPServer::create(apr_pool_t* pool, LLPumpIO& pump, U16 port)
{
	LLSocket::ptr_t socket = LLSocket::create(pool,
											  LLSocket::STREAM_TCP,
											  port);
    if(!socket)
    {
