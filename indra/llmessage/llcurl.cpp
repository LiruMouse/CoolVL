...
// virtual
void LLCurl::Responder::errorWithContent(U32 status,
									 ...
									 const LLIOPipe::buffer_ptr_t& buffer)
{
	...
	{
		llinfos << "Failed to deserialize LLSD. " << mURL << " [" << status
				<< "]: " << reason << llendl;
	}
...
LLCurl::Easy* LLCurl::Easy::getEasy()
{
	...
	{
		// this can happen if we have too many open files (fails in
		// c-ares/ares_init.c)
		llwarns << "allocEasyHandle() returned NULL! Easy handles: "
				<< gCurlEasyCount << " Multi handles: " << gCurlMultiCount
				<< llendl;
		delete easy;
		return NULL;
	}
	...
	CURLcode result = curl_easy_setopt(easy->mCurlEasyHandle,
...
U32 LLCurl::Easy::report(CURLcode code)
{
	...
	if (code == CURLE_OK)
	{
		check_curl_code(curl_easy_getinfo(mCurlEasyHandle,
	...
	if (mResponder)
	{
		mResponder->completedRaw(responseCode, responseReason, mChannels,
								 mOutput);
		mResponder = NULL;
	}
							   ...
							   ResponderPtr responder, S32 time_out, bool post)
{
	...
	// don't verify host name so urls with scrubbed host names will work
	...
	mEasyMutexp(NULL)
{
	...
	if (!mCurlMultiHandle)
	{
		llwarns << "LLCurl::newMultiHandle() returned NULL !  Easy handles: "
				<< gCurlEasyCount << " Multi handles: " << gCurlMultiCount
				<< llendl;
		mCurlMultiHandle = LLCurl::newMultiHandle();
	}
...
void LLCurl::Multi::setState(LLCurl::Multi::ePerformState state)
{
	...
	{
		LLCurl::getCurlThread()->setPriority(mHandle,
											 LLQueuedThread::PRIORITY_NORMAL);
	}
}
...
S32 LLCurl::Multi::process()
{
	...
	while ((msg = info_read(&msgs_in_queue)))
	{
		...
		if (msg->msg == CURLMSG_DONE)
		{
			...
			if (response >= 400)
			{
				// failure of some sort, inc mErrorCount for debugging and
				// flagging multi for destruction
				++mErrorCount;
			}
		}
	}
...
{
	LLMutexLock lock(mMutexp);
	CURLMcode mcode = curl_multi_add_handle(mCurlMultiHandle,
											easy->getCurlHandle());
...
//------------------------------------------------------------
//LLCurlThread
LLCurlThread::CurlRequest::CurlRequest(handle_t handle,
									   LLCurl::Multi* multi,
									   LLCurlThread* curl_thread)
:	LLQueuedThread::QueuedRequest(handle, LLQueuedThread::PRIORITY_NORMAL,
								  FLAG_AUTO_COMPLETE),
	mMulti(multi),
...
////////////////////////////////////////////////////////////////////////////
// For generating a simple request for data using one multi and one easy per
// request
...
bool LLCurlRequest::addEasy(LLCurl::Easy* easy)
{
	...
	if (mProcessing)
	{
		llerrs << "Posting to a LLCurlRequest instance from within a responder is not allowed (causes DNS timeouts)."
			   << llendl;
	}
								 ...
								 LLCurl::ResponderPtr responder)
{
	...
	if (length > 0)
	{
		std::string range = llformat("Range: bytes=%d-%d", offset,
									 offset + length - 1);
						 ...
						 S32 time_out)
{
	...
	LL_DEBUGS("Curl") << "POSTING: " << bytes << " bytes." << LL_ENDL;
	bool res = addEasy(easy);
	return res;
}
...
void LLCurlEasyRequest::setSSLCtxCallback(curl_ssl_ctx_callback callback,
										  void* userdata)
{
	if (isValid() && mEasy)
	{
...
bool LLCurlEasyRequest::getResult(CURLcode* result, LLCurl::TransferInfo* info)
{
	...
	if (!mEasy)
	{
		// Special case - we failed to initialize a curl_easy (can happen if
		// too many open files). Act as though the request failed to connect.
		if (mResultReturned)
		{
			return false;
		}
	...
	// In theory, info_read might return a message with a status other than
	// CURLMSG_DONE. In practice for all messages returned, msg == CURLMSG_DONE
...
{
	return isValid() && mEasy ? std::string(mEasy->getErrorBuffer())
							  : std::string();
}
...
void LLCurl::initClass(F32 curl_reuest_timeout,
					   S32 max_number_handles,
					   bool multi_threaded)
{
