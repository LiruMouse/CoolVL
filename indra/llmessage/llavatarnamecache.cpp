...
public:
	...
	/*virtual*/ void result(const LLSD& content)
	{
		...
		for ( ; it != agents.endArray(); ++it)
		{
			...
			LL_DEBUGS("NameCache") << "LLAvatarNameResponder::result for " << agent_id << " "
		...
		if (num_unresolved > 0)
		{
            llwarns << "LLAvatarNameResponder::result " << num_unresolved
					<< " unresolved ids; " << "expires in " << expires - now
					 << " seconds" << llendl;
			...
			for ( ; it != unresolved_agents.endArray(); ++it)
			{
				...
				llwarns << "LLAvatarNameResponder::result " << "failed id "
						<< agent_id << llendl;

                LLAvatarNameCache::handleAgentError(agent_id);
			}
		}
        LL_DEBUGS("NameCache") << "LLAvatarNameResponder::result " 
							   << LLAvatarNameCache::sCache.size()
							   << " cached names" << LL_ENDL;
	}
	...
	/*virtual*/ void error(U32 status, const std::string& reason)
	{
		...
		llwarns << "LLAvatarNameResponder::error " << status << " " << reason
				<< llendl;
...
void LLAvatarNameCache::handleAgentError(const LLUUID& agent_id)
{
	...
	if (existing == sCache.end())
	{
		...
		llwarns << "LLAvatarNameCache get legacy for agent " << agent_id
				<< llendl;
	...
	else
    {
		...
		LL_DEBUGS("NameCache") << "LLAvatarNameCache use cache for agent "
							   << agent_id 
							   << "user '" << av_name.mUsername << "' "
							   << "display '" << av_name.mDisplayName << "' "
							   << "expires in "
...
void LLAvatarNameCache::requestNamesViaCapability()
{
	...
	for ( ; it != sAskQueue.end(); ++it)
	{
		...
		{
			LL_DEBUGS("NameCache") << "LLAvatarNameCache::requestNamesViaCapability first "
								   << ids << " ids" << LL_ENDL;
	...
	if (!url.empty())
	{
		LL_DEBUGS("NameCache") << "LLAvatarNameCache::requestNamesViaCapability all "
							   << ids << " ids" << LL_ENDL;
		LLHTTPClient::get(url, new LLAvatarNameResponder(agent_ids));
		url.clear();
		agent_ids.clear();
										   ...
										   bool is_group)
{
	...
	LLAvatarName av_name;
	LL_DEBUGS("NameCache") << "LLAvatarNameCache::legacyNameCallback "
						   << "agent " << agent_id << " "
						   << "full name '" << full_name << "'"
...
void LLAvatarNameCache::requestNamesViaLegacy()
{
	...
	for (; it != sAskQueue.end(); ++it)
	{
		...
		LL_DEBUGS("NameCache") << "LLAvatarNameCache::requestNamesViaLegacy agent "
							   << agent_id << LL_ENDL;
	...
	if (!sLastExpireCheck || sLastExpireCheck < max_unrefreshed)
	{
		...
		while (it != sCache.end())
		{
			...
			if (av_name.mExpires < max_unrefreshed)
			{
				...
				LL_DEBUGS("NameCache") << agent_id << " user '"
										...
										LLAvatarName* av_name)
{
	...
	LL_DEBUGS("NameCache") << "LLAvatarNameCache::buildLegacyName "
						   << full_name << LL_ENDL;
}
			...
			if (it != sCache.end())
			{
				*av_name = it->second;
				...
				if (av_name->mExpires < LLFrameTimer::getTotalSeconds())
				{
					...
					{
						LL_DEBUGS("NameCache") << "LLAvatarNameCache::get "
