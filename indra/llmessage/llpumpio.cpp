						...
						bool has_curl_request)
{
	...
	LLLinkInfo link;
#if LL_DEBUG_PIPE_TYPE_IN_PUMP
	LL_DEBUGS("PumpIO") << "LLPumpIO::addChain() " << chain[0] << " '"
						<< typeid(*(chain[0])).name() << "'" << LL_ENDL;
#else
	LL_DEBUGS("PumpIO") << "LLPumpIO::addChain() " << chain[0] << LL_ENDL;
#endif
...
#endif
#if LL_DEBUG_PIPE_TYPE_IN_PUMP
	LL_DEBUGS("PumpIO") << "LLPumpIO::addChain() " << links[0].mPipe << " '"
						<< typeid(*(links[0].mPipe)).name() << "'" << LL_ENDL;
#else
	LL_DEBUGS("PumpIO") << "LLPumpIO::addChain() " << links[0].mPipe << LL_ENDL;
#endif
	LLChainInfo info;
	info.setTimeoutSeconds(timeout);
...
void LLPumpIO::cleanup()
{
	...
	if (mPollset)
	{
		//LL_DEBUGS("PumpIO") << "cleaning up pollset" << LL_ENDL;
...
bool LLPumpIO::isChainExpired(LLChainInfo& chain)
{
	...
	for (links_t::iterator iter = chain.mChainLinks.begin();
		 iter != chain.mChainLinks.end(); ++iter)
	{
								...
								LLIOPipe::EStatus error)
{
	...
	do
	{
		...
		case LLIOPipe::STATUS_NEED_PROCESS:
#if LL_DEBUG_PIPE_TYPE_IN_PUMP
			LL_DEBUGS("PumpIO") << "Pipe " << typeid(*((*rit).mPipe)).name()
