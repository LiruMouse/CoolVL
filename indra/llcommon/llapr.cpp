...
//
// define this virtual function to avoid any mistakenly calling
...
LLAPRFile::LLAPRFile(const std::string& filename, apr_int32_t flags,
					 LLVolatileAPRPool* pool)
:	mFile(NULL),
	mCurrentFilePoolp(NULL)
{
...
apr_status_t LLAPRFile::open(const std::string& filename, apr_int32_t flags,
							 LLVolatileAPRPool* pool, S32* sizep)
{
	apr_status_t s;
	...
	s = apr_file_open(&mFile, filename.c_str(), flags, APR_OS_DEFAULT,
					  getAPRFilePool(apr_pool));
...
S32 LLAPRFile::read(void *buf, S32 nbytes)
{
	if (!mFile) 
	{
		llwarns << "apr mFile is removed by somebody else. Can not read."
				<< llendl;
		return 0;
	}
...
S32 LLAPRFile::write(const void *buf, S32 nbytes)
{
	if (!mFile) 
	{
		llwarns << "apr mFile is removed by somebody else. Can not write."
				<< llendl;
		return 0;
	}
...
//static
apr_file_t* LLAPRFile::open(const std::string& filename,
							LLVolatileAPRPool* pool,
							apr_int32_t flags)
{
	apr_status_t s;
	...
	s = apr_file_open(&file_handle, filename.c_str(), flags, APR_OS_DEFAULT,
					  pool->getVolatileAPRPool());
...
//static
S32 LLAPRFile::seek(apr_file_t* file_handle, apr_seek_where_t where,
					S32 offset)
{
	if (!file_handle)
	{
...
//static
S32 LLAPRFile::readEx(const std::string& filename,
					  void* buf,
					  S32 offset,
					  S32 nbytes,
					  LLVolatileAPRPool* pool)
...
//static
S32 LLAPRFile::writeEx(const std::string& filename,
					   void *buf,
					   S32 offset,
					   S32 nbytes,
					   LLVolatileAPRPool* pool)
					   ...
					   LLVolatileAPRPool* pool)
{
	...
	pool = pool ? pool : LLAPRFile::sAPRFilePoolp;
	s = apr_file_rename(filename.c_str(), newname.c_str(),
						pool->getVolatileAPRPool());
	pool->clearVolatileAPRPool();
...
//static
bool LLAPRFile::isExist(const std::string& filename, LLVolatileAPRPool* pool,
						apr_int32_t flags)
{
	apr_file_t* apr_file;
	apr_status_t s;
	...
	pool = pool ? pool : LLAPRFile::sAPRFilePoolp;
	s = apr_file_open(&apr_file, filename.c_str(), flags, APR_OS_DEFAULT,
					  pool->getVolatileAPRPool());
...
S32 LLAPRFile::size(const std::string& filename, LLVolatileAPRPool* pool)
{
	...
	pool = pool ? pool : LLAPRFile::sAPRFilePoolp;
	s = apr_file_open(&apr_file, filename.c_str(), APR_READ, APR_OS_DEFAULT,
					  pool->getVolatileAPRPool());
...
bool LLAPRFile::makeDir(const std::string& dirname, LLVolatileAPRPool* pool)
{
	...
	pool = pool ? pool : LLAPRFile::sAPRFilePoolp;
	s = apr_dir_make(dirname.c_str(), APR_FPROT_OS_DEFAULT,
					 pool->getVolatileAPRPool());
	pool->clearVolatileAPRPool();
