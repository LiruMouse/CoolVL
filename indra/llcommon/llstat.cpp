...
bool LLStatsConfigFile::loadFile()
{
	if (!mStatsp)
	{
		llwarns << "Tries to load performance configure file without initializing LPerfStats"
				<< llendl;
		return false;
	}
	mChanged = true;
	...
	LLSD stats_config;
	{
		...
		if (file.is_open())
		{
			...
			{
				llinfos << "Performance statistics configuration file ill-formed, not recording statistics"
						<< llendl;
		...
		{	// File went away, turn off stats if it was on
			...
			{
				llinfos << "Performance statistics configuration file deleted, not recording statistics"
						<< llendl;
				mStatsp->setReportPerformanceDuration(0.f);
			}
			return true;
	...
	if (duration > 0)
	{
		if (interval == 0.f)
		{
			llinfos << "Recording performance stats every frame for "
					<< duration << " sec" << llendl;
		}
		else
		{
			llinfos << "Recording performance stats every " << interval
					<< " seconds for " << duration << " seconds" << llendl;
		}
	}
	else
	{
...
void	LLPerfStats::openPerfStatsFile()
{
	...
	{
		std::string stats_file = llformat("/dev/shm/simperf/%s_proc.%d.llsd",
										  mProcessName.c_str(), mProcessPID);
		mFrameStatsFile.close();
...
void LLPerfStats::dumpIntervalPerformanceStats()
{
	...
	if (mFrameStatsFile)
	{
		...
		stats["timestamp"] = U64_to_str(totalTime() / 1000 + gUTCOffset * 1000);
...
void LLStatAccum::sum(F64 value, U64 when)
{
	...
	if (when < mLastTime)
	{
		...
		LL_DEBUGS("Stats") << "LLStatAccum::sum clock has gone backwards from "
						   << mLastTime << " to " << when << ", resetting"
						   << LL_ENDL;
...
void LLStatTime::stop()
{
	...
	//llinfos << "mTotalTimeInFrame incremented from  " << mTotalTimeInFrame
	//		<< " to " << (mTotalTimeInFrame + duration) << llendl; 
...
// virtual
F32 LLStatTime::meanValue(TimeScale scale) const
{
...
LLPerfBlock::LLPerfBlock(LLStatTime* stat)
:	mPredefinedStat(stat),
	mDynamicStat(NULL)
{
	if (mPredefinedStat)
	{
		// If dynamic stats are turned on, this will create a separate entry in
		// the stat map.
...
// Use this constructor for dynamically created LLStatTime objects (not
// pre-defined) with a multi-part key.
// These are also turned on or off via the switch passed in.
LLPerfBlock::LLPerfBlock(const char* key1, const char* key2)
:	mPredefinedStat(NULL),
	mDynamicStat(NULL)
{
...
void LLPerfBlock::initDynamicStat(const std::string& key)
{
	...
	{
		llwarns << "Initialized NULL dynamic stat at '" << sCurrentStatPath
				<< "'" << llendl;
...
LLPerfBlock::~LLPerfBlock()
{
	...
	if (mDynamicStat)
	{
		...
		// Restore the path in case sStatsEnabled changed during this block
		sCurrentStatPath = mLastPath;
	}
}
										 ...
										 LLStatAccum::TimeScale scale)
{
	...
	{	// Put the entry into LLSD "/full/path/to/stat/" = microsecond total time
		...
		{	// WTF?  Shouldn't have a NULL pointer in the map.
			llwarns << "Unexpected NULL dynamic stat at '" << stats_full_path
					<< "'" << llendl;
		}
	}	
}
