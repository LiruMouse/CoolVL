...
// Template specialization of char_traits for U16s. Only necessary on Mac and
// Linux (exists on Windows already)
#include <cstring>
...
public:
	...
	static bool		isPartOfLexicalWord(T c) { return (c == (T)'\'') || LLStringOps::isAlpha(c) || !(LLStringOps::isDigit(c) || LLStringOps::isSpace(c) || LLStringOps::isPunct(c)); }
...
{
public:
	explicit LLStringExplicit(const char* s)
	:	std::string(s)
	{
	}
	...
	LLStringExplicit(const std::string& s, size_type pos,
...
// static
template<class T>
bool LLStringUtilBase<T>::startsWith(const std::basic_string<T>& string,
									 const std::basic_string<T>& substr)
{
...
// static
template<class T>
bool LLStringUtilBase<T>::endsWith(const std::basic_string<T>& string,
								   const std::basic_string<T>& substr)
{
	if (string.empty() || (substr.empty())) return false;
										...
										BOOL& value)
{
	...
	if (temp == "0" || temp == "F" || temp == "f" || temp == "FALSE" ||
