...
void LLQueuedThread::shutdown()
{
	...
	while ((req = (QueuedRequest*)mRequestHash.pop_element()))
	{
		if (req->getStatus() == STATUS_QUEUED ||
			req->getStatus() == STATUS_INPROGRESS)
		{
			++active_count;
	...
	if (active_count)
	{
		llwarns << "~LLQueuedThread() called with active requests: "
				<< active_count << llendl;
	}
}
...
// 				check();
		...
		{
			lockData();
			req->decPriority(start_priority);
			req->setStatus(STATUS_QUEUED);
			mRequestQueue.insert(req);
			unlockData();
...
{
	// call checkPause() immediately so we don't try to do anything before the
...
void LLQueuedThread::QueuedRequest::decPriority(U32 old_priority)
{
	if (mPriority != old_priority)
	{
		return;	// mPriority is already reset
	}

	if (mPriority > LLQueuedThread::PRIORITY_LOW)
	{
		mPriority--;
	}
}
