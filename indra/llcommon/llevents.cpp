...
struct RegisterFlush : public LLEventTrackable
{
	...
	{
		pumps.obtain("mainloop").listen("flushLLEventQueues",
										boost::bind(&RegisterFlush::flush,
													this, _1));
	}
...
void LLEventPumps::flush()
{
	...
	for (PumpMap::iterator pmi = mPumpMap.begin(), pmend = mPumpMap.end();
		 pmi != pmend; ++pmi)
	{
		pmi->second->flush();
	}
										 ...
										 const NameList& before)
{
		 ...
		 dmi != sorted_range.end(); ++dmi)
	{
		...
		if (dmi->second < previous)
		{
			...
			std::ostringstream out;
			out << "New listener '" << name << "' on " << typeid(*this).name()
				<< " '" << getName() << "' would move previous listener '"
				<< dmi->first << "'\nwas: ";
...
void LLReqID::stamp(LLSD& response) const
{
	...
	{
		llinfos << "stamp(" << mReqid
				<< ") preserving existing [\"reqid\"] value " << oldReqid
