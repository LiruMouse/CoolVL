...
namespace {
	...
	public:
								   ...
								   const std::string& message)
		{
			...
			// mFile.flush(); // *FIX: should we do this?
		}
...
namespace LLError
{
	...
	// virtual
	bool Recorder::wantsTime()	{ return false; }
	...
	void removeRecorder(Recorder* recorder)
	{
		...
		Settings& s = Settings::get();
		s.recorders.erase(std::remove(s.recorders.begin(), s.recorders.end(),
									  recorder),
						  s.recorders.end());
	}
}
...
namespace
{
	void writeToRecorders(LLError::ELevel level, const std::string& message)
	{
		...
		std::string messageWithTime, last_with_time;
			 ...
			 i != s.recorders.end(); ++i)
		{
			...
			if (r->wantsTime() && s.timeFunction != NULL)
			{
				if (messageWithTime.empty())
				{
					if (repeats > 0)
					{
						last_with_time = s.timeFunction() + " " + last_message;
					}
