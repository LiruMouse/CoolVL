...
LLXMLNode::~LLXMLNode()
{
	...
	// 'delete mChildren'...
	// Sadly, that's only true if we hadn't had reference-counted smart
...
void LLXMLNode::addChild(LLXMLNodePtr new_child, LLXMLNodePtr after_child)
{
	...
	else
	{
		...
		// children for this node. I'm not going to assert that, because it
		// would be expensive, but don't specify that parameter if you didn't
...
void LLXMLNode::updateDefault()
{
	...
	if (mChildren.notNull())
	{
		...
		for (children_itr = mChildren->map.begin();
			 children_itr != children_end; ++children_itr)
		{
...
bool LLXMLNode::updateNode(LLXMLNodePtr& node, LLXMLNodePtr& update_node)
{
	...
	for (itor = update_node->mAttributes.begin();
		 itor != update_node->mAttributes.end(); ++itor)
	{
...
// static
bool LLXMLNode::parseFile(const std::string& filename,
						  LLXMLNodePtr& node,
...
void LLXMLNode::scrubToTree(LLXMLNode* tree)
{
	...
	if (mChildren.notNull())
	{
		...
		for (itor3=to_delete_list.begin(); itor3 != to_delete_list.end();
			 ++itor3)
		{
...
// underlying system is based on BOOL and LLString; if we change  so that
// they're based on more generic mechanisms, these will be simplified.
...
U32 LLXMLNode::getBoolValue(U32 expected_length, BOOL* array)
{
	...
	{
		LL_DEBUGS("XMLNode") << "LLXMLNode::getBoolValue() failed for node named '"
...
U32 LLXMLNode::getByteValue(U32 expected_length, U8* array, Encoding encoding)
{
	...
	{
		LL_DEBUGS("XMLNode") << "LLXMLNode::getByteValue() failed for node named '"
							 << mName->mString << "' -- expected "
							 << expected_length << " but only found " << i
							 << LL_ENDL;
	}
...
U32 LLXMLNode::getIntValue(U32 expected_length, S32* array, Encoding encoding)
{
	...
	if (i != expected_length)
	{
		LL_DEBUGS("XMLNode") << "LLXMLNode::getIntValue() failed for node named '"
							 << mName->mString << "' -- expected "
							 << expected_length << " but only found " << i
							 << LL_ENDL;
	}
								...
								Encoding encoding)
{
	...
	for (i=0; i<expected_length; ++i)
	{
		...
		{
			llwarns << "getUnsignedValue: Value outside of valid range."
					<< llendl;
			break;
		}
		array[i] = U32(value);
	...
	if (i != expected_length)
	{
		LL_DEBUGS("XMLNode") << "LLXMLNode::getUnsignedValue() failed for node named '"
							 << mName->mString << "' -- expected "
							 << expected_length << " but only found " << i
							 << LL_ENDL;
	}
...
U32 LLXMLNode::getLongValue(U32 expected_length, U64* array, Encoding encoding)
{
	...
	if (mLength > 0 && mLength != expected_length)
	{
		llwarns << "XMLNode::getLongValue asked for " << expected_length
				<< " elements, while node has " << mLength << llendl;
		return 0;
	}
	...
	if (i != expected_length)
	{
		LL_DEBUGS("XMLNode") << "LLXMLNode::getLongValue() failed for node named '"
							 << mName->mString << "' -- expected "
							 << expected_length << " but only found " << i
							 << LL_ENDL;
	}
...
U32 LLXMLNode::getFloatValue(U32 expected_length, F32* array, Encoding encoding)
{
	...
	if (mLength > 0 && mLength != expected_length)
	{
		llwarns << "XMLNode::getFloatValue asked for " << expected_length
				<< " elements, while node has " << mLength << llendl;
		return 0;
	}
	...
	if (i != expected_length)
	{
		LL_DEBUGS("XMLNode") << "LLXMLNode::getFloatValue() failed for node named '"
							 << mName->mString << "' -- expected "
							 << expected_length << " but only found " << i
							 << LL_ENDL;
	}
...
U32 LLXMLNode::getDoubleValue(U32 expected_length, F64* array, Encoding encoding)
{
	...
	if (mLength > 0 && mLength != expected_length)
	{
		llwarns << "XMLNode::getDoubleValue asked for " << expected_length
				<< " elements, while node has " << mLength << llendl;
		return 0;
	}
	...
	if (i != expected_length)
	{
		LL_DEBUGS("XMLNode") << "LLXMLNode::getDoubleValue() failed for node named '"
							 << mName->mString << "' -- expected "
							 << expected_length << " but only found " << i
							 << LL_ENDL;
	}
...
U32 LLXMLNode::getStringValue(U32 expected_length, std::string* array)
{
	...
	if (mLength > 0 && mLength != expected_length)
	{
		llwarns << "XMLNode::getStringValue asked for " << expected_length
				<< " elements, while node has " << mLength << llendl;
		return 0;
	}
	...
	if (num_returned_strings != expected_length)
	{
		LL_DEBUGS("XMLNode") << "LLXMLNode::getStringValue() failed for node named '"
							 << mName->mString << "' -- expected "
							 << expected_length << " but only found "
							 << num_returned_strings << LL_ENDL;
	}
...
U32 LLXMLNode::getUUIDValue(U32 expected_length, LLUUID* array)
{
	...
	if (i != expected_length)
	{
		LL_DEBUGS("XMLNode") << "LLXMLNode::getUUIDValue() failed for node named '"
							 << mName->mString << "' -- expected "
							 << expected_length << " but only found " << i
							 << LL_ENDL;
	}
