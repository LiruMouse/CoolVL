...
public:
	...
	// This structure primarily sorts by the tab group, secondarily by the
	...
	S32			getChildCount()	const					{ return mChildListSize; }
	...
	virtual void	userSetShape(const LLRect& new_rect);
	virtual LLView*	findSnapRect(LLRect& new_rect,
	...
	/* virtual */ BOOL	handleKey(KEY key, MASK mask, BOOL called_from_parent);
	/* virtual */ BOOL	handleUnicodeChar(llwchar uni_char,
	...
	void registerEventListener(std::string name,
	...
	// Display mToolTipMsg if no child handles it.
	...
	/*virtual*/ void	localPointToScreen(S32 local_x, S32 local_y,
										   S32* screen_x, S32* screen_y) const;

	template <class T> T* getChild(const std::string& name,
								   BOOL recurse = TRUE,
	...
	template <class T> T& getChildRef(const std::string& name,
									  BOOL recurse = TRUE) const
	{
		return *getChild<T>(name, recurse, TRUE);
	}

	virtual LLView* getChildView(const std::string& name,
								 BOOL recurse = TRUE,
								 BOOL create_if_missing = TRUE) const;
...
protected:
	...
	void			drawDebugRect();
	void			drawChild(LLView* childp, S32 x_offset = 0,
							  S32 y_offset = 0, BOOL force_draw = FALSE);
...
private:
	virtual bool compareTabOrders(const LLView::tab_order_t & a,
