...
LLSpinCtrl::LLSpinCtrl(const std::string& name,
					   const LLRect& rect,
	...
	mbHasBeenSet(FALSE)
{
	...
	if (!label.empty())
	{
		...
		mLabelBox = new LLTextBox(std::string("SpinCtrl Label"), label_rect,
								  label, font);
		addChild(mLabelBox);
	...
	mUpBtn = new LLButton(std::string("SpinCtrl Up"), up_rect, out_id, in_id,
								   LLStringUtil::null,
	...
	LLRect editor_rect(btn_right + 1, centered_top, getRect().getWidth(),
					   centered_bottom);
	mEditor = new LLLineEditor(std::string("SpinCtrl Editor"), editor_rect,
							   LLStringUtil::null, font, MAX_STRING_LENGTH,
							   &LLSpinCtrl::onEditorCommit, NULL, NULL, this,
							   &LLLineEditor::prevalidateFloat);
	mEditor->setFollowsLeft();
	mEditor->setFollowsBottom();
	mEditor->setFocusReceivedCallback(&LLSpinCtrl::onEditorGainFocus, this);
	// RN: this seems to be a BAD IDEA, as it makes the editor behavior
	// different when it has focus than when it doesn't. Instead, if you always
	// have to double click to select all the text, it's easier to understand
...
void LLSpinCtrl::setPrecision(S32 precision)
{
	...
	{
		llerrs << "LLSpinCtrl::setPrecision - precision out of range"
			   << llendl;
		return;
	}
...
BOOL LLSpinCtrl::handleKeyHere(KEY key, MASK mask)
{
	if (mEditor->hasFocus())
	{
		...
		{
			// text editors don't support revert normally (due to user
