...
// Moved triggerAlerts() to llfloaterchat.cpp to break llui/llaudio library
// dependency.
...
S32 LLTextParser::findPattern(const std::string& text, LLSD highlight)
{
	...
	switch ((S32)highlight["condition"])
	{
		...
		case ENDS_WITH:
		{
			...
			if (pos >= 0 && ltext.length() - pattern.length() == pos)
			{
				found = pos;
			}
			break;
...
LLSD LLTextParser::parsePartialLineHighlights(const std::string& text,
											  ...
											  S32 part, S32 index)
{
	...
	for (S32 i = index, count = mHighlights.size(); i < count; ++i)
	{
				...
				if (start >= 0)
				{
					...
					if (start == 0)
					{
						...
						if (end < len)
						{
							...
							end_llsd = parsePartialLineHighlights(text.substr(end),
																  color,
																  newpart, i);
						}
					}
					else
					{
						...
						start_llsd = parsePartialLineHighlights(text.substr(0, start),
																color, newpart,
																i + 1);
						...
						if (end < len)
						{
							...
							end_llsd = parsePartialLineHighlights(text.substr(start + end),
																  color,
																  newpart, i);
						}
						else
...
bool LLTextParser::parseFullLineHighlights(const std::string& text,
										   LLColor4* color)
{
	for (S32 i = 0, count = mHighlights.size(); i < count; ++i)
	{
		if ((S32)mHighlights[i]["highlight"] == ALL ||
			(S32)mHighlights[i]["condition"] == MATCHES)
		{
...
std::string LLTextParser::getFileName()
{
	...
	if (!path.empty())
	{
		path = gDirUtilp->getExpandedFilename(LL_PATH_PER_SL_ACCOUNT,
											  "highlights.xml");
	}
	return path;
}
...
LLSD LLTextParser::loadFromDisk()
{
	...
	if (filename.empty())
	{
		llwarns << "LLTextParser::loadFromDisk() no valid user directory."
				<< llendl;
	}
	else
	{
...
bool LLTextParser::saveToDisk(LLSD highlights)
{
	...
	if (filename.empty())
	{
		llwarns << "LLTextParser::saveToDisk() no valid user directory."
				<< llendl;
		return FALSE;
	}
	llofstream file;
