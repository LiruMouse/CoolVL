...
// NOTE: this is a MINIMAL implementation. The interface will remain, but the
// implementation will (when the time is right) become dynamic and probably use
// external files.
...
LLResMgr::LLResMgr()
{
	...
	// Note: This is only the most bare-bones version. In the future, load
	// these dynamically, on demand.

	// USA Fonts
	for (U32 i = 0; i < LLFONT_COUNT; ++i)
	{
		mUSAFonts[i] = NULL;
	}
...
void LLResMgr::setLocale(LLLOCALE_ID locale_id)
{
	...
	// RN: for now, use normal 'C' locale for everything but specific UI
	...
	switch (locale_id)
	{
		...
		case LLLOCALE_UK:
			...
			break;
		default:
			llerrs << "Undefined locale id" << llendl;
			setLocale(LLLOCALE_USA);
			break;
	}
}
