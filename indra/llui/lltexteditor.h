...
public:
	...
	virtual BOOL	isSpellDirty() const			{ return mWText != mPrevSpelledText; }
	// Clear dirty state
	virtual void	resetSpellDirty()				{ mPrevSpelledText = mWText; }
	...
	void			selectNext(const std::string& search_text_in,
							   BOOL case_insensitive, BOOL wrap = TRUE);
	...
	// appends text at end
	void 			appendText(const std::string &wtext,
							   bool allow_undo,
							   bool prepend_newline,
							   const LLStyleSP stylep = NULL);

	void 			appendColoredText(const std::string &wtext,
									  bool allow_undo,
									  bool prepend_newline,
									  const LLColor4 &color,
									  const std::string& font_name = LLStringUtil::null);
	// if styled text starts a line, you need to prepend a newline.
	void 			appendStyledText(const std::string &new_text,
									 bool allow_undo,
									 bool prepend_newline,
									 LLStyleSP stylep = NULL);
	void			appendHighlightedText(const std::string &new_text,
										  bool allow_undo,
										  bool prepend_newline,
										  S32 highlight_part,
										  LLStyleSP stylep);
	...
	void			getCurrentLineAndColumn(S32* line, S32* col,
											BOOL include_wordwrap);
	...
	void			setSpellCheck(BOOL b)					{ mSpellCheck = b; }
	const BOOL&		getSpellCheck()							{ return mSpellCheck; }
	...
	{
		mURLcallback = callback1;
		mSecondlifeURLcallback = callback2;
		mSecondlifeURLcallbackRightClick = callback3;
	}
...
protected:
	...
	public:
		...
		virtual BOOL	extendAndExecute(LLTextEditor* editor,
										 S32 pos, llwchar c,
										 S32* delta)			{ llassert(0); return 0; }
		...
		// methods
		S32				insert(LLTextEditor* editor, S32 pos,
							   const LLWString &wstr)			{ return editor->insertStringNoUndo(pos, wstr); }
		S32 			remove(LLTextEditor* editor, S32 pos,
							   S32 length)						{ return editor->removeStringNoUndo(pos, length); }
		S32				overwrite(LLTextEditor* editor,
								  S32 pos, llwchar wc)			{ return editor->overwriteCharNoUndo(pos, wc); }
	...
	S32				insert(const S32 pos, const LLWString& wstr,
						   const BOOL group_with_next_op);
	S32				remove(const S32 pos, const S32 length,
						   const BOOL group_with_next_op);
	S32				append(const LLWString& wstr,
						   const BOOL group_with_next_op);

	// Direct operations

	// returns num of chars actually inserted
...
private:
	...
	void			drawMisspelled();
	void			drawText();
	void			drawClippedSegment(const LLWString &wtext, S32 seg_start,
