...
LLView::LLView(const std::string& name,
			   const LLRect& rect,
			   BOOL mouse_opaque,
			   U32 reshape)
:	mParentView(NULL),
									  ...
									  const LLView* const b) const
{
	...
	if (a_score == -2 && b_score == -2)
	{
		...
		LLView::child_tab_order_const_iter_t a_found = mTabOrder.find(a_ctrl),
																	  b_found = mTabOrder.find(b_ctrl);
		if (a_found != mTabOrder.end()) a_score--;
		if (b_found != mTabOrder.end()) b_score--;
		if (a_score == -3 && b_score == -3)
		{
			// whew!  Once we're in here, they're both in the tab order, and we
			// can compare based on that
...
// Handles key in this object. Checking parents and children happens in
// handleKey()
							   ...
							   std::string& tooltip_msg)
{
	...
	if (!handled && blockMouseEvent(x, y))
	{
		*accept = ACCEPT_NO;
		handled = TRUE;
		lldebugst(LLERR_USER_INPUT) << "dragAndDrop handled by LLView "
									<< getName() << llendl;
	}
...
BOOL LLView::handleMouseDown(S32 x, S32 y, MASK mask)
{
	...
	if (sEditingUI && handled_view)
	{
		...
		if (buttonp || line_editorp || text_editorp || text_boxp)
		{
	...
	if (getVisible() && getEnabled())
	{
			 ...
			 child_it != end; ++child_it)
		{
				...
				viewp->handleScrollWheel(local_x, local_y, clicks))
			{
				...
				{
					sMouseHandlerMessage = "->" + viewp->mName + sMouseHandlerMessage;
				}
...
void LLView::drawDebugRect()
{
	LLUI::pushMatrix();
	{
		...
		if (mChildListSize && !sEditingUI)
		{
			...
			LLFontGL::getFontSansSerifSmall()->renderUTF8(debug_text, 0,
														  (F32)x, (F32)y,
														  border_color,
														  LLFontGL::HCENTER,
														  LLFontGL::BASELINE,
...
// getChildView()
//-----------------------------------------------------------------------------
LLView* LLView::getChildView(const std::string& name, BOOL recurse,
							 ...
							 S32 threshold, S32 padding)
{
	...
	if (snap_type == SNAP_PARENT || snap_type == SNAP_PARENT_AND_SIBLINGS)
	{
		...
		{
		case SNAP_RIGHT:
			if ((parent_local_snap_rect.mRight - test_rect.mRight) * mouse_dir.mX >= 0 &&
				llabs(parent_local_snap_rect.mRight - test_rect.mRight) <= x_threshold)
			{
		...
		case SNAP_LEFT:
			if (test_rect.mLeft * mouse_dir.mX <= 0 &&
				llabs(test_rect.mLeft - parent_local_snap_rect.mLeft) <= x_threshold)
			{
		...
		case SNAP_BOTTOM:
			if (test_rect.mBottom * mouse_dir.mY <= 0 &&
				llabs(test_rect.mBottom - parent_local_snap_rect.mBottom) <= y_threshold)
			{
			...
			break;
		case SNAP_TOP:
			if ((parent_local_snap_rect.mTop - test_rect.mTop) * mouse_dir.mY >= 0 &&
				llabs(parent_local_snap_rect.mTop - test_rect.mTop) <= y_threshold)
			{
...
{
	for (dispatch_list_t::const_iterator itor = mDispatchList.begin(),
...
void LLView::setControlName(const std::string& control_name, LLView* context)
{
	...
	if (!mControlName.empty())
	{
		llwarns << "setControlName called twice on same control ("
				<< control_name << ") !" << llendl;
	...
	if (!control_name.empty())
	{
		...
		if (control)
		{
			...
			mControlConnection = control->getSignal()->connect(boost::bind(&controlListener,
															   _2, getHandle(),
															   std::string("value")));
...
{
	mFloaterControls[name] = new LLControlVariable(name, TYPE_BOOLEAN,
												   initial_value,
												   "Internal floater control");
}
