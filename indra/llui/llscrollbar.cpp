...
LLScrollbar::LLScrollbar(const std::string& name,
						 LLRect rect,
	...
	mOnScrollEndData(NULL)
{
	...
	LLButton* line_down_btn = new LLButton(std::string("Line Down"),
										   line_down_rect, line_down_img,
										   line_down_selected_img,
										   LLStringUtil::null,
										   &LLScrollbar::onLineDownBtnPressed,
										   this, LLFontGL::getFontSansSerif());
...
//	llassert(0 <= mDocPos && mDocPos <= getDocPosMax());
	...
	S32 window_length = mOrientation == LLScrollbar::HORIZONTAL ? getRect().getWidth()
																: getRect().getHeight();
	S32 thumb_bg_length = llmax(0, window_length - 2 * SCROLLBAR_SIZE);
	S32 visible_lines = llmin(mDocSize, mPageSize);
	S32 thumb_length = mDocSize ? llmin(llmax(visible_lines * thumb_bg_length / mDocSize,
											  THUMB_MIN_LENGTH),
										thumb_bg_length)
								: thumb_bg_length;
	...
	if (mOrientation == LLScrollbar::VERTICAL)
	{
		...
		S32 thumb_start = variable_lines ? llmin(llmax(thumb_start_max - mDocPos * (thumb_bg_length - thumb_length) / variable_lines,
													   thumb_start_min),
												 thumb_start_max)
										 : thumb_start_max;
	...
	else
	{
		...
		S32 thumb_start = variable_lines ? llmin(llmax(thumb_start_min + mDocPos * (thumb_bg_length - thumb_length) / variable_lines,
													   thumb_start_min),
												 thumb_start_max)
										 : thumb_start_min;
...
BOOL LLScrollbar::handleMouseDown(S32 x, S32 y, MASK mask)
{
	...
	if (!handled_by_child)
	{
		if (mThumbRect.pointInRect(x, y))
		{
			...
			// No handler needed for focus lost since this clas has no state
			// that depends on it.
...
{
	// Note: we don't bother sending the event to the children (the arrow
	// buttons) because they'll capture the mouse whenever they need hover
	// events.
		...
		if (VERTICAL == mOrientation)
		{
			...
			if (delta_pixels != mLastDelta || mDocChanged)
			{
				...
				if (0 < usable_track_length)
				{
					...
					S32 new_pos = llclamp(S32(variable_lines - ratio * variable_lines + 0.5f),
										  0, variable_lines);
									...
									std::string &tooltip_msg)
{
	...
	if (!drop)
	{
		...
		S32 thumb_length = VERTICAL == mOrientation ? mThumbRect.getHeight()
													: mThumbRect.getWidth();
		S32 thumb_track_length = VERTICAL == mOrientation ? getRect().getHeight() - 2 * SCROLLBAR_SIZE
		...
		S32 new_pos = VERTICAL == mOrientation ? llclamp(S32(variable_lines - ratio * variable_lines + 0.5f),
														 0, variable_lines)
											   : llclamp(S32(ratio * variable_lines + 0.5f),
														 0, variable_lines);
