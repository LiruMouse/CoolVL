...
public:
	...
	void			setWrappedText(const LLStringExplicit& text,
								   F32 max_width = -1.0);	// -1 means use existing control width
	...
	void			setClickedCallback(void (*cb)(void *data),
									   void* data = NULL)		{ mClickedCallback = cb; mCallbackUserData = data; }
	...
	virtual BOOL	setTextArg(const std::string& key,
