...
void LLScrollableContainerView::init()
{
	...
	mBorder = new LLViewBorder(std::string("scroll border"), border_rect,
...
{
	//llinfos << "LLScrollableContainerView::scrollVertical() " << new_pos
	//		<< llendl;
	if (mScrolledView)
	{
		LLRect doc_rect = mScrolledView->getRect();
...
BOOL LLScrollableContainerView::needsToScroll(S32 x, S32 y,
											  LLScrollableContainerView::SCROLL_ORIENTATION axis) const
{
		...
		mScrollbar[VERTICAL]->getVisible())
	{
		...
		if (mScrollbar[HORIZONTAL]->getVisible())
		{
			...
			if (right_scroll_rect.pointInRect(x, y) &&
				mScrollbar[HORIZONTAL]->getDocPos() < mScrollbar[HORIZONTAL]->getDocPosMax())
			{
				mScrollbar[HORIZONTAL]->setDocPos(mScrollbar[HORIZONTAL]->getDocPos() + auto_scroll_speed);
				mAutoScrolling = TRUE;
				handled = TRUE;
		...
		if (mScrollbar[VERTICAL]->getVisible())
		{
			...
			if (bottom_scroll_rect.pointInRect(x, y) &&
				mScrollbar[VERTICAL]->getDocPos() < mScrollbar[VERTICAL]->getDocPosMax())
			{
...
BOOL LLScrollableContainerView::handleToolTip(S32 x, S32 y, std::string& msg,
											  LLRect* sticky_rect)
{
	S32 local_x, local_y;
	for (S32 i = 0; i < SCROLLBAR_COUNT; ++i)
...
void LLScrollableContainerView::draw()
{
	if (mAutoScrolling)
	{
		...
		mAutoScrollRate = llmin(mAutoScrollRate + LLFrameTimer::getFrameDeltaTimeF32() * AUTO_SCROLL_RATE_ACCEL,
	...
	if (mScrolledView)
	{
		...
		// Draw the scrolled area.
		{
			...
			BOOL show_v_scrollbar = FALSE;
			BOOL show_h_scrollbar = FALSE;
			calcVisibleSize(mScrolledView->getRect(), &visible_width,
							&visible_height, &show_h_scrollbar,
							&show_v_scrollbar);
	...
	if (show_v_scrollbar)
	{
		if (doc_rect.mTop < getRect().getHeight() - border_width)
		{
			mScrolledView->translate(0,
									 getRect().getHeight() - border_width - doc_rect.mTop);
		}
												 ...
												 const LLCoordGL& desired_offset)
{
	if (!mScrolledView)
	{
		llwarns << "LLScrollableContainerView::scrollToShowRect with no view !"
				<< llendl;
		return;
	}
	...
	// can't be so far left that right side of rect goes off screen, or so far
	// right that left side does
	S32 horiz_offset = llclamp(desired_offset.mX,
							   llmin(0, -visible_width + rect.getWidth()), 0);
	// can't be so high that bottom of rect goes off screen, or so low that top
	// does
	S32 vert_offset = llclamp(desired_offset.mY, 0,
	...
	S32 bottom_offset = vert_offset == 0 ? scrolled_rect.mTop - rect.mBottom - visible_height
										 : top_offset;
	...
	S32 right_offset = horiz_offset == 0 ? rect.mRight - scrolled_rect.mLeft - visible_width
										 : left_offset;
...
LLView* LLScrollableContainerView::fromXML(LLXMLNodePtr node, LLView* parent,
										   LLUICtrlFactory* factory)
{
	...
	LLScrollableContainerView* ret = new LLScrollableContainerView(name, rect,
																   (LLPanel*)NULL,
																   opaque,
																   color);
