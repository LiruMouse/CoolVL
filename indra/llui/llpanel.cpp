...
BOOL LLPanel::childSetTextArg(const std::string& id, const std::string& key,
...
BOOL LLPanel::childSetLabelArg(const std::string& id, const std::string& key,
							   const LLStringExplicit& text)
{
	LLView* child = getChild<LLView>(id);
	if (child)
...
BOOL LLPanel::childSetToolTipArg(const std::string& id, const std::string& key,
								 const LLStringExplicit& text)
{
...
void LLPanel::childShowTab(const std::string& id, const std::string& tabname,
						   bool visible)
{
...
void LLPanel::childSetTabChangeCallback(const std::string& id,
...
void LLPanel::childSetKeystrokeCallback(const std::string& id,
...
void LLPanel::childSetPrevalidate(const std::string& id,
...
void LLPanel::childSetWrappedText(const std::string& id,
...
void LLPanel::childSetAction(const std::string& id, void(*function)(void*),
							 void* value)
{
...
void LLPanel::childSetActionTextbox(const std::string& id,
									void(*function)(void*),
									void* value)
{
...
void LLPanel::childSetControlName(const std::string& id,
...
//virtual
LLView* LLPanel::getChildView(const std::string& name, BOOL recurse,
...
{
	LLEmbeddedPanel(LLPanel* panelp,
		...
		mVisibleAmt(1.f)	// default to fully visible
	{
		...
		mResizeBar = new LLResizeBar(std::string("resizer"), mPanel, LLRect(),
									 min_dim, S32_MAX, side);
	...
	F32 getCollapseFactor()
	{
		...
		{
			F32 collapse_amt = clamp_rescale(mCollapseAmt, 0.f, 1.f, 1.f,
											 (F32)mMinWidth / (F32)llmax(1, mPanel->getRect().getWidth()));
			return mVisibleAmt * collapse_amt;
		}
		else
		{
			F32 collapse_amt = clamp_rescale(mCollapseAmt, 0.f, 1.f, 1.f,
											 llmin(1.f, (F32)mMinHeight / (F32)llmax(1, mPanel->getRect().getHeight())));
			return mVisibleAmt * collapse_amt;
		}
	}
							 ...
							 EAnimate animate, S32 index)
{
	...
	LLEmbeddedPanel* embedded_panel = new LLEmbeddedPanel(panel,
														  mOrientation,
														  min_width,
														  min_height,
														  auto_resize,
...
void LLLayoutStack::updateLayout(BOOL force_resize)
{
		 ...
		 panel_it != end; ++panel_it)
	{
		...
		if (panelp->getVisible()) 
		{
			(*panel_it)->mVisibleAmt = lerp((*panel_it)->mVisibleAmt, 1.f,
											LLCriticalDamp::getInterpolant(ANIM_OPEN_TIME));
		...
		else // not visible
		{
			(*panel_it)->mVisibleAmt = lerp((*panel_it)->mVisibleAmt, 0.f,
											LLCriticalDamp::getInterpolant(ANIM_CLOSE_TIME));
		 ...
		 panel_it != end; ++panel_it)
	{
		...
		// automatically resizing only track total available headroom, but
		// don't use it for automatic resize logic
		 ...
		 panel_it != end; ++panel_it)
	{
		...
		if (mOrientation == HORIZONTAL)
		{
			(*panel_it)->mResizeBar->setResizeLimits((*panel_it)->mMinWidth, 
													 (*panel_it)->mMinWidth + shrink_headroom_total);
		}
		else // VERTICAL
		{
			(*panel_it)->mResizeBar->setResizeLimits((*panel_it)->mMinHeight, 
