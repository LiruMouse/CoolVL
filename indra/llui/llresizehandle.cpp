...
BOOL LLResizeHandle::handleHover(S32 x, S32 y, MASK mask)
{
	...
	if (hasMouseCapture())
	{
		...
		if (resizing_view)
		{
			...
			{
			case LEFT_TOP:
				snap_view = resizing_view->findSnapEdge(scaled_rect.mLeft,
														mouse_dir, SNAP_LEFT,
														SNAP_PARENT_AND_SIBLINGS,
														LLUI::sSnapMargin);
				test_view = resizing_view->findSnapEdge(scaled_rect.mTop,
														mouse_dir, SNAP_TOP,
														SNAP_PARENT_AND_SIBLINGS,
														LLUI::sSnapMargin);
				if (!snap_view)
			...
			case LEFT_BOTTOM:
				snap_view = resizing_view->findSnapEdge(scaled_rect.mLeft,
														...
														SNAP_PARENT_AND_SIBLINGS,
														LLUI::sSnapMargin);
				test_view = resizing_view->findSnapEdge(scaled_rect.mBottom,
				...
				break;
			case RIGHT_TOP:
				snap_view = resizing_view->findSnapEdge(scaled_rect.mRight,
