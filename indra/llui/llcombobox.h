...
public:
	...
	// remove item by index, return TRUE if found and removed:
	...
	// Select current item by name using selectItemByLabel. Returns FALSE
	// if not found:
	...
	// remove item "name", return TRUE if found and removed:
...
{
public:
	LLFlyoutButton(const std::string& name,
				   const LLRect& rect,
				   const std::string& label,
				   void (*commit_callback)(LLUICtrl*, void*) = NULL,
				   void* callback_userdata = NULL);
