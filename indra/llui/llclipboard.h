...
public:
	...
	// copy-and-pasted clipboard. The second is the so-called "primary"
	// clipboard which is implicitly copied upon selection on platforms which
	// expect this (i.e. X11/Linux).

	void				copyFromSubstring(const LLWString& copy_from,
										  S32 pos,
										  S32 len,
	...
	void				copyFromPrimarySubstring(const LLWString& copy_from,
												 S32 pos,
												 S32 len,
												 const LLUUID& source_id = LLUUID::null);
