...
class LLLineEditor : public LLUICtrl, public LLEditMenuHandler, protected LLPreeditor
{
public:
...
private:
	...
	virtual BOOL	getPreeditLocation(S32 query_position,
									   LLCoordGL *coord,
									   LLRect *bounds,
									   LLRect *control) const;
...
private:
	...
	static LLPointer<LLUIImage> parseImage(std::string name,
										   LLXMLNodePtr from,
...
public:
	...
	virtual LLXMLNodePtr getXML(bool save_children = true) const;
	static LLView* fromXML(LLXMLNodePtr node,
						   LLView *parent,
						   LLUICtrlFactory *factory);

	void setText(const LLStringExplicit &new_text)	{ mSearchEdit->setText(new_text); }

	void setSearchCallback(void (*search_callback)(const std::string& search_string,
												   void* user_data),
												   void* data)
	{
