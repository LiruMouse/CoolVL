...
void gl_state_for_2d(S32 width, S32 height)
{
	...
	glLoadIdentity();
	glOrtho(0.0f, llmax(window_width, 1.f), 0.0f, llmax(window_height,1.f),
			-1.0f, 1.0f);
...
void gl_rect_2d_offset_local(S32 left, S32 top, S32 right, S32 bottom,
							 const LLColor4 &color, S32 pixel_offset,
							 BOOL filled)
{
	gGL.color4fv(color.mV);
...
void gl_rect_2d(S32 left, S32 top, S32 right, S32 bottom,
...
void gl_draw_image(S32 x, S32 y, LLTexture* image, const LLColor4& color,
...
void gl_draw_scaled_image(S32 x, S32 y, S32 width, S32 height,
						  LLTexture* image, const LLColor4& color,
						  const LLRectf& uv_rect)
{
	if (NULL == image)
	{
...
void gl_draw_scaled_image_with_border(S32 x, S32 y, S32 border_width,
									  ...
									  LLTexture* image, const LLColor4& color,
									  BOOL solid_color, const LLRectf& uv_rect)
{
	if (NULL == image)
	{
		llwarns << "image == NULL; aborting function" << llendl;
									  ...
									  const LLRectf& scale_rect)
{
	...
	if (solid_color)
	{
		...
		gGL.getTexUnit(0)->setTextureAlphaBlend(LLTexUnit::TBO_MULT,
...
void gl_draw_rotated_image(S32 x, S32 y, F32 degrees, LLTexture* image,
...
void gl_draw_scaled_image_inverted(S32 x, S32 y, S32 width, S32 height,
								   LLTexture* image, const LLColor4& color,
								   const LLRectf& uv_rect)
{
	if (NULL == image)
...
{
	gGL.flush();
	glLineWidth(width * lerp(sGLScaleFactor.mV[VX], sGLScaleFactor.mV[VY],
							 0.5f));
}
...
// On Windows, the user typically sets the language when they install the app
// (by running it with a shortcut that sets InstallLanguage). On Mac or on
// Windows if the viewer executable is ran directly, the language follows the
// OS language. In all cases the user can override the language manually in
// preferences. JC
// static
...
:	mScissorState(GL_SCISSOR_TEST), mEnabled(enabled)
{
	if (mEnabled)
	{
		pushClipRect(rect);
