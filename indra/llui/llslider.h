...
{
public:
	LLSlider(const std::string& name,
			 const LLRect& rect,
			 ...
			 F32 max_value,
			 F32 increment,
			 BOOL volume, // TODO: create a "volume" slider sub-class or just use image art
			 const std::string& control_name = LLStringUtil::null);

	virtual LLXMLNodePtr getXML(bool save_children = true) const;
	static  LLView* fromXML(LLXMLNodePtr node,
							LLView* parent, class LLUICtrlFactory* factory);
	...
	void			setMouseDownCallback(void (*cb)(LLUICtrl*, void*))	{ mMouseDownCallback = cb; }
	void			setMouseUpCallback(void (*cb)(LLUICtrl*, void*))	{ mMouseUpCallback = cb; }
